...
Zotero Agents 是一个面向 Zotero 7 的**框架型插件**，它提供了统一的 AI 与自动化工作流执行壳层：
...
1. 从 [Releases](https://github.com/leike0813/zotero-agents/releases) 页面下载最新的 `.xpi` 文件。
...
1. 打开 `编辑` → `首选项` → `Zotero Agents` → `SkillRunner Local Runtime`
...
_若不使用一键部署_：打开 `编辑` → `首选项` → `Zotero Agents` → `Backend Manager`，添加 Skill-Runner 端点。
