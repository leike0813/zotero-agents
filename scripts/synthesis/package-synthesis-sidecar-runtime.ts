...
async function main() {
  ...
  await fs.copyFile(
    path.join(root, "rust/synthesis-sidecar/licenses.json"),
  ...
  const cargoLockSha256 = await sha256File(
    path.join(root, "rust/synthesis-sidecar/Cargo.lock"),
  );
  const toolchain = (
    await fs.readFile(
      path.join(root, "rust/synthesis-sidecar/rust-toolchain.toml"),
      "utf8",
    )
  ).match(/channel\s*=\s*"([^"]+)"/)?.[1];
  if (!toolchain) {
  ...
  const workspace = await fs.readFile(
    path.join(root, "rust/synthesis-sidecar/Cargo.toml"),
    "utf8",
  );
