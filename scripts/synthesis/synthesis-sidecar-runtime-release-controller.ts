...
async function main() {
  if (process.argv.includes("--help")) {
    process.stdout.write(
      "Usage: tsx scripts/synthesis/synthesis-sidecar-runtime-release-controller.ts init --workflow-run=<url> --release-pipeline-revision=<sha256> [--receipt=<path>]\n       tsx scripts/synthesis/synthesis-sidecar-runtime-release-controller.ts advance --step=<step> [--status=<status>] [--failure=<message>] [--receipt=<path>]\n",
    );
    return;
  }
