...
$cliReleasePath = Join-Path $repoRoot "releases" "host-bridge" "cli-release.json"
$profileTemplate = Join-Path $wrapperSkillRoot "assets" "profile.template.json"
$installPs1 = Join-Path $repoRoot "rust" "zotero-bridge" "scripts" "install.ps1"
$installSh = Join-Path $repoRoot "rust" "zotero-bridge" "scripts" "install.sh"
...
$releaseSetPath = Join-Path $repoRoot "releases" "host-bridge" "release-set.json"
...
if ([string]$releaseSet.schema -notin @("host-bridge.release-set.v1", "host-bridge.release-set.v2", "host-bridge.release-set.v3", "host-bridge.release-set.v4")) {
...
if ([string]$cliRelease.version -ne $cliVersion) {
  throw "bundle CLI version does not match releases/host-bridge/cli-release.json"
}
