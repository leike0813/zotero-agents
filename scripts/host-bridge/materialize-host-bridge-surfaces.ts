...
export function materializeHostBridgeSurfaces(args: {
  ...
  copy(
    join(root, "rust/zotero-bridge/scripts/install.ps1"),
    join(cliBundle, "install.ps1"),
  );
  copy(
    join(root, "rust/zotero-bridge/scripts/install.sh"),
...
function stagingReleaseSet(root: string): ReleaseSet {
  const cli = readJson(join(root, "releases/host-bridge/cli-release.json")) as {
    version?: string;
