...
export function buildHostBridgeReviewMirrorInventory(
  ...
  const definitionsPath = join(root, "contracts/host-bridge/surfaces.json");
  ...
  return {
    ...
    candidateReleaseSet: releaseIdentity(
      readJson(join(root, "releases/host-bridge/release-set.json")),
    ),
      ...
      : command === "finalize"
        ...
        : command === "check"
          ? checkHostBridgeReviewMirror({
              targetRoot: option("target") || "artifacts/host-bridge-review",
            })
