...
.DESCRIPTION
    ...
    new orphan snapshot.
.EXAMPLE
    .\scripts\host-bridge\publish-host-bridge-cli-bundle.ps1 -DryRun
.EXAMPLE
    .\scripts\host-bridge\publish-host-bridge-cli-bundle.ps1 -AllowDirty
.EXAMPLE
    .\scripts\host-bridge\publish-host-bridge-cli-bundle.ps1 -Push
#>
[CmdletBinding()]
param(
...
$InstallPs1Path = Join-Path $DevRoot 'rust' 'zotero-bridge' 'scripts' 'install.ps1'
$InstallShPath = Join-Path $DevRoot 'rust' 'zotero-bridge' 'scripts' 'install.sh'
$BinRoot = Join-Path $DevRoot 'addon' 'bin'
$ReleaseSetPath = Join-Path $DevRoot 'releases' 'host-bridge' 'release-set.json'
...
if (-not (Test-Path -LiteralPath $ReleaseSetPath)) {
    Log-Error "Release set not found at releases/host-bridge/release-set.json"
}
$releaseSet = Get-Content -Encoding UTF8 -LiteralPath $ReleaseSetPath -Raw | ConvertFrom-Json
if ([string]$releaseSet.schema -notin @('host-bridge.release-set.v1', 'host-bridge.release-set.v2', 'host-bridge.release-set.v3', 'host-bridge.release-set.v4') -or -not [string]$releaseSet.releaseSetId) {
