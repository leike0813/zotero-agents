...
export function classifyHostBridgeReleaseChanges(
  ...
  const cliBinaryInputs = files.some(
    (path) =>
      path.startsWith("rust/zotero-bridge/src/") ||
      path === "rust/zotero-bridge/Cargo.toml" ||
      path === "rust/zotero-bridge/Cargo.lock" ||
      path === "rust/zotero-bridge/cli-build-recipe.json" ||
      path === "scripts/host-bridge/build-zotero-bridge-cli.mjs" ||
      path === "scripts/host-bridge/package-zotero-bridge-cli.mjs",
  );
  const installers = sourceFiles.some(
    (path) =>
      path.startsWith("rust/zotero-bridge/scripts/") ||
