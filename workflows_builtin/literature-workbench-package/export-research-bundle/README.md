...
全自动执行。系统先评估现有 Topic，收集所有选中 Topic 的 `resolved_paper_set` 并按 `paper_ref` 去重；随后执行有界 Zotero 元数据锚点检索，将搜索结果并入同一候选集合，再进行文献评估并区分核心文献与相关文献。检索匹配题名、作者、年份、刊名和标签等已索引元数据，不执行全文或摘要语义检索。选中 Topic 的关联文献会强制保留，即使语义评分低于普通阈值或总数超过非 Topic 文献上限。
...
Topic、图谱、分析产物或原文不可用时，workflow 会使用仍可读取的证据继续执行，并在结果中记录诊断和警告。若候选为零且 Topic 或库检索状态不可确认，Stage 40 会保留为可重试错误，不会误报“无相关文献”；只有确认空集或完成文献评估后，才可能以无相关文献结束且不登记研究产品。
