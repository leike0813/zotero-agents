...
function tagEntryFromSynthesis(entry) {
  ...
  return {
    tag,
    facet,
    source: asString(entry?.source || "synthesis"),
    note: asString(entry?.note),
    deprecated: Boolean(entry?.deprecated),
  };
}
  ...
  return normalizeStagedEntryWithBindings(
    {
      ...
      parentBindings: Array.isArray(entry?.parent_bindings)
        ? entry.parent_bindings
        : [],
...
async function loadSynthesisControlledState() {
  ...
  const entries = normalizePersistedEntries(
    (Array.isArray(snapshot?.entries) ? snapshot.entries : [])
      ...
      .filter((entry) => entry.tag && !entry.deprecated),
  );
  return {
    corrupted: false,
    entries,
...
async function persistSynthesisStagedEntries(entries) {
  ...
  await synthesis.stageTagSuggestions({
    entries: normalized.map((entry) => ({
      ...
      facet: entry.facet,
      note: entry.note,
      source_flow: entry.sourceFlow || STAGED_SOURCE_FLOW,
      parent_bindings: normalizeParentBindings(entry.parentBindings || []),
    })),
  });
...
async function loadControlledVocabularySnapshot(tagVocabularyBridge) {
  const bridge = tagVocabularyBridge || resolveTagVocabularyBridge();
  const loaded = await bridge.loadPersistedState();
...
async function loadStagedVocabularySnapshot(tagVocabularyBridge) {
  const bridge = tagVocabularyBridge || resolveTagVocabularyBridge();
  const loaded = await bridge.loadPersistedStagedState();
  const entries = Array.isArray(loaded.entries) ? [...loaded.entries] : [];
  return {
    entries,
    lowerSet: new Set(
...
async function intakeSuggestTagsToStaged(args) {
  ...
  if (summary.staged.length > 0) {
    try {
      await tagVocabularyBridge.persistStagedEntries(nextStaged);
      appendTagRegulatorSuggestLog({
...
async function intakeSuggestTagsToVocabulary(args) {
  ...
  if (summary.added.length > 0) {
    ...
    try {
      ...
      try {
        await tagVocabularyBridge.persistStagedEntries(fallbackStaged);
        appendTagRegulatorSuggestLog({
...
async function reconcileSuggestTagsAgainstCurrentState(args) {
  const suggestTagEntries = Array.isArray(args?.suggestTagEntries)
    ? args.suggestTagEntries
    : [];
  if (suggestTagEntries.length === 0) {
  ...
  const stagedReconciled = await mergeCurrentParentIntoStagedSuggestEntries({
...
async function mergeCurrentParentIntoStagedSuggestEntries(args) {
  const tagVocabularyBridge = resolveTagVocabularyBridge();
  const suggestTagEntries = Array.isArray(args?.suggestTagEntries)
    ? args.suggestTagEntries
    : [];
