const COPY = {
  "en-US": {
    ...
    layoutBody: "Topic and paper material is organized under `topics/` and `papers/`; the root also contains the README, manifest, and bibliography. Every Topic and paper has its own stable logical-ID directory; paper payloads remain directly inside that paper directory. Markdown images are packaged only from the source Markdown directory tree and retain their source-relative paths.",
...
export function renderResearchBundleReadme(args = {}) {
  ...
  const bibliography = args.bibliography || {};
  const bibliographyLabel = bibliography.status === "generated"
    ? text(bibliography.translator?.label || bibliography.actual_format, copy.unavailable)
    : copy.unavailable;
  ...
  const lines = [
    ...
    `- \`references.bib\` — ${bibliographyLabel}.`,
    "",
    `## ${copy.topicIndex}`,
    "",
