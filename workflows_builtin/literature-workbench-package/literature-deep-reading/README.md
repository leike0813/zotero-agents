...
- **选中父条目**：插件会在该条目的附件中按文献 source policy 选择一个来源
- **选中父条目和附件时**：父条目归属优先；直接选中的附件只为未同时选中的父条目提供来源
- **来源优先级**：Markdown 优先于 PDF；多个 PDF 按 `createdAt` 最早、文件名稳定排序，多个 Markdown 优先选择与最早 PDF basename 相同者，否则按同样顺序选择最早 Markdown；没有 Markdown 时选择最早 PDF
- **只处理首篇**：每个父条目只处理一个附件
...
HTML 成功写入并附加到父条目后，workflow 会移除条目上的 `status:need-deep-reading`。失败、跳过或尚未附加正式产物时保留；状态清理失败只产生部分成功警告，不回滚 HTML 产物。
