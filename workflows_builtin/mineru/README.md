...
3. 点击 **从预设添加 Generic HTTP Profile**
4. 选择 `MinerU Official` 并确认，然后将 token 占位提示替换为上一步获取的 API Token：

| 字段 | 值 |
|------|-----|
| 显示名称 | `MinerU Official` |
...
MinerU 精准解析接口对单次解析有页数限制。此 workflow 会在执行前读取 PDF 页数：

- **≤200 页**：按普通单文件流程提交
- **>200 页**：按页码范围拆成多个任务，每段不超过 200 页
- **无法读取页数**：不猜测拆分，按普通单文件流程提交，并在运行诊断中记录原因

分片优先使用 PDF 书签/目录中的章、节、chapter、section 边界；没有可靠目录时按页数均衡拆分。分片只通过 MinerU 的页码范围参数提交，仍上传原 PDF 文件，因此不能规避文件大小限制。
...
| 常规（15-40 页） | 1-2 分钟 |
| 长论文（40+ 页） | 2-3 分钟 |
| 长文档（>200 页） | 拆成多个任务，整体耗时取决于分片数和 MinerU 队列 |
...
- **内容**：解析后的 Markdown 文本
- **编码**：UTF-8
- 长 PDF 分片结果会按页码顺序拼接，分片之间保留一个空行

### 2. 图片目录
- **位置**：PDF 同目录 `Images_<itemKey>/`
- **内容**：从 PDF 提取的图片文件
- Markdown 中的图片路径已自动改写为本地相对路径
- 长 PDF 分片的图片会合并到同一个图片目录，不增加分片子目录
