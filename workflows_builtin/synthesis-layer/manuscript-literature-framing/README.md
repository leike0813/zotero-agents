...
2. **材料收集**：从 Zotero 库检索相关文献，固化四件套状态、文献质量快照与证据角色
3. **多角度框架分析**：分析论文定位和叙事线
4. **写作计划**：生成 Introduction/Related Work 结构计划
5. **草稿生成**：输出 LaTeX 草稿 + 引用映射 + 证据清单

材料收集阶段生成 `writing.manuscript_evidence_inventory@1.0.0`。每篇确认范围内的文献记录 `paper_ref`、四件套 manifest 状态、固化的 `literature_quality`、`evidence_role`、理由和 caveats。质量只用于校准主证据、辅助证据、背景、反证和局限材料的使用方式与措辞强度；不会形成综合排名，也不会硬过滤低分文献。后续框架分析、写作计划和草稿均复用这份快照，不再重新评价文献内在质量。
