...
All five Git and five WebDAV runtime commands route through the shared `SynthesisClient.sync` transport contract. The Workbench acquires a fresh client inside the existing single-flight closure for every command, preserves action trimming and the `keep_local` default, and keeps run/retry failure-state handling separate from pause/resume/conflict result handling. Only `syncWebDavNow` remains deferred; the other nine commands start immediately. Sync polling and the chrome refresh fast path are unchanged. State/configuration/credential/connection-test projections remain outside this command capability, and production Workbench has no full-service import.
