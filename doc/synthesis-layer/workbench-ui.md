...
Tag Vocabulary validation, projection rebuild, regulator export, atomic canonical entry update/delete, atomic staged update, staged bulk promote/discard/clear, and import preview/apply route through `SynthesisClient.tags`. The maintenance/export commands retain their existing empty single-flight arguments, confirmation, start timing, progress polling, clipboard ownership, and Home/Tags refresh behavior. Canonical entry update/delete retain trimmed identifiers, facet-prefix fallback, note normalization, `{ originalTag }` single-flight keys, empty-input skips, immediate start, Tags-only invalidation, and the existing UI-owned delete confirmation. Their strict DTOs discard unknown fields and return opaque results through singular diagnostic failure handling. The Tag domain performs canonical entry read, conflict/not-found handling, metadata-preserving rename or deletion, alias/replacement maintenance, validation, and persistence in one repository transaction; delete-missing is a no-op, and only a committed mutation notifies autosync. Staged update retains original-tag/facet/source-flow fallbacks, snake-case host aliases, the `{ tag }` single-flight key, empty-tag skip, immediate start, and Tags-only invalidation. Its strict DTO trims strings, permits an empty note, and canonicalizes positive parent bindings; the Tag domain performs update, rename, collision merge, and casing cleanup in one repository transaction without protocol validation or canonical autosync. Empty note continues to preserve an existing target note. Promote/discard retain the array-or-singular input normalization, String/trim/empty filtering, first-tag single-flight key, and Workbench-side empty-selection skip; their strict client selection DTO also accepts an empty array for Workflow Host no-op compatibility. Clear keeps its no-argument global key. All three staged bulk commands start immediately, add no confirmation, deferred start, progress callback, streaming, or singular diagnostic transformation, and refresh only Tags. `importTagVocabulary` and `previewTagVocabularyImport` continue to share the preview operation and global empty-args key, while apply retains its action-only key and two-action allowlist. Both import operations validate non-blank primitive strings without rewriting payload whitespace, remain immediate and callback-free, and refresh only Tags. Bootstrap remains on its current path, audit remains a Workflow Host client operation, and Sync is the final direct-service Workbench slice.
