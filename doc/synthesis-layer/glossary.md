...
| Public Maintenance Operation | Durable, user-visible identity for accepted library maintenance. Its lifecycle is owned by the sidecar maintenance module from admission through dispatch, progress, terminal receipt, control, and restart reconciliation. |
| Maintenance Operation View | Typed read projection of a Public Maintenance Operation. It exposes stable identity, scope, lifecycle state, progress, timestamps, and an optional terminal receipt without exposing the persisted basis or repository record. |
| Maintenance Receipt | Durable terminal business outcome attached to a Public Maintenance Operation. It records success, failure, cancellation, or timeout plus retryability and sanitized diagnostics. |
| Maintenance Continuation | Explicit permission to dispatch the same pending operation after restart reconciliation changed it to `continuation_required`. It preserves operation identity and does not publish another operation-started event. |
| Retry Successor | New Public Maintenance Operation created from a retryable terminal predecessor and an idempotency key. The successor has its own identity, lifecycle, and single operation-started event. |
| Execution Ownership | Compare-and-set winner that alone may dispatch work or publish a lifecycle event. Durable insert owns initial submit and retry; the `continuation_required` to `queued` transition owns continuation. |
