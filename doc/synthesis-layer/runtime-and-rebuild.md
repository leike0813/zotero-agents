...
The WS5 repository slice uses `packages/synthesis-repository` as the environment-neutral SSOT for `synt_schema_meta`, `synt_cache_basis`, and `synt_operation`. The Node service opens a persistent shadow database only below its profile runtime root, validates a profile/data-root/schema identity marker, and cancels interrupted shadow operations on restart before discovery is published. Health and handshake expose only an O(1) `isolated_shadow` identity snapshot. The plugin repository reuses the same foundation DDL and CRUD, but production `state/synthesis.db`, canonical Topic files, all public use cases, and production mutation authority remain plugin-owned. This slice is infrastructure for WS6 shadow parity; WS7 must perform a separate atomic single-writer cutover with no automatic in-process fallback.
