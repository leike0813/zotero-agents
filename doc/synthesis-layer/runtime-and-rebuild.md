...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for `synt_schema_meta`, `synt_cache_basis`, and `synt_operation`, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database only below its profile runtime root, validates a profile/data-root/schema identity marker, and cancels interrupted shadow operations on restart before discovery is published. Health and handshake expose only an O(1) `isolated_shadow` identity snapshot. The authenticated `workbench.chrome.read` canary projects fixed cache readiness and bounded operation jobs from that shadow through the same application query reused by plugin chrome; it does not expose repository paths or plugin-only storage, WebDAV, review, or canonical state. Production `SynthesisClient`, `state/synthesis.db`, canonical Topic files, and mutation authority remain plugin-owned. WS6 still owns parity verification and WS7 must perform a separate atomic single-writer cutover with no automatic fallback.
