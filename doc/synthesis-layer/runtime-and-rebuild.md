...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation, Topic, Reference Refresh, Reference Matching/Review, Citation Graph, and Tag Vocabulary projection facts, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates identity/schema markers, reconciles interrupted operations, and recovers the canonical journal before discovery. It composes five private domain applications after recovery. Reference Refresh accepts complete library summaries and artifact descriptors, creates an exact changed-payload read plan, then consumes one hash-bound materialization for a transactional full or bounded-source CAS promotion. Reference Matching/Review separately prepares both strict matcher passes over one materialized Host snapshot and applies them only after Host/reference basis recapture; proposal decisions share production status and accepted-fact revocation semantics. Tag Vocabulary exposes the 15 non-import/checkpoint use cases against isolated state, dispatches validation and index construction through the bounded worker, and atomically promotes vocabulary, staged suggestions, and pending Host effects after revision recapture. Missing or failed Host delivery leaves effects pending and never rolls back committed vocabulary. Digest remains descriptor-only; graph and related projections are only marked stale when graph facts change and are never executed by either reference application. The Graph application retains worker-backed strict full rebuild and graph-basis-guarded metrics/layout. Reads are indexed, bounded, and restart-persistent; mutation leases fail competitors immediately. Health and handshake remain O(1) path-free identities, and the HTTP router exposes none of these application mutation surfaces. Production `SynthesisClient`, Host reads/writes, `state/synthesis.db`, canonical Topic files, graph basis/promotion, Tag import/checkpoint/WebDAV, downstream effects, and mutation authority remain plugin-owned. WS6 still owns parity verification and WS7 requires a separate atomic single-writer cutover with no automatic fallback.
