...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation, Topic, Reference Refresh, Reference Matching/Review, Citation Graph, Tag Vocabulary, and Concept KB projection facts, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates identity/schema markers, reconciles interrupted operations, and recovers the canonical journal before discovery. It composes six private domain applications after recovery. Concept KB keeps concepts, senses, aliases, relations, review items, topic links, manifest revision, and last-good index state in one CAS aggregate. Proposal/review/display/delete policy stays in the application; strict index and read-only query computation use the bounded worker. Index promotion recaptures the manifest and failure preserves last-good state. Health and handshake remain O(1) path-free identities, and the HTTP router exposes none of these application mutation surfaces. Production `SynthesisClient`, Host reads/writes, `state/synthesis.db`, canonical Topic files, graph basis/promotion, Tag and Concept import/checkpoint/WebDAV, downstream effects, and mutation authority remain plugin-owned. WS6 still owns parity verification and WS7 requires a separate atomic single-writer cutover with no automatic fallback.
