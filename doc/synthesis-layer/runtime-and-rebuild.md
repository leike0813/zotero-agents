...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for `synt_schema_meta`, `synt_cache_basis`, and `synt_operation`, with `packages/synthesis-application` above it for bounded use cases and Topic canonical semantics. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates profile/data-root/schema identity markers, cancels interrupted shadow operations, and recovers the single canonical transaction journal before discovery. Health and handshake expose only O(1) path-free identities. `workbench.chrome.read` projects fixed operational chrome; authenticated `topics.canonical.inspect` returns only `absent|ready|invalid`, hashes, and sorted section descriptors and never enters the worker pool. Canonical JSON, SHA-256, path identity, section filename, and text rules are shared with plugin compatibility exports, but no Topic apply/promote capability or production route exists. Production `SynthesisClient`, `state/synthesis.db`, canonical Topic files, and mutation authority remain plugin-owned. WS6 still owns parity verification and WS7 must perform a separate atomic single-writer cutover with no automatic fallback.
