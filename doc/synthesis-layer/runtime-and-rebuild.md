...
The approved Stage 1 sidecar direction is documented in `artifact/synthesis_sidecar_service_stage1_refactor_plan_20260715.md` and the active Synthesis migration OpenSpec changes. The repository now contains an independently compiled Node service foundation under `apps/synthesis-service`: it provides loopback liveness, authenticated protocol/profile/schema handshake, separate lifecycle-token shutdown, bounded request parsing, redacted lifecycle diagnostics, and a service-main-process isolated repository foundation. Five platform bundles combine that service tree with product-owned Node `24.18.0`; strict manifests, signed upstream provenance, per-file hashes, staged installation, atomic active/previous pointers, repair, and rollback produce trusted absolute runtime paths.

The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation plus Topic, Reference Refresh, Reference Matching/Review, Citation Graph, Tag Vocabulary, Concept KB, and Topic Graph shadow facts, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates identity/schema markers, reconciles interrupted operations, and recovers canonical journals before readiness. It composes seven private domain applications, one private knowledge-checkpoint coordinator, one private durable bundle application, and one private WebDAV application. The durable application recognizes all 23 durable entity kinds, emits deterministic v2 bundles, verifies v2 or legacy v1 sources, and proves stable export capture by recapturing repository and canonical bases after Topic current reads. Its private import path previews deterministic diffs/conflicts into one single-use receipt, blocks tombstone apply, and commits live facts under repository aggregate/index CAS. Topic JSON/Markdown uses a strict multi-Topic batch coordinated by the SQLite commit receipt, so restart rolls a matching commit forward and discards an uncommitted batch. The WebDAV application consumes only durable and strict secret-free Host ports, persists canonical queue/conflict state under the opaque profile/data-root identity, validates HEAD before remote assets, applies only a clean acknowledged preview, publishes bundles then manifest then conditional HEAD, bounds retries to four delays, and drains before the durable/canonical/repository owners close. Its default Node Host port is disabled, it has no automatic trigger, and the HTTP router exposes none of these private surfaces. Production `SynthesisClient`, Host credentials/HTTP, `state/synthesis.db`, canonical Topic files, discovery effects, and mutation authority remain plugin-owned. WS5 next covers debug/maintenance foundations; WS6 owns parity verification and WS7 requires a separate atomic single-writer cutover with no automatic fallback.
...
- Tag Vocabulary validation and index construction use one strict engine capped at 25,000 entries, 50,000 aliases, 10,000 abbreviations, and 256 facets. The private sidecar application computes through two bounded worker operations outside SQLite, recaptures the vocabulary revision, and promotes by CAS; production canonical mutation validation remains synchronous inside the existing plugin repository transaction until cutover. Index failure or malformed output cannot advance active index state.
- Concept KB index and query computation uses one strict asynchronous engine capped at 25,000 concepts, 100,000 senses, 250,000 aliases, 256 aliases per concept, 100 query labels, and 4,096 code units per string. The private application invokes two internal worker operations, promotes only against the captured manifest, and never writes during query. Production projection promotion remains plugin-owned; proposal matching is not part of the engine.
- Topic Graph index computation uses one strict asynchronous engine capped at 25,000 nodes, 100,000 edges, and 4,096 code units per string. It derives only roots and unplaced identifiers. The private application invokes one bounded worker operation and promotes only against the captured manifest; production graph/review rows, canonical files, discovery effects, public DTOs, and projection registry remain plugin-owned.
