...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation plus Topic, Reference Refresh, Reference Matching/Review, Citation Graph, Tag Vocabulary, Concept KB, and Topic Graph shadow facts, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates identity/schema markers, reconciles interrupted operations, and recovers the canonical journal before discovery. It composes seven private domain applications plus one private knowledge-checkpoint coordinator after recovery. The coordinator captures active Tag/Concept/Topic Graph rows atomically, verifies a strict versioned bounded payload, previews full-replacement diffs with one in-process receipt, and applies only after three-basis recapture in one SQLite transaction. It leaves Tag staged/audit/pending-effect rows unchanged and preserves all three last-good index payloads as stale. Health and handshake remain O(1) path-free identities, and the HTTP router exposes none of these mutation surfaces. Production `SynthesisClient`, Host reads/writes, `state/synthesis.db`, canonical Topic files, graph basis/promotion, canonical checkpoint/JSON import/WebDAV, discovery effects, and mutation authority remain plugin-owned. Durable bundle transport remains a later WS5 slice; WS6 owns parity verification and WS7 requires a separate atomic single-writer cutover with no automatic fallback.
