...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation plus Topic, Reference Refresh, Reference Matching/Review, Citation Graph, Tag Vocabulary, Concept KB, and Topic Graph shadow facts, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates identity/schema markers, reconciles interrupted operations, and recovers the canonical journal before discovery. It composes seven private domain applications, one private knowledge-checkpoint coordinator, and one private durable bundle exporter after recovery. The exporter recognizes all 23 durable entity kinds, emits deterministic v2 bundles, verifies v2 or legacy v1 sources, and proves a stable cross-storage snapshot by recapturing repository and canonical bases after Topic current reads. It writes bundles before the manifest, admits one operation, and drains before the canonical store and repository close. Health and handshake remain O(1) path-free identities, and the HTTP router exposes none of these private surfaces. Production `SynthesisClient`, Host reads/writes, `state/synthesis.db`, canonical Topic files, graph basis/promotion, import/apply, sync index, WebDAV transport, discovery effects, and mutation authority remain plugin-owned. Durable import and WebDAV composition remain later WS5 slices; WS6 owns parity verification and WS7 requires a separate atomic single-writer cutover with no automatic fallback.
