...
The runtime keeps the authenticated Node v1 HTTP front door and uses one product-owned Rust child for Citation Graph Metrics plus the five private deterministic operations: Tag Vocabulary validation/index, Concept KB index/query, and Topic Graph index. The native workspace owns strict cross-language DTO/canonical semantics, the six Rust kernels, acknowledged bounded paging for large deterministic payloads, JSON-lines worker framing, and a dependency-free candidate HTTP mode. Layout, graph build/transfer, the remaining kernels, repository, application, lifecycle, and the formal runtime manifest remain Node-owned until their dedicated Rust stages. The production DB and canonical owner remain plugin-side, and the Rust routes have no TypeScript fallback. Force layout moves through an explicit layout v2 contract.
