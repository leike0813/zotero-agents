...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation, Topic application, and Citation Graph projection rows, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates identity/schema markers, reconciles interrupted operations, and recovers the canonical journal before discovery. It composes private Topic and Citation Graph applications after recovery. The graph application accepts only strict full rebuild input, executes build/metrics/layout through the existing bounded worker, transactionally compare-and-swap promotes structure/light metrics, and graph-basis-guards complex metrics and explicit layouts. Reads are indexed/bounded and restart-persistent; one mutation lease fails competing work immediately. Health and handshake remain O(1) path-free identities, and the HTTP router exposes neither application mutation surface. Production `SynthesisClient`, `state/synthesis.db`, canonical Topic files, graph basis/promotion, and mutation authority remain plugin-owned. WS6 still owns parity verification and WS7 requires a separate atomic single-writer cutover with no automatic fallback.
