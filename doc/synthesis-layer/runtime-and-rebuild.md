...
Workflow Topic option queries, startup runtime reconciliation, protected database reset, default-client invalidation, related-items notifier echo classification, Workflow Host synthesis operations, both production and read-only Workbench chrome, surface, Topic detail, and paper digest reads, production Topic Report export reads, production Workbench progress polling, Citation Graph layout/cache commands, all Workbench Reference commands, Workbench Concept commands, Topic artifact delete/purge plus discovery-hint reject/restore, Workbench Topic Graph commands, Tag Vocabulary validation/rebuild/regulator export, atomic canonical entry update/delete, atomic staged Tag update, staged Tag bulk promote/discard/clear, and Tag import preview/apply enter through grouped `SynthesisClient` capabilities. Both Workbench paths keep chrome and surfaces separate and do not use the legacy full snapshot. Citation Graph commands use `client.graph`; Reference commands use `client.references`; Concept rebuild, display-text update, review, and deletion use `client.concepts`; Topic artifact/discovery commands use `client.topics`; Topic Graph rebuild and decisions use `client.topicGraph`; the migrated Tag maintenance/export, canonical entry mutation, staged update/bulk, and import slices use `client.tags`. Maintenance, Concept, Topic Graph, and Tag rebuild contracts carry no UI callback or streaming contracts, and persisted operation progress is surfaced by the existing 500 ms no-argument `workbench.readProgress()` poll, which does not reconcile or mutate operation rows. Reference, Concept, Topic, Topic Graph, canonical and staged Tag mutation, Tag selection, and Tag import requests cross the boundary as strict canonical DTOs, while legal domain failures and diagnostics remain opaque JSON-safe results. Canonical Tag entry update/delete reads the complete aggregate and performs validation, reference maintenance, and persistence in one repository transaction; diagnostics and no-ops do not notify autosync, while a committed mutation notifies once. Staged Tag update performs read, collision merge, variant cleanup, and replacement in one repository transaction without protocol validation or canonical autosync. Tag import validates a non-blank string but preserves the original payload for service-owned parsing, preview state, and autosync. Production Workbench phased prewarm is plugin-side orchestration over the existing chrome and surface reads: one captured read state feeds chrome first and then ordered surface reads, with incremental host-cache and active-runtime publication. Sync is the final direct-service slice in the production Workbench. The Workflow Host exposes twelve use-case methods instead of the complete service. It snapshots live Zotero items and materializes Topic result paths into bounded `asset/NNNN` payloads before invoking the client; the in-process adapter reconstructs read-only asset access for the current service. Default and read-only clients share one legacy composition root and remain in-process, so this boundary does not change process, database, canonical file, mirror, or Zotero ownership.
