...
Workflow Topic option queries, startup runtime reconciliation, protected database reset, default-client invalidation, related-items notifier echo classification, and Workflow Host synthesis operations now enter through grouped `SynthesisClient` capabilities. The Workflow Host exposes twelve use-case methods instead of the complete service. It snapshots live Zotero items and materializes Topic result paths into bounded `asset/NNNN` payloads before invoking the client; the in-process adapter reconstructs read-only asset access for the current service. The default client remains in-process, so this boundary does not change process, database, canonical file, mirror, or Zotero ownership.
