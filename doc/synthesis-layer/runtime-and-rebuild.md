...
The approved Stage 1 sidecar direction is documented in `artifact/synthesis_sidecar_service_stage1_refactor_plan_20260715.md` and the active Synthesis migration OpenSpec changes. The repository now contains an independently compiled Node service foundation under `apps/synthesis-service`: it provides only loopback liveness, authenticated protocol/profile/schema handshake, separate lifecycle-token shutdown, bounded request parsing, and redacted lifecycle diagnostics. The plugin does not package, launch, discover, supervise, or call this service, and the service has no domain, worker, persistence, canonical-file, sync, or Host capability.

Production execution therefore remains in-process. The plugin is still the only owner of `synthesis.db` and Topic canonical files, and the default `SynthesisClient` still resolves the legacy in-process composition. The service foundation is a development/test control plane for later WS4 packaging, supervision, worker, and remote-client changes; it is not a production fallback.
