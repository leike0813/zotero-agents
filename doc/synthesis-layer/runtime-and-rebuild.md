...
The runtime currently keeps the authenticated Node v1 HTTP front door and uses a product-owned Rust child as the only Citation Graph Metrics v2 kernel. The three-crate native workspace owns strict Metrics DTO/canonical semantics, the Metrics algorithm, JSON-lines worker framing, and a dependency-free candidate HTTP mode. Layout, graph build, private kernels, repository, application, lifecycle, and the formal runtime manifest remain Node-owned until the R8 native-runtime cutover. The production DB and canonical owner remain plugin-side, and there is no Metrics fallback to the TypeScript oracle. Force layout moves through an explicit layout v2 contract.
