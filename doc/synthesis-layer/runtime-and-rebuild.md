...
The WS5 foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation and isolated Topic application rows, with `packages/synthesis-application` above it for bounded use cases and Topic canonical semantics. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates profile/data-root/schema identity markers, cancels interrupted shadow operations, and recovers the single canonical transaction journal before discovery. It then composes a private Topic application that resolves bounded materialized assets, lists indexed Topic state, reads one complete shadow current for detail, and performs optimistic complete or structured-patch promotion. Health and handshake expose only O(1) path-free identities. `workbench.chrome.read` projects fixed operational chrome; authenticated `topics.canonical.inspect` returns only `absent|ready|invalid`, hashes, and sorted section descriptors and never enters the worker pool. The HTTP router exposes no Topic application write method. Canonical JSON, SHA-256, path identity, section filename, text rules, bundle validation, and optimistic apply decisions are shared with plugin compatibility exports, while production Topic list/detail/apply remains disconnected. Production `SynthesisClient`, `state/synthesis.db`, canonical Topic files, and mutation authority remain plugin-owned. WS6 still owns parity verification and WS7 must perform a separate atomic single-writer cutover with no automatic fallback.
