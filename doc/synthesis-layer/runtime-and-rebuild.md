...
The approved target is a product-owned native Rust sidecar with one executable per platform and a replaceable Rust worker process for bounded CPU kernels. Rust migration R1 has made the normalized v1 process-boundary schemas, conformance corpus, canonical bytes/hashes, and fingerprint language-neutral under `packages/synthesis-contracts`; Node remains their executable TypeScript oracle. No Rust workspace or executable exists yet. The next change is `introduce-synthesis-rust-sidecar-metrics-vertical-slice`. The production DB and canonical owner remain plugin-side until isolated Rust engine, repository, canonical, application, lifecycle, packaging, and five-platform gates pass, followed by one atomic cutover with no Node fallback. Force layout moves through an explicit layout v2 contract. See `artifact/synthesis_sidecar_rust_migration_plan_20260718.md`.
...
The current milestone is **Stage 1 / WS5 — Private Isolated Synthesis Foundation Complete**. Its foundation uses `packages/synthesis-repository` as the environment-neutral SSOT for foundation plus Topic, Reference Refresh, Reference Matching/Review, Citation Graph, Tag Vocabulary, Concept KB, and Topic Graph shadow facts, with `packages/synthesis-application` above it for bounded use cases. The Node service opens a persistent shadow database and Topic canonical shadow only below its profile runtime root, validates identity/schema markers, reconciles interrupted operations, and recovers canonical journals before readiness. It composes seven private domain applications plus private knowledge-checkpoint, durable-bundle, WebDAV, and debug/maintenance coordinators. Debug capture reads a repository basis transactionally, inspects at most 1,000 canonical Topic descriptors outside SQLite, then recaptures the basis; concurrent change returns `superseded`. Read projections never repair, rebuild, create operations, or write state, and the Node profiler is unavailable by default. Shutdown drains debug/maintenance before WebDAV and the remaining owners. The HTTP router exposes none of these private surfaces. Production `SynthesisClient`, Host credentials/HTTP, `state/synthesis.db`, canonical Topic files, Host paper details, discovery effects, and mutation authority remain plugin-owned. This is not Stage 1 completion, Rust parity, production readiness, cutover, or real-machine acceptance: the replacement WS6 verifies Rust parity, and the replacement WS7 performs one Rust single-writer cutover with no automatic fallback.
