...
| Debug/maintenance shadow application | active private WS5 application | Shared strict projections provide bounded status, schema, cache, operation, snapshot, Topic/paper inspect, pure diff, and optional redacted profiling. Repository basis is captured before and after out-of-transaction canonical inspection; change returns `superseded`. Reads have zero mutation authority, Node profiling defaults to unavailable, and checkpoint/durable/protected reset reuse their existing owners. The application drains first and has no public route or automatic invocation. WS5 is complete; WS6 shadow parity is next. |
