...
| Citation Graph build engine | in-process production + measured worker canary | Legacy paper projection and production full, source-slice, and related-items fallback construction share the bounded environment-neutral build contract. Wire-bounded fixtures run through the authenticated internal canary; Core 200 and the explicit normal/target/stress runner record envelope, rebuild, worker, memory, event-loop, and cancellation evidence. The 2k/20k boundary already exceeds request-node and response bounds, normal times out in the worker, and target/stress cannot materialize under the isolated parent budget. Production composition, Repository/Host reads, hashing, and promotion remain application-owned pending a bounded large-transfer contract. |
...
| Process topology | three worker operations, two production routes | The plugin owns the production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. Citation Graph layout and metrics are production routes; graph build is an internal-only canary in the same supervised worker while all six non-routed production engines remain in process. There is no automatic fallback or shadow trigger. |
