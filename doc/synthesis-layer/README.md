...
| Node sidecar runtime package | packaged installation boundary | `apps/synthesis-service` compiles into five product-owned Node `24.18.0` runtime bundles. Strict manifests, real per-file hashes, versioned staging, atomic active/previous pointers, repair, and rollback produce verified absolute Node and entrypoint paths. The plugin does not launch or supervise them, no production consumer routes to them, and they cannot access production data or engines. |
