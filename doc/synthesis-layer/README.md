...
| Client boundary | migration slices active | Workflow Topic options, startup/maintenance lifecycle, default invalidation, related-items notifier echo classification, the twelve-method Workflow Host facade, and four region-scoped read-only harness queries use grouped `SynthesisClient` capabilities. Workflow Zotero items are snapshotted and Topic run assets are materialized before crossing the boundary; default and read-only adapters share one legacy composition root and still delegate to the in-process service. Production Workbench, Host Bridge, and MCP remain on the recorded migration allowlist. |
