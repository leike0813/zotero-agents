...
The current executable front door remains the WS5 mutation-disabled Node service and its isolated foundations. Node is a frozen migration oracle rather than the delivery target. The product-owned Rust workspace and worker executable implement fourteen private operations behind one shared bounded compute pool: Citation Graph Metrics, the five Tag Vocabulary/Concept KB/Topic Graph kernels, and the eight Reference Matcher/Topic Structured Artifact/Citation Graph build operations. Cross-language schemas, corpora, canonical bytes/hashes, source fingerprints, licenses, provenance, acknowledged paging, and candidate size budgets are governed together. The Node Worker retains only the R6 Citation Graph layout kernels. Production database/canonical ownership and runtime bundle/pointer v1 remain unchanged; R6 layout, R7 durable parity, R8 native lifecycle, and R9 production cutover remain separate changes. See `artifact/synthesis_sidecar_rust_migration_plan_20260718.md` and the active Synthesis migration OpenSpec change.
...
| Sidecar runtime migration | R5 complex kernels complete | `packages/synthesis-contracts` owns the checked v1 schema/corpus inventory and canonical semantics. One Rust executable serves all fourteen migrated operations; the five-platform candidate workflow passed provenance, smoke, 15 MiB per-target, and 75 MiB aggregate native gates. R6 layout, R7 durable parity, R8 native manifest/lifecycle, and R9 production cutover have not started. |
| Process topology | Node v1 front door with one shared Rust compute child | The plugin owns production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. The Node service owns private application orchestration and selects the Rust child for Metrics, Tag Vocabulary, Concept KB, Topic Graph, Reference Matcher, Topic Structured Artifact, and Citation Graph build/transfer compute. The Node Worker retains only Citation Graph layout until R6. Production ownership moves only after R7–R9. |
