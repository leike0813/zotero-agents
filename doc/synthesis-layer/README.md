...
| Process topology | seven worker operations, one transfer capability, two production routes | The plugin owns the production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. Citation Graph layout and metrics are production routes; graph build has wire-bounded and packed-transfer worker canaries. Tag validation/index, Concept index/query, and Topic Graph index are private isolated-shadow operations with no HTTP or production route. Transfer execution is explicit/internal, with no automatic fallback or shadow trigger. |
