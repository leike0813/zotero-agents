...
| Process topology | two production compute routes | The plugin owns the production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. Citation Graph layout and metrics share the supervised Node worker; the other six engines remain in-process and there is no automatic fallback. |
