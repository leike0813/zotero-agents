...
- Sync configuration, encrypted credentials, URL construction, default WebDAV HTTP, and prefs-configured Git adapter construction belong to production composition adapters. The application service receives a Git runtime binding plus secret-free `SynthesisHostWebDavSyncPort`; missing/readonly bindings are explicitly disabled.
