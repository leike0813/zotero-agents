...
9. [Durable Bundle Sync](./webdav-durable-sync.md) defines WebDAV as the sole durable-state exchange transport and SQLite as the local materialized store.
...
| Citation Graph build engine | process-ready in-process boundary | Legacy paper projection and production full, source-slice, and related-items fallback construction share the bounded environment-neutral build contract. Repository/Host reads and hashing remain application-owned; compute runs outside the write lock and promotion rechecks the durable sidecar-fact basis. |
...
| Citation Graph metrics engine | process-ready in-process boundary | Metrics v2 PageRank, weak components, normalization, and role scoring run through the same bounded environment-neutral package. Full, incremental, and manual refresh share lock-free compute plus graph-hash-guarded promotion; canonical metrics hashing remains application-owned. |
...
| Node sidecar runtime foundation | development/test control plane | `apps/synthesis-service` compiles to a plain-Node loopback server with minimal health, authenticated protocol/profile/schema handshake, distinct lifecycle authorization, bounded requests, redacted JSONL diagnostics, and graceful shutdown. It is not packaged or launched by the plugin, exposes no domain capabilities, and cannot access production data or engines. |
| Process topology | current in-process production state | The plugin still owns all production Synthesis application execution, `synthesis.db`, and Topic canonical writes through the in-process client. The isolated Node foundation does not change production routing or provide fallback. |
