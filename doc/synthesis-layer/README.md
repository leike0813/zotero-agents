...
The current executable front door remains the WS5 mutation-disabled Node service and its isolated foundations. Node is a frozen migration oracle rather than the delivery target. The product-owned Rust workspace and worker executable implement fifteen private compute operations behind one bounded pool, including the R6 Citation Graph layout v2 kernels. R7 has verified the independent Rust repository and canonical-store foundation; its earlier thirteen-family generic inventory was not application-parity evidence. Workbench and Topic now form the first real typed Node/Rust differential slice, using separate mutable roots and one strict application corpus. Production database/canonical ownership and runtime bundle/pointer v1 remain unchanged. R7 application parity is incomplete, so R8 native lifecycle remains blocked until the remaining application clusters gain the same typed differential evidence; R9 production cutover remains separate. See `artifact/synthesis_sidecar_rust_migration_plan_20260718.md` and the active Synthesis migration OpenSpec change.
...
| Debug/maintenance shadow application | Node oracle; typed Rust parity pending | Shared TypeScript projections provide bounded status, schema, cache, operation, snapshot, Topic/paper inspect, pure diff, and optional redacted profiling. The former generic Rust family inventory has been removed and does not count as application parity. Only Workbench chrome and canonical inspect are exposed as authenticated, bounded, mutation-disabled canaries. |
...
| Sidecar runtime migration | R7 durable foundation verified; application parity partial | The durable corpus proves repository schema/PRAGMA/transaction and canonical journal/recovery behavior. The independent `synthesis-typed-application-parity-v1` corpus proves only Workbench and Topic typed application behavior. The five-platform workflow runs both corpora, Rust tests, smoke, and size gates. Citation/Reference, Tag/Concept/Topic Graph, and Checkpoint/Bundle/WebDAV/Debug application clusters remain pending; R8 native manifest/lifecycle cannot start until those slices complete. |
