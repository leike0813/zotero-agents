...
The executable delivery target is the product-owned Rust native runtime selected by manifest v2; the mutation-disabled Node service is now a frozen differential oracle. The Rust workspace implements all typed application parity clusters and fifteen private compute operations behind one bounded persistent child. R8 locally covers native installation, identity, lifecycle, isolated repository/canonical composition, worker framing, disk-backed Citation Graph transfer, shutdown, smoke, and package budgets. Production database, canonical, Host, graph-basis, promotion, and `SynthesisClient` ownership remain plugin-side. Five-platform candidate evidence is still required for R8 acceptance, while R9 remains the separate production single-writer cutover. See `artifact/synthesis_sidecar_rust_migration_plan_20260718.md` and the active Synthesis migration OpenSpec changes.
...
| Sidecar runtime migration | R8 local native gates implemented; remote evidence pending | Durable, typed application, citation/reference, Tag/Concept/Topic Graph, checkpoint/bundle/WebDAV/debug, native contract, and worker-transfer corpora cover the Rust candidate. The five-platform workflow runs those checkers before native smoke and package budgets. It is read-only and does not publish or cut over production ownership. |
| Process topology | native R8 candidate + frozen Node oracle | The plugin owns production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. The Rust service owns only derived shadow roots, two read canaries, and the private compute/transfer surface. Its persistent child is lazy and shared by direct and paged work. Node is retained only for differential evidence; production ownership moves only in R9. |
