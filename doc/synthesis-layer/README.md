...
| Sidecar runtime migration | Rust pivot active; Node package frozen | `apps/synthesis-service` compiles into five product-owned Node `24.18.0` bundles totaling about 203 MB. They remain an isolated behavior/packaging oracle and are not synced into the formal XPI. The approved target is one native Rust executable per platform, native manifest v2, no Node/download fallback, and compressed 15/75/100 MiB runtime/XPI budgets. |
| Process topology | current Node oracle; target Rust serve/worker | Today the plugin owns production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion, while selected compute routes and private shadow operations can execute in Node. The target keeps the plugin seam but replaces the external process with Rust `serve` plus a replaceable Rust worker process; production ownership moves only after isolated parity and five-platform gates. |
