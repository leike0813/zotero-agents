...
| Client boundary | migration slices active | Workflow Topic options, startup/maintenance lifecycle, default invalidation, related-items notifier echo classification, the twelve-method Workflow Host facade, production plus read-only Workbench chrome, surface, Topic detail, and paper digest reads, production Topic Report export reads, production Workbench progress polling, Citation Graph layout/cache commands, all Workbench Reference commands, Workbench Concept commands, Topic artifact delete/purge plus discovery-hint reject/restore, Workbench Topic Graph commands, Tag Vocabulary validation/rebuild/regulator export, atomic canonical entry update/delete, atomic staged Tag update, staged Tag bulk promote/discard/clear, and Tag import preview/apply use grouped `SynthesisClient` capabilities. Citation Graph commands use `client.graph`; Reference commands use `client.references`; Concept rebuild, display-text update, review, and deletion use `client.concepts`; Topic artifact/discovery commands use `client.topics`; Topic Graph rebuild and decisions use `client.topicGraph`; the migrated Tag maintenance/export, canonical entry mutation, staged update/bulk, and import slices use `client.tags`. Maintenance, Concept, Topic Graph, and Tag rebuild contracts carry no UI progress callbacks, while mutation contracts use strict canonical DTOs and return opaque JSON-safe results. Canonical Tag entry update/delete reads, validates, maintains aliases and replacements, and persists in one repository transaction; only committed mutations notify autosync. Staged Tag update performs rename/collision merge in one repository transaction without Tag protocol validation or canonical autosync, Tag import accepts a non-blank string payload without rewriting its whitespace, and progress remains the side-effect-free no-argument `workbench.readProgress()` poll. Workbench phased prewarm is plugin-side orchestration over the existing chrome and surface reads; it adds no callback or full-snapshot client capability. Workflow Zotero items are snapshotted and Topic run assets are materialized before crossing the boundary; default and read-only adapters share one legacy composition root and still delegate to the in-process service. Sync is the final direct-service slice in the production Workbench; legacy composition, Workbench, Host Bridge, and MCP remain on the recorded migration allowlist. |
