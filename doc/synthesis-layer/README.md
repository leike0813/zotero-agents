...
| Process topology | one production compute route | The plugin owns the production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. Only Citation Graph layout compute crosses the supervised Node worker; the other seven engines remain in-process and there is no automatic fallback. |
