...
| Client boundary | migration slices active | Workflow Topic options, startup/maintenance lifecycle, default invalidation, related-items notifier echo classification, the twelve-method Workflow Host facade, and production plus read-only Workbench chrome, surface, Topic detail, and paper digest reads use grouped `SynthesisClient` capabilities. Workbench phased prewarm is plugin-side orchestration over the same existing chrome and surface reads; it adds no callback or full-snapshot client capability. Workflow Zotero items are snapshotted and Topic run assets are materialized before crossing the boundary; default and read-only adapters share one legacy composition root and still delegate to the in-process service. Production Workbench progress polling, report reads, commands, and mutations still resolve the legacy service directly, so legacy composition, Workbench, Host Bridge, and MCP remain on the recorded migration allowlist. |
