...
| Client boundary | migration slices active | Workflow Topic options, startup/maintenance lifecycle, default invalidation, related-items notifier echo classification, and the twelve-method Workflow Host facade use grouped `SynthesisClient` capabilities. Workflow Zotero items are snapshotted and Topic run assets are materialized before crossing the boundary; the default adapter still delegates to the in-process service. Other consumers remain on the recorded migration allowlist. |
