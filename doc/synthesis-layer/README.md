...
| Process topology | current in-process production state | The plugin still owns all production Synthesis application execution, `synthesis.db`, and Topic canonical writes through the in-process client. The supervised Node control plane does not change production routing or provide fallback. |
