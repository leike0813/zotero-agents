...
11. [Citation Graph Build Large Transfer](./citation-graph-large-transfer.md) defines authenticated bounded input/output staging while packed worker execution and production routing remain deferred.
...
| Citation Graph build engine | in-process production + worker and transfer canaries | Legacy paper projection and production full, source-slice, and related-items fallback construction share the bounded environment-neutral build contract. Core 200 records why monolithic worker routing is unsafe; Core 201 adds authenticated canonical-JSON row staging with 4 MiB pages, symmetric manifests, idempotent retry, bounded sessions, TTL, and output reads. Staging does not execute the worker. Production composition, Repository/Host reads, hashing, and promotion remain application-owned pending packed worker integration and a separate production cutover. |
...
| Process topology | three worker operations, one transfer capability, two production routes | The plugin owns the production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. Citation Graph layout and metrics are production routes; graph build has a wire-bounded worker canary plus a service-owned staging canary, while all six non-routed production engines remain in process. Staging does not spawn compute, and there is no automatic fallback or shadow trigger. |
