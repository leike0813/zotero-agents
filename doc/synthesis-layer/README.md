...
| Sidecar runtime migration | R5 complex kernels in acceptance | `packages/synthesis-contracts` owns the checked v1 schema/corpus inventory and canonical semantics. One Rust executable serves all fourteen migrated operations; candidate workflows enforce provenance plus 15 MiB per-target and 75 MiB aggregate native budgets. Native manifest v2 and the final 100 MiB XPI budget remain later gates. |
