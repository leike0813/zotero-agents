...
The current executable front door remains the WS5 mutation-disabled Node service and its isolated foundations. Node is a frozen migration oracle rather than the delivery target. The product-owned Rust workspace and worker executable implement fifteen private compute operations behind one bounded pool, including the R6 Citation Graph layout v2 kernels. R7 adds independent Rust repository, canonical-store, and application owners plus two authenticated mutation-disabled read canaries. The language implementations consume one durable corpus while keeping separate temporary profile roots, databases, canonical roots, and writer leases. Production database/canonical ownership and runtime bundle/pointer v1 remain unchanged; R7 is ready for verification, while R8 native lifecycle and R9 production cutover remain separate changes. See `artifact/synthesis_sidecar_rust_migration_plan_20260718.md` and the active Synthesis migration OpenSpec change.
...
| Sidecar isolated repository foundation | Node oracle + R7 Rust shadow candidate | The Node service retains its frozen WS5 per-profile SQLite oracle. The Rust candidate owns a separately derived shadow repository with the complete 51-table/40-index schema, locked SQLite configuration, strict row conversion, nested savepoints, startup reconciliation, backup, locking, and deterministic close. Its canonical and application owners use separate derived roots and one writer admission. Neither implementation can see production paths or share a live owner; R8 lifecycle and R9 cutover remain pending. |
...
| Debug/maintenance shadow application | Node oracle + private R7 Rust application | Shared strict projections provide bounded status, schema, cache, operation, snapshot, Topic/paper inspect, pure diff, and optional redacted profiling. The R7 Rust application reaches repository, canonical, compute, and remote effects only through explicit ports; WebDAV receives no credentials or generic HTTP client. Its mutation use cases remain library-internal differential surfaces. Only Workbench chrome and canonical inspect are exposed as authenticated, bounded, mutation-disabled canaries. Production cutover and real-machine acceptance remain pending. |
...
| Sidecar runtime migration | R7 shadow parity ready for verify | `packages/synthesis-contracts` owns the checked v1 contracts and the shared R7 durable corpus. One Rust executable serves fifteen compute operations plus the two bounded read canaries. Local Rust tests, Node/Rust corpus checks, license inventory, candidate smoke, and the 15 MiB single-target gate pass; the five-platform workflow carries repository locking, journal recovery, application parity, smoke, and 15/75 MiB gates but still needs a controlled remote run. R8 native manifest/lifecycle and R9 production cutover have not started. |
| Process topology | Node v1 production front door + isolated Rust candidates | The plugin owns production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion. The Node service remains the private application oracle and delegates migrated compute to the Rust child. Separately launched R7 candidate instances own only derived Rust shadow roots and expose two read canaries; production composition does not reference them. Production ownership moves only in R9 after R8 lifecycle work. |
