...
| Durable bundle export shadow application | active private WS5 application | Shared contracts define all 23 durable entity kinds, strict bounded v2 bundle/manifest encoding, deterministic production-compatible canonical bytes and hashes, and legacy v1 read/verify. The private application recaptures the SQLite aggregate and canonical Topic identities around current-file reads, rejects mixed or damaged snapshots, and writes sorted bundles before the manifest. It has no import/apply, sync index, WebDAV transport, worker, HTTP/RPC, `SynthesisClient`, Workbench, Host Bridge, or MCP capability. |
