...
The current executable front door remains the WS5 mutation-disabled Node service and its isolated foundations. Node is a frozen migration oracle rather than the delivery target. The product-owned Rust workspace and worker executable now implement Citation Graph Metrics plus the five private deterministic operations for Tag Vocabulary, Concept KB, and Topic Graph behind one shared bounded compute pool. Cross-language schemas, corpora, canonical bytes/hashes, source fingerprints, licenses, provenance, acknowledged paging, and candidate size budgets are governed together. Production database/canonical ownership and runtime bundle/pointer v1 remain unchanged; the active R4 change must still pass its local and separately authorized five-platform gates before archival. See `artifact/synthesis_sidecar_rust_migration_plan_20260718.md` and the active Synthesis migration OpenSpec change.
