...
| Citation Graph build engine | in-process production + isolated worker canary | Legacy paper projection and production full, source-slice, and related-items fallback construction share the bounded environment-neutral build contract. Wire-bounded fixtures can run through the authenticated internal worker canary, but production composition, Repository/Host reads, hashing, and promotion remain application-owned. The 8 MiB / 250,000-request-node / 50,000-response-node wire is not a production-scale transfer contract. |
