...
The executable delivery target and local production owner are the
product-owned Rust native runtime selected by manifest v3. Node is retained
only as a frozen differential oracle and no normal route may select it.
R9a binds the complete fingerprinted production-client inventory, native
composition, reverse-Host boundary, durable cutover receipt/coordinator,
verified DB/WAL/canonical backup and restore, production-copy preflight, live
owner lock, critical-smoke evidence, and mutation admission into one atomic
handoff. The default `SynthesisClient` switches only after Rust has verified
the receipt-bound evidence and persisted activation; a failed pre-admission
handoff restores the verified backup, while a post-admission failure enters
Rust-only repair. The retained Node and generated-artifact inventory is
recorded in `artifact/synthesis_r9a_retirement_baseline_20260727.md` and is
not a release authorization. Seven-platform R8/R9b acceptance remains external
evidence; signing, XPI publication, synchronization, and release stay out of
this local change.
...
| Sidecar runtime supervisor | R9a local production owner active; Node oracle frozen | Plugin startup performs the receipt-bound Rust handoff, then supervises the manifest-v2 Rust executable with profile-scoped production owner/discovery/lease state, event-driven exit/orphan detection, bounded health fallback, restart, and crash-loop fuse. Node remains differential-only and cannot regain a live root. |
...
| Workbench UI | hard-cut target | UI presents cache status, explicit operations, and Review & Overrides management. Debug builds also project one read-only, attempt-correlated native startup failure across every Workbench surface, Task Manager, runtime diagnostic export, and the direct-launch terminal. |
...
| Sidecar runtime migration | R9a local cutover complete; R9b acceptance pending | The 95 flat client operations and thirteen reverse-Host operations are closed and checked. Backup/restore, native empty-profile initialization, receipt recovery, production-copy preflight, explicit live paths, complete Rust dispatch, critical smoke, mutation admission, supervisor wiring, and default-route cutover are locally complete. Seven-platform and clean-machine evidence is a separately governed R9b acceptance task; no release is authorized here. |
| Process topology | Rust is the production owner after verified activation | Normal startup performs the strict Rust preflight/live handoff and gives the activated Rust service exclusive production ownership. The plugin provides only bounded reverse-Host adapters. There is no request-level fallback, shared live root, or path that re-enables Node ownership. |
