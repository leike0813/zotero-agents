...
The current executable implementation remains the WS5 mutation-disabled Node service and its isolated foundations. On 2026-07-18 the approved delivery target changed to a native Rust sidecar because the five Node prebuilds total about 203 MB and neither a universal XPI of that size nor post-install runtime download is acceptable. Node is now a frozen migration oracle: it is not the target for new capabilities, production ownership cutover, formal XPI inclusion, or real-machine release acceptance. Rust migration R1 is complete: the normalized v1 process-boundary schemas, positive/negative corpus, canonical UTF-8 bytes/hashes, capability inventory, and computed fingerprint are contracts-owned and governance-checked. No Rust workspace or executable exists yet, and production database/canonical ownership, Node routing, and runtime bundle/pointer v1 remain unchanged. The next change is `introduce-synthesis-rust-sidecar-metrics-vertical-slice`. See `artifact/synthesis_sidecar_rust_migration_plan_20260718.md` and the active Synthesis migration OpenSpec changes.
...
| Sidecar runtime supervisor | active Node oracle; Rust target pending | Plugin startup can launch the verified product-owned Node runtime with profile-scoped owner/discovery/lease state, event-driven exit/orphan detection, low-frequency health fallback, bounded restart, and crash-loop fuse. This is a frozen, mutation-disabled migration oracle, not the formal XPI target or production data owner. Native manifest v2 will preserve the supervision invariants while replacing Node-specific launch identity. |
| Sidecar isolated repository foundation | active WS5 Node oracle | The service main process owns a persistent per-profile SQLite shadow containing foundation plus seven private Topic, Reference Refresh, Reference Matching/Review, Citation Graph, Tag Vocabulary, Concept KB, and Topic Graph applications. A private knowledge-checkpoint coordinator spans the active Tag/Concept/Topic Graph rows; the private durable exporter captures every available durable row plus Topic registry bases in one transaction. The repository initializes before discovery, reconciles interrupted operations, and closes after private application work drains. It cannot see production paths and does not change production DB/canonical ownership. The next work is Rust contract/engine/durable parity in isolated roots, followed by one Rust single-writer cutover. |
...
| Debug/maintenance shadow application | active private WS5 application | Shared strict projections provide bounded status, schema, cache, operation, snapshot, Topic/paper inspect, pure diff, and optional redacted profiling. Repository basis is captured before and after out-of-transaction canonical inspection; change returns `superseded`. Reads have zero mutation authority, Node profiling defaults to unavailable, and checkpoint/durable/protected reset reuse their existing owners. The application drains first and has no public route or automatic invocation. The current milestone is **Stage 1 / WS5 — Private Isolated Synthesis Foundation Complete**; Stage 1 completion, Rust parity, production cutover, and real-machine acceptance remain pending. |
...
| Sidecar runtime migration | R1 contract/corpus complete; Node package frozen | `packages/synthesis-contracts` owns the checked v1 schema/corpus inventory and canonical semantics. `apps/synthesis-service` still compiles into five product-owned Node `24.18.0` oracle bundles totaling about 203 MB; no Rust executable exists and they remain outside the formal XPI. The next change introduces the Rust workspace/worker framing through the Metrics vertical slice. Native manifest v2 and compressed 15/75/100 MiB runtime/XPI budgets remain later gates. |
| Process topology | current Node oracle; Rust executable absent | Today the plugin owns production `SynthesisClient`, `synthesis.db`, Topic canonical writes, graph basis, and promotion, while selected compute routes and private shadow operations can execute in Node. R1 changed only contracts and governance. The target still replaces the external process with Rust `serve` plus a replaceable Rust worker process; production ownership moves only after isolated parity and five-platform gates. |
