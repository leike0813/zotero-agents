...
| Client boundary | migration slices active | Workflow Topic options, startup/maintenance lifecycle, default invalidation, related-items notifier echo classification, the twelve-method Workflow Host facade, production plus read-only Workbench reads, Topic Report export, progress polling, Citation Graph, Reference, Concept, Topic, Topic Graph, Tag, and Git/WebDAV Sync commands use grouped `SynthesisClient` capabilities. Sync commands use the shared `client.sync.git` and `client.sync.webDav` transport contract; each command acquires a fresh default client after invalidating the legacy service, while state/configuration/credential/connection-test reads stay on their existing projections and raw Host Bridge/MCP APIs. Strict request DTOs and opaque JSON-safe results keep UI callbacks and domain internals outside the client boundary. Workbench phased prewarm remains plugin-side orchestration over chrome and surface reads. Default and read-only adapters share one legacy composition root and still delegate to the in-process service. Production Workbench has no full-service import; legacy composition, Host Bridge, and MCP are the three recorded direct consumers. |
