...
Tag Vocabulary validation、projection rebuild、regulator export、atomic canonical entry update/delete、atomic staged update、staged bulk promote/discard/clear 与 import preview/apply 在各自命令闭包内解析默认 `SynthesisClient`，并调用 `client.tags`。Maintenance 三路均使用空 single-flight args；validation 与 export 立即启动并保持 Home-only 默认失效，rebuild 保留 protected confirmation、deferred start 与 Tags 失效。Rebuild 不传递 progress callback，进度只来自 `workbench.readProgress()`；export 返回 string array 后仍由 host 写入 clipboard，格式为每个 tag 一行并保留尾随换行。Canonical entry update/delete 保留 trim、facet prefix fallback、note 字符串归一化、`{ originalTag }` single-flight、空值跳过、立即启动和 Tags-only 失效；delete confirmation 仍由 UI 拥有。两路通过 strict DTO 调用 client，并用 singular `failOnDiagnostic` 接入既有失败反馈。领域层在同一 repository transaction 内读取完整 canonical Tag aggregate，完成 conflict/not-found/no-op 判定、隐藏元数据与未触及时间戳保留、alias/replacement 级联、protocol validation 与持久化；只有实际 commit 才通知一次 autosync。Staged update 保留 `originalTag`/facet/source-flow 回退、`source_flow`/`parent_bindings` host aliases、`{ tag }` single-flight、空 tag 跳过、立即启动与 Tags-only 失效；严格 DTO 在 adapter 内 trim 字符串并 canonicalize parent bindings，领域层在一个 repository transaction 中完成 rename/collision merge，不触发 protocol validation 或 canonical autosync。Promote/discard 保留 `tags` array 与 singular `tag` 输入、String/trim/空值过滤、首 tag single-flight key 和空选择跳过；client 使用允许空数组的严格 selection DTO，供 Workflow Host 保持 no-op 兼容。Clear 保留空参数全局 single-flight key。三路 staged bulk 均立即启动、不使用 singular diagnostic failure，并只失效 Tags。`importTagVocabulary` 与 `previewTagVocabularyImport` 共享 preview operation 和空 single-flight args；apply 只以 trim 后的受支持 action 作为 single-flight args。Import DTO 要求非空 string payload，但保留原始字符串交由 service 解析并管理 preview state；preview/apply 均立即启动、不增加 confirmation、defer、progress 或 diagnostic 转换，并只失效 Tags。Bootstrap 保持当前路径；audit 继续由 Workflow Host 经 `client.tags` 执行。Sync 是 Workbench 最后保留的 direct-service slice。
