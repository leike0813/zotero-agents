...
常规读取场景应优先使用 `item`、`note` 和 `synthesis` 语义命令。Synthesis
场景应优先使用 `synthesis <subcommand>`，而不是 `call synthesis.*`。
...
scope 时，审批进入 ACP Chat panel。没有 ACP scope 时，Host Bridge 使用全局
Zotero approval UI。用户拒绝、等待超时或目标 ACP UI 不可用时，CLI 返回
`ok: false`，category 为 `permission`。带有 ACP scope 的请求不会 fallback
...
| 输入 | 行为 |
| --- | --- |
| 参数省略 | `synthesis <subcommand> --input` 与 `call --input` 可省略，解析为 `{}` |
...
所有 Synthesis 子命令都通过 `POST <endpoint>/call` 调用对应
`synthesis.*` capability，但 agent-facing CLI 不需要写 raw capability 名。
`--input` 复用通用 `JSON_OR_FILE` 规则；省略时 input 为 `{}`。

子命令映射：

| CLI subcommand | Capability |
| --- | --- |
| `topics list` | `topics.list` |
| `topics get-context` | `topics.get_context` |
| `schemas get` | `schemas.get` |
| `library-index get` | `library_index.get` cache view |
| `resolvers resolve` | `resolvers.resolve` |
| `reference-index get` | `reference_index.get` cache view |
| `citation-graph overview` | `citation_graph.get_overview` cache view |
| `citation-graph get-slice` | `citation_graph.get_slice` cache view |
| `citation-graph get-metrics` | `citation_graph.get_metrics` cache view |
| `paper-artifacts manifest` | `paper_artifacts.get_manifest` |
| `paper-artifacts read` | `paper_artifacts.read` |
| `paper-artifacts export-filtered` | `paper_artifacts.export_filtered` |
| `paper-artifacts resolve-topic-digest` | `paper_artifacts.resolve_topic_digest` |
