...
result/input manifest 文件由 provider 另行放在 `result/<skillId>.n/` 与
`.acp/<skillId>.n/` 子空间；这不改变 Host Bridge CLI 的 profile、token
或命令解析行为。
...
`workflow describe` 返回指定 workflow 的 selection 要求、workflow option schema、
可用 backend profiles、provider option schema，以及基于本次
`--workflow-options` / `--provider-profile` draft 规范化后的预览。describe 是只读
endpoint，不会提交 workflow。

`workflow submit` 需要显式 selection，不会使用 Zotero 当前 UI selection
作为 fallback。`--items` 接收 Zotero item ref 数组，并映射为
...
或 no-selection workflow 使用 `--none`，映射为：
...
`--workflow-options` 只传 workflow manifest `parameters` 对应的值。
`--provider-profile` 是本次执行覆盖文件，只允许包含 `schema`、
...
- workflow submit 必须使用显式 selection，不能从当前 Zotero UI selection 推断。
- provider profile 文件只作为本次执行覆盖；不得保存 bearer token、backend auth、
  baseUrl 或本地路径。
...
发布的 CLI bundle 提供不依赖 Node 的安装入口：
...
安装脚本自动解析当前平台，不支持 `--platform` 覆盖。重复运行即升级；只有
当目标 binary 的 SHA-256 与 bundle 不同时才替换。Agent 应优先使用
`--yes --json`，并通过 `--endpoint`、`--profile`、`--token-env` 或
`ZOTERO_BRIDGE_*` 环境变量传入运行配置，不打印真实 token。
...
```text
POST <endpoint>/workflows/describe
Authorization: Bearer <token>
Content-Type: application/json
...
schema、可用 backend profiles、provider option schema 和规范化 draft。
...
`--workflow-options` 只表示 workflow manifest `parameters` 的值：
...
| `input_json_invalid` | `validation` | 7 |
| `invalid_workflow_describe_request` | `validation` | 7 |
| `invalid_workflow_submit_request` | `validation` | 7 |
...
| `invalid_capability_input`, `invalid_workflow_input`, `invalid_workflow_describe_request`, `invalid_workflow_submit_request`, `invalid_file_id`, `bad_request` | `validation` |
