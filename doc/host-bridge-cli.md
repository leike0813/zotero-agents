...
`scope` 用于把需要审批的 Host Bridge CLI 请求关联回当前 ACP skill run。
当 `scope.kind` 是 `acp-skill-run` 或 `acp-run`，且包含 `requestId` 或
`runId` 时，审批请求会进入 ACP Skills UI。
...
zotero-bridge synthesis <subcommand> [--input <JSON_OR_FILE>]
...
当请求来自 ACP run scope 时，审批进入 ACP Skills UI。没有 run scope 时，
Host Bridge 使用全局 Zotero approval UI。用户拒绝、等待超时或 UI 不可用时，
CLI 返回 `ok: false`，category 为 `permission`。
...
| `resolve-topic-paper-digest` | `synthesis.resolve_topic_paper_digest` |
| `get-review-input` | `synthesis.get_review_input` |
