...
def build_parser() -> argparse.ArgumentParser:
    ...
    refresh_parser = sub.add_parser("refresh", help="Atomically replace the local paged library snapshot.")
    refresh_parser.add_argument("--limit", type=int, default=200, help="Host Bridge page size.")
    refresh_parser.add_argument("--library-id", help="Restrict snapshot to one library id.")
    refresh_parser.add_argument("--collection-id", help="Restrict snapshot to one collection id.")
    refresh_parser.add_argument("--collection-key", help="Restrict snapshot to one collection key.")
    refresh_parser.add_argument("--tag", help="Restrict snapshot to one exact tag.")
    refresh_parser.add_argument("--item-type", help="Restrict snapshot to one Zotero item type.")
    refresh_parser.add_argument("--query", help="Optional bounded library text query.")
    refresh_parser.set_defaults(func=refresh)

    search_parser = sub.add_parser("search", help="Search the current local snapshot without contacting Zotero.")
    search_parser.add_argument("query", help="Local full-text search expression.")
    search_parser.add_argument("--limit", type=int, default=25, help="Maximum matches to return.")
    search_parser.set_defaults(func=search)

    item_parser = sub.add_parser("item", help="Read one cached item by key, id, or stable ref.")
    item_parser.add_argument("ref", help="Cached item key, numeric id, or stable item ref.")
    item_parser.set_defaults(func=item)

    stats_parser = sub.add_parser("stats", help="Read local index counts and freshness metadata.")
    stats_parser.set_defaults(func=stats)

    workflow_refresh_parser = sub.add_parser("workflow-refresh", help="Atomically refresh the local Host Bridge workflow catalog.")
    workflow_refresh_parser.set_defaults(func=workflow_refresh)

    workflow_show_parser = sub.add_parser("workflow-show", help="Read one cached workflow descriptor.")
    workflow_show_parser.add_argument("workflow_id", help="Workflow id from the cached catalog.")
    workflow_show_parser.set_defaults(func=workflow_show)

    run_register_parser = sub.add_parser("run-register", help="Register one Host-owned workflow run for bounded monitoring.")
    run_register_parser.add_argument("--run-id", required=True, help="Host-owned workflowRunId.")
    run_register_parser.add_argument("--workflow-id", required=True, help="Workflow id associated with the run.")
    run_register_parser.add_argument("--state", help="Initial run state.")
    run_register_parser.add_argument("--payload", help="Optional initial status JSON, file path, or @file.")
    run_register_parser.set_defaults(func=run_register)

    run_watch_parser = sub.add_parser("run-watch", help="Perform one non-blocking status pass over registered active runs.")
