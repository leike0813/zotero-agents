...
If workflow selection or options validation fails, preserve the reviewed refs and report the failure; obtain an explicit corrected scope through Generic before validating another invocation. A selection basis failure ends that acquisition, and an incomplete stored ref remains non-executable; neither failure authorizes replacing the input from the active pane. If the workflow contract changes, stop and return to Generic with the changed requirements. If a native unit becomes uncertain, preserve its `submissionId`, `queueId`, ordinal, source refs, and exposed task identity, then reconcile the submission and live run state before any new work.
