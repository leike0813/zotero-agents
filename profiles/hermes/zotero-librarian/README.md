...
On a failed Host Bridge operation, inspect `retryable`, `stateChange`, `handleConsumption`, `safeNextActions`, and optional `nextCommand`. Query `operation get <operationId>` when a response is uncertain, re-read live Host state when `stateChange` is changed or unknown, query workflow or apply receipts before resuming, and do not reuse consumed or unknown handles. Local index or monitor failures may be repaired from their source services, but local cached state is never the authority for current Host facts.
