...
- [Zotero 10](https://www.zotero.org/download/)、[Zotero 9](https://www.zotero.org/download/)、または [Zotero 7](https://www.zotero.org/download/)（バージョン ≥ 7.0）
...
> **Zotero バージョンについて**：本プラグインは Zotero 7、9、10 をサポートします。Windows と Linux では代表バージョン 7.0.32、9.0.6、10.0.1 をテストしています。macOS 上の Zotero 10 は、現時点では非ブロッキングの正式 XPI スモークテストが証拠であり、過去のすべてのパッチ版を網羅するものではありません。
...
| **Kilo Code Auto Free** | 無料 | 組み込みの Auto Free モードが各リクエストを適切な無料モデルに自動ルーティング。APIキーやアカウント不要 |
| **OpenCode Zen / OpenRouter 無料** | 無料 | OpenCode Zen は組み込みの無料モデルを提供；OpenRouter も無料ティアのモデル（Gemini 2.5 Flash、DeepSeek V3 など）を提供。レート制限ありだがコストゼロ |

### 無料版の制限事項

無料モデルは始めるのに最適ですが、以下のトレードオフがあります：

| 制限 | 内容 |
|------|------|
| **レート制限** | プロバイダーの負荷に応じて毎分5〜20リクエストに制限されることがあります。複数論文のバッチ処理は大幅に遅くなる可能性があります |
| **並列処理** | 通常は単一の同時リクエストのみ。複数のワークフローを同時に送信するとキューに入るか失敗します |
| **モデルの可用性** | ピーク時には無料モデルのプールが枯渇することがあります。「モデルが利用不可」または「容量超過」エラーが発生する可能性があります |
| **モデルの変更** | プロバイダーが無料モデルを通知なしに静かに切り替える（アップグレードまたはダウングレード）ことがあります。出力品質が実行ごとに変動する可能性があります |
| **SLAなし** | 無料ティアには稼働保証がありません。サービスが一時的に利用できなくなったり、提供が終了したりする可能性があります |

> 信頼性の高いバッチ処理や本番利用が必要な場合は、有料プラン（OpenCode Go または Coding Plan）を検討ください。論文1本あたりのコストは、節約できる時間に比べてごくわずかです。
...
| **Kilo Code** | 組み込みの Auto Free モード — 利用可能な無料モデルに自動ルーティング、設定不要。XDG 環境変数による設定分離に対応。有料 API キーでも動作 | **無料** (Auto Free) | ⭐⭐⭐ 優れた無料オプション |
| **Opencode** | Qwen3.5-Plus / Kimi-K2.5 / GLM-5 などのモデルが文献タスクで優れた性能を発揮。[OpenCode Go](https://opencode.ai/go?ref=SZDFT9GZKW) は低価格のクォータを提供；Zen エディションは組み込みの無料モデルを含む；OpenRouter の無料モデルも使用可能 | 無料 (Zen / OpenRouter) または低コスト (Go) | ⭐⭐⭐ 強く推奨 |
