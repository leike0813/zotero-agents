...
Zotero Agents は Zotero 7 向けの**フレームワーク型プラグイン**です。AI や自動化ワークフローのための汎用実行シェルを提供します：
...
1. [Releases](https://github.com/leike0813/zotero-agents/releases) ページから最新の `.xpi` ファイルをダウンロードします。
...
1. `編集` → `設定` → `Zotero Agents` → `SkillRunner Local Runtime` を開く
...
_ワンクリックデプロイを使用しない場合_：`編集` → `設定` → `Zotero Agents` → `Backend Manager` で Skill-Runner エンドポイントを追加。
...
アイテムを右クリック → `Zotero Agents` → ワークフローを選択。
