...
本步骤在 `persist_core_sections` 之后执行。目标是把本次 update 已经形成的语义上下文转成插件可摄取的 KG proposal sidecars，并同步生成 topic discovery 使用的 interest metadata。你只写 proposal 和 discovery metadata，不写 canonical KG assets，也不把这些内容写进最终正文 section。
