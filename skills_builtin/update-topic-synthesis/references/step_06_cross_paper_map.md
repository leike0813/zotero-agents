...
- `gap_candidates`：分清真实研究空白、库内覆盖不足、artifact 缺失和评价缺口；本阶段 `gap_type` 只能是 `library_coverage_gap` / `evidence_gap` / `method_gap` / `evaluation_gap` / `review_gap`。
  这是 cross-paper map 的候选缺口枚举；后续 final/core `gaps[].gap_type` 另有 runtime 接受范围。
  ...
  "review_outline_seeds": [
    {
      ...
      "candidate_refs": [
        "route:convergence-stabilization",
        "claim:practicality-depends-on-stabilization"
      ]
    }
  ],
  "diagnostics": []
  ...
  "taxonomy_candidates": [
    {
      "id": "route:2020",
      "label": "2020 methods",
      "paper_unit_refs": ["pu:1:DETR2020"]
    }
  ],
  "claim_candidates": [{ "id": "claim:best", "claim": "这些方法都很好" }]
}
```
