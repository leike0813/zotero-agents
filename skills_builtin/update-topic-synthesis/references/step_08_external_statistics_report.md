# Step 08 External Literature, Statistics, And Report
...
`persist_external_statistics_report` 会在 Stage 9 立即检查 report 标题、正文深度、段落形态和上述维度的来源完整性；不要等到最终 bundle 阶段才发现浅层报告。

## Stage 9 Payload

Stage 9 不直接手写最终 section 文件。请写：
...
Stage 9 payload 中，runtime 会从已登记工件中保真合并。Patch update 只需提交实际要替换的 Stage 9 section，但仍必须保持该 section 的完整语义深度。
