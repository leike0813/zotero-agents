...
- `taxonomy`：从多个分类轴解释 topic 语义边界内的研究路线、机制、边界、代表论文、成熟度、优势和局限；`axes` 选择 2-4 个最适合当前 topic 的视角，每个 `axis_type` 必须是 `problem_formulation`、`technical_mechanism`、`evidence_scope`、`research_route` 或 `application_context`；`axis_rationale` 只写一句短说明。如果 workset 只覆盖子域，taxonomy 仍要按 topic 本身的宏观结构组织。
...
- 不要把多个 taxonomy axes 写成同一套分类标题；每个 axis 应体现不同分类角度。
    ...
    "axes": [
      {
        ...
        "axis_rationale": "按 Computer Vision 的宏观研究路线组织，避免让当前检测/分割 workset 反向收缩 topic identity。",
        "nodes": [
          {
          ...
          },
          {
            ...
            "strengths": [
              "能解释当前库内 DETR-family 改进论文的主要证据贡献。"
            ],
            "limitations": [
      ...
      },
      {
        ...
        "axis_rationale": "按当前 evidence 中最可证据化的机制差异组织，用于解释检测路线中的方法分化。",
        "nodes": [
          {
            "id": "mechanism:query-based-set-prediction",
            "title": "Query-based set prediction",
            "definition": "用 object queries 和 bipartite matching 直接生成目标集合，减少候选框和后处理依赖。",
            "core_problem": "把目标检测从手工 proposal/NMS 流程转成端到端集合预测问题。",
            "mechanism": "Transformer decoder queries 与 ground-truth objects 做集合匹配，并通过全局注意力聚合图像上下文。",
            "strengths": ["能提供清晰的端到端检测公式。"],
            "limitations": [
              "训练收敛和 query matching 稳定性仍是当前 evidence 中的重要瓶颈。"
            ],
            "maturity": "active method family",
            "representative_papers": ["1:DETR"],
            "source_paper_refs": ["1:DETR"]
          ...
          {
            "id": "mechanism:denoising-query-training",
            "title": "Denoising query training",
            "definition": "通过 denoising targets 和 query design 改善端到端检测训练稳定性。",
            "core_problem": "缓解 query-based detector 训练慢、匹配不稳定和早期收敛不足的问题。",
            "mechanism": "在训练中加入带噪声的目标查询和辅助目标，使 decoder 更快学习匹配与定位。",
            "strengths": ["能解释 DETR-family 后续改进的主要 evidence 贡献。"],
            "limitations": [
              "它属于检测子域中的训练机制改进，不能代表全部 Computer Vision 路线。"
            ],
            "maturity": "active optimization mechanism",
            "representative_papers": ["1:DINO"],
            "source_paper_refs": ["1:DINO"]
          }
        ]
...
本技能正常输出一个用于 `core_enrichment` 的 `topic_synthesis_handoff` JSON 对象；硬门禁或取消路径输出一个 `topic_synthesis_canceled` JSON 对象。
