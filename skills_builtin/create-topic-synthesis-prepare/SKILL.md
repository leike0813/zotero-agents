...
为新建主题综合运行准备主题意图、resolver、文献工作集和分析上下文；需要可通过 `zotero-bridge` 访问 Zotero 文库。
