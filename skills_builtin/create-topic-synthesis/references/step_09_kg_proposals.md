...
本步骤在 `persist_core_sections` 之后执行，目标是把新建 topic synthesis 过程中已经形成的语义上下文转成插件可摄取的 KG proposal sidecars。你只写 proposal，不写 canonical KG assets，也不把 proposal 写进最终正文 section。
