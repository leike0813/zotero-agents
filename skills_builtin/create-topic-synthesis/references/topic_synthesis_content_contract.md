...
完整 `topic_synthesis` artifact 应包含以下 section：
...
- `paper_evidence`、`evidence_map`、`source_artifacts`、`diagnostics` 偏机器溯源。
- `topic`、`summary`、`positioning`、`taxonomy`、`timeline_events`、`claims`、
  `comparison_matrix`、`debates`、`gaps`、`external_literature_analysis`、
...
`synthesis_report` 是面向人类阅读的连续正文。它不是 Markdown export 的替代品，也不
是 JSON section 的重复，而是将结构化分析串成一篇短报告。
...
1. topic 定义与范围。
2. 主要研究路线。
   - 该章节应以 `taxonomy.summary` 为直接上游材料。
   - 可以重写为更自然的报告语言，但不得绕开 taxonomy nodes 另起炉灶。
   - 必须保留路线之间的关系、主流/新兴/成熟判断和核心 trade-off。
3. 历史沿革和递进逻辑。
   - 该章节应以 `timeline_events.summary` 为直接上游材料。
   - 可以根据报告节奏压缩，但必须保留 phase 递进、里程碑论文和问题演化链条。
4. 核心结论。
5. 主要比较与争议。
6. gaps 和覆盖限制。
7. 外部文献与建议入库方向。

也就是说，`taxonomy.summary` 与 `timeline_events.summary` 是 `synthesis_report`
中两个核心章节的内容真源。最终报告可以润色、压缩和衔接，但不应把这两部分重新生成成
彼此不一致的说法。
