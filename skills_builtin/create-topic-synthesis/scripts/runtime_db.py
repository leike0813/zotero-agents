...
def derive_topic_id_from_title(title: str) -> str:
    slug = (
        title.lower()
        .strip()
        .replace("&", " and ")
    ...
    if slug:
        return slug
    suffix = sha256_text(canonical_json({"topic_title": title}))[
        len("sha256:") : 16
    ]
    return f"topic-{suffix}"


def normalize_create_topic_context_payload(payload: dict) -> dict:
    allowed_keys = {
        "topic_title",
        "aliases",
        "definition",
    ...
    if unknown:
        raise ValueError(
            "create topic context payload contains runtime-owned or unknown fields: "
            + ", ".join(unknown)
        )
    title = _clean_text(payload.get("topic_title"))
    if not title:
        raise ValueError("create topic context payload requires topic_title")
    duplicate_status = _clean_text(payload.get("duplicate_status"))
    if duplicate_status not in CREATE_DUPLICATE_STATUSES:
        raise ValueError(
            "duplicate_status must be one of: "
            + ", ".join(sorted(CREATE_DUPLICATE_STATUSES))
        )
    aliases = _clean_string_list(payload.get("aliases"), "aliases")
    include = _clean_string_list(payload.get("scope_include"), "scope_include")
    exclude = _clean_string_list(payload.get("scope_exclude"), "scope_exclude")
    duplicate_candidate_ids = _clean_string_list(
        payload.get("duplicate_candidate_ids"), "duplicate_candidate_ids"
    )
    topic_definition = {
        "id": derive_topic_id_from_title(title),
        "title": title,
        "aliases": aliases,
    ...
    duplicate_check = {
        ...
        "candidate_topic_ids": duplicate_candidate_ids,
        "reason": _clean_text(payload.get("duplicate_reason")),
    }
    normalized = {
...
def persist_topic_intent(conn: sqlite3.Connection, payload: dict) -> dict:
    ...
    payload = normalize_create_topic_context_payload(payload)
    ...
    *,
    ...
    for row in receipt_rows:
        ...
        if action_name in ACTION_ALIASES:
            errors.append(f"runtime_integrity_noncanonical_action_receipt: {action_name}")
        try:
            result = json.loads(row["result_json"])
        except Exception:
