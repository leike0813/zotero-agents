...
- `result/result.json` 由 ACP runner 在 final assistant envelope 校验通过后写入；本 skill 的脚本不得写入或要求 agent 手写该文件。
- `scripts/stage_runtime.py` stdout 只是 stage action 回执，例如 `{"ok": true, ...}`，不是最终 assistant output。
- `__SKILL_DONE__` 只属于 ACP assistant turn envelope，不得写入 `manuscript-literature-framing.result.json` 或 `assets/output.schema.json`。
...
```json
{
  ...
  "message": "Required Zotero or Synthesis host tools are unavailable, so manuscript literature framing cannot proceed.",
...
每个 analysis payload 必须包含对应阶段的最小结构字段；脚本只校验字段存在和非空，不判断 taxonomy、gap 或 contribution alignment 的语义质量。
...
Final assistant output 必须且只能是合法 JSON object，不得追加解释或 Markdown fence。最终输出不是 `scripts/stage_runtime.py` 的 action 回执，也不是原样输出无 marker 的业务结果文件。

完成或取消后，必须读取 run workspace 根目录下的 `manuscript-literature-framing.result.json`，再输出 ACP final envelope：

- 根字段必须包含 `"__SKILL_DONE__": true`。
...
Canceled final branch 使用 `kind: "manuscript_literature_framing_canceled"`，同样必须包含 `"__SKILL_DONE__": true`。
