...
|------------|-------------|------|
| **Most Synthesis heavy computation still shares the UI process** | Citation Graph layout and Metrics use the bounded Rust worker with no local fallback. Graph build has a wire-bounded canary plus an authenticated packed-transfer canary: 4 MiB pages are streamed one at a time, normal 2,000/100,000 input is hard-gated under the worker limit, and output is published atomically. Production graph build, Advanced Matching, and the other five kernels still execute in the Zotero process. | Add plugin-owned basis recapture and paged promotion in a separate production graph-build cutover; migrate remaining kernels through separate verified changes |
| **Synthesis persistence has not cut over to the service** | The Node WS5 foundation remains the frozen oracle. R7 now has independent Rust shadow repository, canonical-store, and application crates plus authenticated mutation-disabled Workbench/canonical read canaries; Node and Rust never share a database, canonical root, or writer lease. Local repository locking, journal recovery, application/corpus parity, canary smoke, and package-size gates are implemented, while the five-platform workflow still requires a controlled remote run. Production `synthesis.db`, Topic canonical files, WebDAV credentials/HTTP, Host paper details, downstream effects, and all production domain writes remain in plugin composition. | Verify R7 on all five targets, then implement R8 native lifecycle and R9 explicit single-writer cutover without automatic fallback |
