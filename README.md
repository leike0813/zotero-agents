...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Seven private environment-neutral Topic, Reference Refresh, Reference Matching/Review, Citation Graph, Tag Vocabulary, Concept KB, and Topic Graph applications operate only on that isolated state. A private knowledge-checkpoint coordinator can capture, preview, and atomically replace the three SQLite-only Tag/Concept/Topic Graph aggregates with a single-use in-process receipt; it preserves local Tag work rows and marks retained last-good indexes stale. None has an RPC route or automatic invocation. Production `synthesis.db`, Topic canonical files, Host reads/writes, graph basis/promotion, canonical checkpoint/JSON import/WebDAV behavior, downstream effects, and all domain writes remain in plugin composition. | Complete the durable-bundle/WebDAV slices, then WS6 shadow parity and WS7 explicit single-writer cutover without automatic fallback |
