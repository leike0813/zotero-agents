...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. A private environment-neutral Topic application can list, inspect, and optimistically apply complete or structured-patch fixtures into that isolated state, with CAS/recovery and indexed projections; no remote apply capability exists. Production `synthesis.db`, Topic canonical files, and all domain writes remain in plugin composition. | Complete the remaining WS5 application families and WS6 shadow parity, then perform WS7 as one explicit single-writer cutover without automatic fallback |
