...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Private environment-neutral Topic and Citation Graph applications operate only on that isolated state: Topic supports bounded list/detail/apply, while Citation Graph supports bounded inspection/reads, full rebuild, metrics refresh, and explicit layout recomputation with transactional graph-basis promotion. Neither application has an RPC route or automatic invocation. Production `synthesis.db`, Topic canonical files, graph basis/promotion, and all domain writes remain in plugin composition. | Complete WS6 shadow parity, then perform WS7 as one explicit single-writer cutover without automatic fallback |
