...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile three-table shadow SQLite foundation plus an identity-bound Topic canonical shadow with CAS/recovery and descriptor-only inspect. Neither shadow receives production paths or exposes an apply/write capability; production `synthesis.db`, Topic canonical files, and all domain writes remain in plugin composition. | Connect the remaining WS5 Topic application use cases, complete WS6 shadow parity, then perform WS7 as one explicit single-writer cutover without automatic fallback |
