...
| **Synthesis persistence has not cut over to the service** | The current milestone is **Stage 1 / WS5 — Private Isolated Synthesis Foundation Complete**; this is not Stage 1 completion, production readiness, cutover, or real-machine acceptance. The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Seven private domain applications plus checkpoint, durable-bundle, WebDAV, and debug/maintenance coordinators operate only on that isolated state. Debug reads are bounded, JSON-safe, side-effect free, and return `superseded` rather than mixing repository and canonical bases; Node profiling is unavailable by default. Existing checkpoint, durable, and protected-reset owners retain mutation authority. None has an RPC route or automatic invocation. Production `synthesis.db`, Topic canonical files, WebDAV credentials/HTTP, Host paper details, downstream effects, and all production domain writes remain in plugin composition. | Run WS6 shadow parity, then WS7 explicit single-writer cutover without automatic fallback |
