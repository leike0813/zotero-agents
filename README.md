...
|------------|-------------|------|
| **Most Synthesis heavy computation still shares the UI process** | Citation Graph layout and Metrics share one bounded sidecar admission/fuse. Layout runs in the Node Worker; Metrics v2 runs only in the bundled Rust child, with no local fallback. Graph build has a wire-bounded canary plus an authenticated packed-transfer canary: 4 MiB pages are streamed one at a time, normal 2,000/100,000 input is hard-gated under the worker limit, and output is published atomically. Production graph build, Advanced Matching, and the other five kernels still execute in the Zotero process. | Add plugin-owned basis recapture and paged promotion in a separate production graph-build cutover; migrate remaining kernels through separate verified changes |
