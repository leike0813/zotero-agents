...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Private environment-neutral Topic, Reference Refresh, Reference Matching/Review, and Citation Graph applications operate only on that isolated state. Reference Refresh uses bounded changed-artifact materialization; Matching/Review executes both strict matcher passes and promotes proposals/facts only after Host/reference basis recapture. Topic and Graph retain their bounded application surfaces. None has an RPC route or automatic invocation, and private reference changes only mark downstream projections stale. Production `synthesis.db`, Topic canonical files, Host reads, graph basis/promotion, downstream effects, and all domain writes remain in plugin composition. | Complete WS6 shadow parity and paged Host materialization, then perform WS7 as one explicit single-writer cutover without automatic fallback |
