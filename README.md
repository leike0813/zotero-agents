...
|------------|-------------|------|
| **Most Synthesis heavy computation still shares the UI process** | Citation Graph layout and metrics now share the bounded supervised sidecar worker. Graph build has a wire-bounded canary plus an authenticated packed-transfer canary: 4 MiB pages are streamed one at a time, normal 2,000/100,000 input is hard-gated under the worker limit, and output is published atomically. Production graph build, Advanced Matching, and the other five kernels still execute in the Zotero process. The two production worker routes fail closed without local fallback; graph-build canaries are never invoked automatically. | Add plugin-owned basis recapture and paged promotion in a separate production graph-build cutover; migrate remaining kernels through separate verified changes |
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Seven private environment-neutral domain applications operate only on that isolated state. A private knowledge-checkpoint coordinator handles the three SQLite-only knowledge aggregates, while the private durable-bundle application verifies v1/v2 bundles, previews incremental diffs/conflicts, and applies a single-use receipt with SQLite CAS plus recoverable multi-Topic JSON/Markdown promotion. A private WebDAV application now owns strict HEAD discovery, preview-first import, deterministic snapshot publication, ETag conflicts, retry state, and shutdown drain over injected ports; its Node composition uses an identity-bound shadow state store and a disabled Host port. Tombstone apply remains blocked. None has an RPC route or automatic invocation. Production `synthesis.db`, Topic canonical files, WebDAV credentials/HTTP, downstream effects, and all production domain writes remain in plugin composition. | Complete WS5 debug/maintenance foundations, then WS6 shadow parity and WS7 explicit single-writer cutover without automatic fallback |
