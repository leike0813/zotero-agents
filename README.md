...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Seven private environment-neutral domain applications operate only on that isolated state. A private knowledge-checkpoint coordinator handles the three SQLite-only knowledge aggregates, while the private durable-bundle application now verifies v1/v2 bundles, previews incremental diffs/conflicts, and applies a single-use receipt with SQLite CAS plus recoverable multi-Topic JSON/Markdown promotion. Tombstone apply is still blocked. None has an RPC route or automatic invocation. Production `synthesis.db`, Topic canonical files, Host reads/writes, WebDAV transport/index, downstream effects, and all production domain writes remain in plugin composition. | Complete the private WebDAV composition slice, then WS6 shadow parity and WS7 explicit single-writer cutover without automatic fallback |
