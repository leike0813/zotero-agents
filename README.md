...
| **Synthesis persistence has not cut over to the service** | The Node service now owns a persistent, per-profile three-table shadow SQLite foundation for lifecycle and parity work. It cannot access production paths and exposes no repository capability; production `synthesis.db`, Topic canonical files, and all domain writes remain in plugin composition. | Complete WS6 shadow parity, then perform WS7 as one explicit single-writer cutover without automatic fallback |
