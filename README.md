...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Private environment-neutral Topic, Reference Refresh, and Citation Graph applications operate only on that isolated state. Reference Refresh uses a bounded two-stage prepare/apply materialization protocol and transactional full or source-scoped CAS promotion; Topic and Graph retain their existing bounded application surfaces. None has an RPC route or automatic invocation. Production `synthesis.db`, Topic canonical files, Host reads, graph basis/promotion, and all domain writes remain in plugin composition. | Complete WS6 shadow parity and paged Host materialization, then perform WS7 as one explicit single-writer cutover without automatic fallback |
