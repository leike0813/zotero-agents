...
|------------|-------------|------|
| **Most Synthesis heavy computation still shares the UI process** | Citation Graph layout and metrics now share the bounded supervised sidecar worker. Graph build has an internal wire-bounded worker canary and reproducible cross-process benchmark, but its 2k/20k fixture already exceeds structural request and response limits; normal scale times out in the worker and target/stress cannot materialize under the isolated benchmark budget. Production graph build, Advanced Matching, and the other five kernels still execute in the Zotero process and can briefly freeze the UI. The two production worker routes fail closed and do not fall back locally; the canary is not invoked automatically. | Define bounded input staging, result access/validation, and worker transfer before routing graph build; remaining kernels migrate through separate verified changes |
