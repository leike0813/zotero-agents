...
|------------|-------------|------|
| **Most Synthesis heavy computation still shares the UI process** | Citation Graph layout and metrics now share the bounded supervised sidecar worker. Graph build has a wire-bounded worker canary, benchmark evidence, and an authenticated large-transfer staging canary with 4 MiB pages, bounded sessions, TTL, and symmetric input/output manifests. Staging does not execute normal-scale compute, and production graph build, Advanced Matching, and the other five kernels still execute in the Zotero process. The two production worker routes fail closed without local fallback; neither graph-build canary is invoked automatically. | Connect staged rows to a packed bounded worker and validate result paging before a separate production graph-build cutover; migrate remaining kernels through separate verified changes |
