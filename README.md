...
| **Synthesis persistence has not cut over to the service** | The Node service owns a persistent per-profile shadow SQLite repository and identity-bound Topic canonical shadow. Private environment-neutral Topic, Reference Refresh, Reference Matching/Review, Citation Graph, and Tag Vocabulary applications operate only on that isolated state. Reference Refresh uses bounded changed-artifact materialization; Matching/Review executes both strict matcher passes and promotes proposals/facts only after Host/reference basis recapture. Tag Vocabulary validates and indexes through the bounded worker, promotes vocabulary/staging/effect rows transactionally, and leaves unavailable Host effects pending. None has an RPC route or automatic invocation. Production `synthesis.db`, Topic canonical files, Host reads/writes, graph basis/promotion, import/checkpoint/WebDAV behavior, downstream effects, and all domain writes remain in plugin composition. | Complete WS6 shadow parity and paged Host materialization, then perform WS7 as one explicit single-writer cutover without automatic fallback |
