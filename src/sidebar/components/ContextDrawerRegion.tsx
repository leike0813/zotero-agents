...
function WorkspaceTask(props: {
  ...
  const itemActions = Array.isArray(task.itemActions)
    ...
    : [];
  const submission =
    task.submission && typeof task.submission === "object"
      ? (task.submission as Record<string, unknown>)
      : null;
  let submissionBadge: { symbol: string; tooltip: string } | null = null;
  if (submission && safeText(submission.symbol) && task.terminal !== true) {
      ...
      >
        <div class="assistant-workspace-drawer-task-content">
          ...
          <div class="assistant-workspace-drawer-task-title skillrunner-workspace-task-title">
            {submissionBadge ? (
              <span
                class="assistant-workspace-drawer-task-submission"
                title={submissionBadge.tooltip}
                aria-label={submissionBadge.tooltip}
              >
                {submissionBadge.symbol}
              </span>
            ) : null}
            <span class="assistant-workspace-drawer-task-title-text">
