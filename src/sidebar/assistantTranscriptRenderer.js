...
function trimVirtualTranscriptPages(state, options) {
  ...
  const anchor = state.lastVirtual
    ? nonNegativeInteger(state.lastVirtual.startIndex, Number.MAX_SAFE_INTEGER)
...
function estimatedVirtualRowHeight(entry, estimatedHeight) {
  const textLength = virtualTranscriptEstimatedText(entry && entry.item).length;
...
function buildVirtualTranscriptLayout(entries, state, options, totalRows) {
  ...
  entries.forEach(function (entry) {
    ...
    const height = measuredVirtualRowHeight(state, entry, key, estimatedHeight);
    positions.push({
      entry,
      key,
      index,
...
function restoreVirtualScrollAnchor(container, virtual, anchor) {
  if (!container || !virtual || !anchor || !Array.isArray(virtual.positions)) {
    return false;
  }
  ...
  const offset = Math.max(
    0,
    Math.min(finiteNumber(anchor.offset, 0), Math.max(0, position.height - 1)),
  );
...
function buildVirtualTranscriptWindow(container, state, options) {
  ...
  return {
    ...
    cachedBottomBoundary: layout.positions[layout.positions.length - 1].bottom,
...
function measureVirtualTranscriptRows(container, state, options, dirtyRowKeys) {
...
function decorateMarkdownCodeBlocks(body, options) {
  ...
  codeBlocks.forEach(function (code) {
    ...
    button.setAttribute("aria-label", transcriptText(labels, "copyCodeBlock"));
...
function stableToolActivityGroupKey(run, fallbackIndex) {
  const first = run[0] || {};
  return (
    String(first.itemId || "").trim() || "unknown-" + String(fallbackIndex || 0)
  );
}
...
function itemRole(item) {
  ...
  if (item.rowKind === "tool-call" || item.rowKind === "tool-activity-group") {
    return "tool";
  }
...
function updateTranscriptClasses(row, item, options) {
  ...
  row.classList.toggle("is-process", kind === "process" || kind === "thought");
...
function renderAssistantTranscript(options) {
  ...
  if (virtualized) {
    const nextOwnerKey =
      ...
      String(opts.page && opts.page.ownerKey ? opts.page.ownerKey : "").trim();
  ...
  if (virtualized) {
    ...
    if (pageRejected) {
      ...
      if (mergedPage && !hadCachedPages && opts._virtualScrollRender !== true) {
        resetTranscriptScrollState(container);
      }
      const latestOptions = Object.assign({}, opts);
  ...
  if (virtualized && virtualState && virtual) {
    maybeRequestVirtualTranscriptPages(container, virtualState, virtual, opts);
  }
