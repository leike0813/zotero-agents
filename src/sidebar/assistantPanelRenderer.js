...
function renderSelectControl(container, selector, options) {
  ...
  label.appendChild(
    el("span", "assistant-panel-selector-label", selector.label || selector.id),
  );
  ...
  entries.forEach(function (entry) {
    ...
    if (entry && entry.sentinel)
      option.setAttribute("data-assistant-sentinel", safeText(entry.sentinel));
...
function renderAssistantBanner(container, snapshot, options) {
  ...
  (Array.isArray(panel.context.metadata) ? panel.context.metadata : []).forEach(
    function (item) {
  ...
  if (kind === "auth" && interaction.auth) {
    ...
    const diagnostics = [
      authUrl && !authUrlLinkable
        ...
        : null,
      [labelOf(panel, "userCodePrefix", "user_code:"), safeText(auth.userCode)],
      [
...
function renderAssistantReply(container, snapshot, options) {
  ...
  button.disabled =
    !panel.reply.enabled || (panel.reply.sending === true && !interruptAction);
...
function reconcileWorkspaceTaskRows(existingBody, desiredBody) {
  ...
  const taskStartIndex = directChildren(desiredBody).findIndex(function (node) {
    return safeText(node.getAttribute("data-assistant-task-key"));
  });
...
function reconcileWorkspaceGroups(existingSection, desiredSection) {
  ...
  desiredGroups.forEach(function (desiredGroup, index) {
    const key = safeText(desiredGroup.getAttribute("data-assistant-group-key"));
    desiredKeys.add(key);
...
function renderAssistantContextDrawer(container, snapshot, options) {
  ...
  (Array.isArray(panel.drawers.contexts) ? panel.drawers.contexts : []).forEach(
    function (entry) {
...
function renderDetailsDrawer(container, snapshot, options) {
  ...
  header.appendChild(
    el(
      "strong",
      "",
      panel.drawers.detailsTitle || labelOf(panel, "details.title", "Details"),
    ),
  );
...
function renderPermissionRequestDrawer(root, snapshot, options) {
  ...
  if (!overlay) {
    overlay = el("section", "assistant-panel-permission-drawer-overlay hidden");
  ...
  const review =
    request.review && typeof request.review === "object" ? request.review : {};
  const meta = [
