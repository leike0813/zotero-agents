...
type ServerState = {
  ...
  hostBridgeStatus?: HostBridgeStatusSnapshot;
};
...
function isOriginAllowed(request: HttpRequest) {
  ...
  try {
    ...
    if (state.hostBridgeStatus?.lanEnabled === true) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const response = await handleZoteroMcpJsonRpc(payload, {
        ...
        resolveHostBridgeStatus: () => state.hostBridgeStatus!,
...
function syncMcpRouteStateFromHostBridge(server: HostBridgeStatusSnapshot) {
  updateState({ hostBridgeStatus: server });
  if (!isZoteroMcpServerEnabled()) {
...
async function startServer(server: HostBridgeStatusSnapshot) {
  updateState({
    status: "starting",
    lastError: "",
  });
  ...
  try {
    syncMcpRouteStateFromHostBridge(server);
    if (state.status !== "running") {
      throw new Error(
        server.lastError ||
          server.lastRecoveryReason ||
          "Host Bridge route is unavailable",
      );
    }
    const endpoint = mcpEndpointFromHostBridge(server);
    const facts = endpointFacts(endpoint);
...
export async function ensureZoteroMcpServer(
  ...
  if (args.hostBridgeStatus) {
    syncMcpRouteStateFromHostBridge(args.hostBridgeStatus);
  }
  ...
  if (!args.hostBridgeStatus) {
    throw new Error(
      "Host Bridge owner must provide a running route before MCP can start",
    );
  }
  if (!startingPromise) {
    startingPromise = startServer(args.hostBridgeStatus).finally(() => {
...
export async function handleZoteroMcpHostAccessRequest(
  request: HttpRequest,
  hostBridgeStatus: HostBridgeStatusSnapshot,
) {
  if (isZoteroMcpServerEnabled() && state.status !== "running") {
    await ensureZoteroMcpServer({ hostBridgeStatus });
  } else if (isZoteroMcpServerEnabled()) {
    syncMcpRouteStateFromHostBridge(hostBridgeStatus);
  }
...
export function configureZoteroMcpServerForTests(
  ...
  updateState({
    ...
    hostBridgeStatus: {
      ...
      host: HOST,
      port: 0,
      endpoint: (args.endpoint || "http://127.0.0.1:0/mcp").replace(
        /\/mcp\/?$/,
        "/bridge/v2",
      ),
      remoteEndpoint: "",
