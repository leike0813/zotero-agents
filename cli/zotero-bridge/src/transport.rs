...
pub(super) fn last_operation_id() -> Option<String> {
    LAST_OPERATION_ID
...
pub(super) fn health(config: &BridgeConfig) -> Result<Value, CliError> {
...
pub(super) fn post(config: &BridgeConfig, path: &str, body: Value) -> Result<Value, CliError> {
...
pub(super) fn download(config: &BridgeConfig, path: &str) -> Result<DownloadResponse, CliError> {
    let endpoint = parse_endpoint(&config.endpoint)?;
    let target = format!("{}{}", endpoint.base_path, path);
    let scope_text = config
        .scope
...
fn parse_endpoint(endpoint: &str) -> Result<ParsedEndpoint, CliError> {
    let without_scheme = endpoint.strip_prefix("http://").ok_or_else(|| {
        CliError::config(
            ...
            "Only http:// Zotero Bridge service endpoints are supported in v2",
        )
    })?;
...
mod tests {
    ...
    fn parses_bridge_endpoint() {
        let endpoint = parse_endpoint("http://127.0.0.1:26570/bridge/v2").unwrap();
        assert_eq!(endpoint.host, "127.0.0.1");
        assert_eq!(endpoint.port, 26570);
        assert_eq!(endpoint.base_path, "/bridge/v2");
    }
    ...
    fn spawn_download_server(responses: Vec<Vec<u8>>) -> (u16, thread::JoinHandle<()>) {
        ...
        let handle = thread::spawn(move || {
            for response in responses {
                ...
                assert!(request.contains("GET /bridge/v2/files/file-1 HTTP/1.1"));
                assert!(request.contains("Authorization: Bearer secret-token"));
    ...
    #[test]
    fn authenticated_get_can_query_local_bridge_without_returning_token() {
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let port = listener.local_addr().unwrap().port();
        let handle = thread::spawn(move || {
            let (mut stream, _) = listener.accept().unwrap();
        ...
        set_current_command("bridge manifest");
        let result = get(&config, "/manifest").unwrap();
