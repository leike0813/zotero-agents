...
| **Kilo Code Auto Free** | Бесплатно | Встроенный режим Auto Free автоматически направляет каждый запрос к подходящей бесплатной модели. Не требуется API-ключ или учётная запись |
| **OpenCode Zen / OpenRouter Free** | Бесплатно | OpenCode Zen включает встроенные бесплатные модели; OpenRouter также предоставляет бесплатные модели (Gemini 2.5 Flash, DeepSeek V3). С ограничением скорости, но без затрат |

### Ограничения бесплатной версии

Бесплатные модели — отличный способ начать, но у них есть компромиссы:

| Ограничение | Чего ожидать |
|-------------|--------------|
| **Ограничение скорости** | Запросы могут замедляться — от 5 до 20 запросов в минуту в зависимости от нагрузки провайдера. Пакетная обработка значительно замедляется |
| **Параллельность** | Обычно только один одновременный запрос. Отправка нескольких рабочих процессов одновременно может поставить их в очередь или привести к сбою |
| **Доступность моделей** | Пул бесплатных моделей может быть исчерпан в часы пик. Возможны ошибки «модель недоступна» или «превышена ёмкость» |
| **Смена моделей** | Провайдеры могут незаметно заменять бесплатные модели без предупреждения. Качество вывода может варьироваться между запусками |
| **Отсутствие SLA** | Бесплатные тарифы не предоставляют гарантий доступности. Сервисы могут быть временно недоступны или прекращены |

> Если вам нужна надёжная пакетная обработка или промышленное использование, рассмотрите платный план (OpenCode Go или Coding Plan) — стоимость одной статьи ничтожна по сравнению с экономией времени.
...
| **Kilo Code** | Встроенный режим Auto Free — автоматически направляет к доступным бесплатным моделям без настройки. Поддерживает изоляцию конфигурации через переменные XDG. Также работает с платными API-ключами | **Бесплатно** (Auto Free) | ⭐⭐⭐ Отличный бесплатный вариант |
| **Opencode** | Qwen3.5-Plus / Kimi-K2.5 / GLM-5 и другие модели отлично справляются с литературными задачами. [OpenCode Go](https://opencode.ai/go?ref=SZDFT9GZKW) предлагает дешёвую квоту; Zen-версия включает встроенные бесплатные модели; также можно использовать бесплатные модели OpenRouter | Бесплатно (Zen / OpenRouter) или дёшево (Go) | ⭐⭐⭐ Настоятельно рекомендуется |
...
| Документ | Описание |
|----------|----------|
| [Архитектура и потоки](docs/architecture-flow.md) | Общий обзор конвейера выполнения (включая диаграмму Mermaid) |
| [Руководство разработчика](docs/dev_guide.md) | Ключевые компоненты, модель конфигурации, цепочка выполнения |
| [Компоненты воркфлоу](docs/components/workflows.md) | Схема манифеста, обработчики, фильтрация ввода, семантика выполнения |
| [Компоненты провайдеров](docs/components/providers.md) | Система контрактов провайдеров, типы запросов |
| [Стратегия тестирования](docs/testing-framework.md) | Две среды выполнения, режимы lite/full, контрольные точки CI |
| [Слой Synthesis](docs/synthesis-layer/README.md) | Внутреннее устройство графов знаний, графов цитирований и концептуальной базы знаний |
