...
- [ ] 12.5 After `stabilize-synthesis-sidecar-prebuild-pipeline` produces one trusted-verification-bound v3 result under separate execution authorization, replace the stale eight-capability bundles with its exact seven-platform content-addressed candidate and prove freshness, licenses, provenance, SBOM, per-target size, aggregate size, and native-only XPI inventory
