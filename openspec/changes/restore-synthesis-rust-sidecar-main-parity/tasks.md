...
## 19. Restore Audited Production Business Chains

- [x] 19.1 Reopen the change with fixed-baseline requirements for Topic representative-image reads, incremental Graph Related Items effects, staged Tag legacy-binding migration, and real-route reachability evidence
- [x] 19.2 Add failing real production-route, Host response-governance, and focused Rust evidence before implementing the three chains
- [x] 19.3 Restore typed Topic representative-image resolution, strict Host validation, bounded policy, and best-effort public projection
- [x] 19.4 Restore Related Items planning, pending-before-Host persistence, strict receipt coordination, independent operation terminal, echo, undo, and reopen behavior after successful manual incremental Graph refresh
- [x] 19.5 Restore the shared staged Tag legacy-binding migration gate, strict batched Host partition validation, atomic CAS rewrite, startup attempt, and six-entry-point coverage
- [x] 19.6 Correct active documentation, append premerge audit evidence, and run the focused OpenSpec, contract, Rust, build, surface, route, and diff gates without claiming full premerge acceptance

### 2026-08-12 audited production business-chain evidence

- The grouped Topic digest client now reaches the existing representative-image Host capability only when requested and note-bound. The real process route covered available, absent, unavailable, and opt-out results; strict Rust tests covered DTO closure, base64, decoded size, byte metadata, MIME, dimensions, presentation metadata, and diagnostic bounds.
