...
### Requirement: Legacy synthesize-topic workflow remains published

The old single `synthesize-topic` workflow SHALL remain published as a builtin
workflow.

#### Scenario: Legacy workflow is loaded

- **WHEN** builtin synthesis-layer workflows are scanned
- **THEN** `synthesize-topic` SHALL be loaded.
