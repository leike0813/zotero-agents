...
### Requirement: Workflow manifest schema allows execution.supportedBackends

The workflow manifest schema SHALL allow authors to declare compatible backend
types under `execution.supportedBackends`.

#### Scenario: Author declares ACP-only backend compatibility

- **WHEN** a workflow manifest declares `"execution": { "supportedBackends": ["acp"] }`
