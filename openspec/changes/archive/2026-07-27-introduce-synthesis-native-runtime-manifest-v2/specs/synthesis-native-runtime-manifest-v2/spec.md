...
### Requirement: Native runtime manifest v2 SHALL have one strict identity

Every installable Synthesis runtime MUST use
`synthesis-sidecar-runtime-bundle.v2`, identify `rust-native`, one supported
...
capabilities, creation/expiry policy, platform signature state, and a complete
sorted file inventory.

#### Scenario: A Node or ambiguous manifest is supplied
- **WHEN** a manifest contains a Node version, upstream Node archive, JavaScript entrypoint, unknown field, unsafe path, duplicate file, unsupported target, mismatched triple, or incomplete identity
- **THEN** the complete manifest SHALL be rejected before any declared file is read, written, or executed
