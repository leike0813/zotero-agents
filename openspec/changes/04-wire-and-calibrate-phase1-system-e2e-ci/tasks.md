...
- [ ] 6.3 Verify all promoted Windows release cells include passing `CG-02` evidence and the recorded Zotero 9 classification; leave any unsupported cell non-blocking
  ...
  - The remaining Windows blocker is a single case: `PA-01` fails with `family_cleanup_indeterminate` and a boolean assertion inside `reads a historical Topic without rewriting its read-only metadata`. Promotion of any Windows cell still needs that plus trustworthy `CG-02` evidence and the Zotero 9 classification.
