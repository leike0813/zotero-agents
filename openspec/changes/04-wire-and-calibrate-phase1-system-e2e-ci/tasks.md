...
- [ ] 6.3 Verify all promoted Windows release cells include passing `CG-02` evidence and the recorded Zotero 9 classification; leave any unsupported cell non-blocking
  ...
  - `shorten-windows-sidecar-session-paths` fixed that layout, and `e2e-calibration-release-431d6836-r14` (run `35487844842`) confirms it: the Windows cells record no launch failure at all and log 430–608 succeeded sidecar operations, finishing in 3.8–4.6 min. The remaining Windows blocker is narrower: 6 of 10 cases pass, and `PA-01` (`family_cleanup_indeterminate`), `PM-01`, `PM-04`, and `RH-02` fail. Three of those fail because the runner cannot write `<session root>/test-checkpoints/<checkpoint>.armed` (`NS_ERROR_FILE_NOT_FOUND`), and `PA-01` fails an assertion.
