# skillrunner-workspace-surface Specification

## Purpose
The SkillRunner run workspace projects its read model into v1 region
publications through a stateless surface adapter, serves transcript reads
page-first from the bounded in-memory session history, and preserves
SkillRunner run semantics end to end.
## Requirements
