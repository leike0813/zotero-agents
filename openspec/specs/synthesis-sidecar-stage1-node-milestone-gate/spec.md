...
legacy active/previous pointers, fallback routing, and release assets MUST
contain no Node Synthesis service, JavaScript worker implementation, or
mechanism that can restore one. Runtime delivery MUST contain only the seven
manifest-v3 Rust bundles and the fixed XPI `current` installation. Historical
source remains available only through version control, not a tracked deprecated
copy.
