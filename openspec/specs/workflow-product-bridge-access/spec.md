...
asset and SHALL use the existing registered-file download protocol.
...
- **THEN** the bridge SHALL validate that the managed asset belongs to that
  product and still exists
- **AND** it SHALL return public asset metadata with a short-lived opaque file
  descriptor registered as a workflow artifact
- **AND** it SHALL NOT return bytes or an internal absolute path in capability
  JSON.

#### Scenario: Asset cannot be resolved
- **WHEN** the selected asset is missing, belongs to another product, or resolves
  outside managed product storage
- **THEN** the bridge SHALL return the stable `workflow_product_asset_not_found`
  error
- **AND** it SHALL not register a file handle.
