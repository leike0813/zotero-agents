# skillrunner-runkey-ssot Specification

## Purpose
Defines the contract for SkillRunner run identity using `runKey` as the single source of truth for UI operations, replacing fallbacks to `requestId`, `localRunId`, `taskId`, and sequence identifiers.
...
SkillRunner lightweight projection read SHALL expose `runKey` from the projection row key and SHALL NOT require reading full run payloads.
