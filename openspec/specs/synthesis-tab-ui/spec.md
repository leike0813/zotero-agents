...
The Synthesis Workbench SHALL retain the current production startup failure until a later startup generation succeeds. It SHALL provide retry and diagnostics actions, SHALL deduplicate transient notifications by failure generation, and SHALL distinguish a manual-recovery failure from an ordinary unavailable state without replacing its stable reason code.
...
#### Scenario: Migration requires manual recovery
- **WHEN** startup reports `manual-recovery-required`
- **THEN** Workbench explains that automatic startup stopped without replacing existing Synthesis data
- **AND** retains the stable reason code and retry and diagnostics actions
