...
The CI workflow SHALL push the built site to a Gitee Pages branch and trigger a Gitee Pages rebuild.
...
The existing `deploy-user-docs.yml` SHALL be updated to use Docusaurus build commands and paths.

#### Scenario: Updated build command runs
- **WHEN** the CI workflow runs
- **THEN** `npm run docs:build` is executed and exits with code 0
