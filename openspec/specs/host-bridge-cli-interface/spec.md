...
The CLI SHALL expose `surface identity --json`, `surface describe <command> --json`, and `surface search --intent <intent> --json` without loading a Host Bridge profile or connecting to Zotero.
...
Every CLI error envelope SHALL include `retryable`, `stateChanged`, `handleConsumed`, and `safeNextActions`, with optional `nextCommand` when one exact recovery command is known.
