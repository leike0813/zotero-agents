...
The CLI SHALL expose `surface identity --json`, `surface describe <command> --json`, and `surface search --intent <intent> --json` without loading a Host Bridge profile or connecting to Zotero. The embedded `host-bridge.agent-surface.v2` descriptor SHALL separate canonical argv, exact property-to-option or positional bindings, decoded payload schema, result data and delivery schema, intent and operation guidance, endpoint/capability mapping, staged approval, typed effects, pagination/file output, typed-handle transitions, and preconditioned recovery for every agent-facing command.
