...
- `src/modules/workflow/ui/workflowExecute.ts`：执行主链路
- `src/modules/*Dialog.ts`：设置/Dashboard/日志窗口
- `src/modules/taskDashboardHistory.ts`：Dashboard 历史持久层（30 天保留）
- `src/modules/skillRunner/surface/skillRunnerManagementDialog.ts`：SkillRunner 管理页内嵌宿主
- `src/providers/skillrunner/managementClient.ts`：SkillRunner 管理 API 客户端（Dashboard 观察页）
- `src/modules/workflow/ui/workflowDebugProbe.ts`：Debug Probe 诊断 workflow 支持
- `src/modules/workflow/catalog/workflowPackageDiagnostics.ts`：Workflow Package 诊断工具
- `src/modules/skillRunner/runtime/skillRunnerCtlBridge.ts`：SkillRunner 本地后端控制桥接
- `src/modules/skillRunner/run/skillRunnerTaskReconciler.ts`：SkillRunner 任务收敛器
- `src/modules/skillRunner/run/skillRunnerRunStore.ts`：SkillRunner 专用 run store 与 UI projection 来源
- `src/modules/skillRunner/connection/skillRunnerConnectionGovernor.ts`：SkillRunner HTTP/SSE 连接治理
...
- 默认路线按当前 `references/Skill-Runner` 源码执行：先 `POST /v1/jobs`
...
- `references/Skill-Runner/docs/api_reference.md` 中的 `/v1/temp-skill-runs`
...
- 架构总览：`docs/architecture-flow.md`
...
- Workflow Hook Helpers：`docs/components/workflow-hook-helpers.md`
- Zotero Host Capability Broker SSOT：`docs/components/zotero-host-capability-broker-ssot.md`
- Plugin Skill Registry / ACP Compatible SSOT：`docs/components/plugin-skill-registry-and-acp-compatible-ssot.md`
- Runtime Persistence Governance SSOT：`docs/components/runtime-persistence-governance-ssot.md`
- Synthesis Layer 文档入口：`docs/synthesis-layer/README.md`
- Synthesis Library SSOT / Sidecar Cache：`docs/synthesis-layer/library-ssot-and-sidecar-cache.md`
- Synthesis Layer 分域模型：`docs/synthesis-layer/domain-model.md`
- Synthesis Registry/Citation Graph：`docs/synthesis-layer/registry-and-citation-graph.md`
- Synthesis Topics/Discovery：`docs/synthesis-layer/topics-and-discovery.md`
- Synthesis Runtime/Rebuild：`docs/synthesis-layer/runtime-and-rebuild.md`
- Providers：`docs/components/providers.md`
- Selection/Context：`docs/components/selection-context.md`
- Selection/Context Schema：`docs/components/selection-context.schema.json`
- Zotero Host 能力与写入：`docs/components/zotero-host-capability-broker-ssot.md`
- Job Queue：`docs/components/job-queue.md`
- Transport：`docs/components/transport.md`
- Local Cache（占位设计）：`docs/components/local-cache.md`
- UI Shell：`docs/components/ui-shell.md`
- Assistant Workspace ACP Surface SSOT：`docs/components/assistant-workspace-acp-surface-ssot.md`
- Mock：`docs/components/zotero-mock.md`
- 测试：`docs/testing-framework.md`
- SkillRunner 状态机 SSOT：`docs/components/skillrunner-provider-state-machine-ssot.md`
- SkillRunner sequence 状态机：`docs/skillrunner-sequence-recovery-state-machine.md`
- SkillRunner run workspace SSOT：`docs/components/skillrunner-provider-global-run-workspace-tabs-ssot.md`
- 本地后端一键部署状态机：`docs/components/skillrunner-local-runtime-oneclick-state-machine-ssot.md`
