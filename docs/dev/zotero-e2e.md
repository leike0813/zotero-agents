...
outer runner 在 `artifacts/test-diagnostics/system-e2e/<runId>/run-manifest.json` 写入 sanitized Run Manifest。浏览器 reporter 仍把事件发送给 scaffold，同时把同一结构化事件镜像到 loopback sink；manifest 不解析日志文本。终态为 `complete`、`aborted` 或 `incomplete`，缺少 public、typed、lifecycle、cleanup 或 health evidence 时不从日志推断通过。reporter 上报的任一用例失败（`zotero-test-fail-detail`）都会让该 invocation 失败并在 HB-03 重启前后保持，所以「用例在 family 记录之外失败」不会留下看似 `complete` 的 manifest；family cleanup 抛错记为 `family_cleanup_failed` 并把消息写进归档的 runner 输出。可发布诊断只保存 workspace-relative reference；敏感或 private-format artifact 只记稳定的 `withheld` reason code。
...
| Lane | Cell identity | Families | 当前状态 |
| --- | --- | --- | --- |
...
`CG-02` Windows cell 与 stress cell 跑同一个 close 测试，区别只在形态：catalog 形态带 `CG-02` case identity、30 轮，并把 `ZOTERO_E2E_TRIGGER_LANE` 标为 `cg-02-windows`；stress 形态是 100 轮、无 case identity。两种形态的 env 都由 `scripts/run-zotero-e2e-stress.ts` 的 `buildSynthesisCloseTestEnvironment` 生成，compat cell 直接复用它，本机命令是 `npm run test:zotero:e2e:cg-02`（catalog）与 `npm run test:zotero:e2e:stress`。

`CG-02` 只能从 debug 构建取得证据，因此该 lane 不按校准 tag 触发：

```bash
gh workflow run system-e2e-evidence.yml --ref dev -f lane=cg-02-windows
```
...
PR 与 main 由 `.github/workflows/ci.yml` 的 `CI` workflow 执行。规划把 blocking cell 拆成 E2E 与非 E2E 两个矩阵：长期存在的 blocking cell 使用 `build` 产出的普通插件构建；已晋级的 E2E cell 则由 `zotero-compatibility-e2e-blocking` 执行，下载 `prepare` 写出的候选物（staged sidecar 加 `compatibility-artifact-identity.json`），并按 planner 给出的 family grouping 与 fixture scale 运行。这条 lane 不带 `continue-on-error`，候选物缺失会直接失败而不是静默跳过；没有 promoted cell 时它的空矩阵整条跳过，因此既有 blocking cell 的关键路径不受 sidecar 准备影响。
...
新 workflow 尚未进入默认分支时，用 `e2e-calibration-pr-*`、`e2e-calibration-main-*`、`e2e-calibration-release-*` tag 从该 tag 指向的提交启动非发布校准。每个 lane 创建三枚唯一 tag，分别保留三个独立 workflow run；每次 push 最多包含三枚 tag（建议同一轮各推 PR/main/release 一枚），超过三枚时 GitHub 不创建 tag push workflow run。`CG-02` lane 不用 tag，因为它需要 debug 构建（见上文），tag 是 detached 的生产构建。workflow 只选 planner 中的 E2E cells，release 校准包含 Linux/Windows，但不调用 `npm run release`，也不产生发布 authority。
