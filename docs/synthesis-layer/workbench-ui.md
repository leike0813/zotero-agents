...
outline is present; the first opened report therefore retains a nonzero,
scrollable body without requiring the page to be opened again.
...
Topic artifact deletion and purge plus discovery-hint rejection and restoration route through strict `SynthesisClient.topics` commands. Delete and purge retain their destructive confirmations and immediate single-flight execution; deletion still surfaces a returned domain reason when the Topic is not found. Topic lists carry only discovery count/status. Reader Topic Detail owns the bounded Discovery section: at most 20 open candidates with Reject actions and 20 rejected candidates with Restore actions, both using the strict `TopicDiscoveryCandidate` DTO and literature identity as the missing-title fallback. Discovery-hint actions return the same DTO, retain trimmed identifiers, empty-ID skipping, singular `diagnostic` handling, and selected-surface refresh, so the refreshed Topic Detail remains authoritative and plural domain diagnostics remain reviewable results. Delete and purge refresh Home/Topics. These commands add no progress callback, streaming state, deferred start, or identifier aliases; Topic queries and mirror operations remain on their current paths.

Topic Graph projection rebuild, edge acceptance/rejection, and review actions route through a distinct `SynthesisClient.topicGraph` capability rather than Citation Graph. Rebuild keeps protected confirmation, deferred start, persisted progress polling, and its current Home-only refresh while carrying no UI callback. Edge actions retain trimmed identifiers, empty-ID skipping, their shared decision single-flight key, and singular diagnostic handling. A low-confidence review approval is terminal: it atomically writes the `confirmed` edge and closes the review as `approved`, with no second confirmation step. Both decisions mark the rebuildable Topic Graph index stale without starting rebuild. Confirming a `broader_than` relation refreshes the persisted discovery cascade, whether it arrives through a direct edge accept or through an approved `broader_than` review; a refresh failure is reported as a warning after the relation commit. The three mutations refresh Home/Topics/Graph/Review. The Topics graph relation review area lists only still-open work: `suggested` edges and `open` review items. Confirmed or rejected edges and approved or rejected reviews are terminal and leave that area; the Review Center shows them only when the status filter is `accepted` or `all`.
