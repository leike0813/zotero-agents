...
| --- | --- | --- |
| Application | `rust/synthesis-sidecar/crates/synthesis-application` | Topic/Graph/Concept/Tag/Reference/checkpoint/durable/WebDAV use cases, validation, receipts, and promotion policy |
| Repository | `rust/synthesis-sidecar/crates/synthesis-repository` | Foundation v2 SQLite facts, bounded queries, migrations, and the single-writer boundary |
| Canonical store | `rust/synthesis-sidecar/crates/synthesis-canonical-store` | Topic representation validation and derivation, typed current views, transport-neutral assets, basis-guarded staging, transaction identity, journal, receipt, and recovery |
