...
- 实现文件：`tests/setup/zotero-mock.ts`
- Node 测试入口会通过 `--require tests/setup/zotero-mock.ts` 自动注入全局 `Zotero`
- Parity 治理合同：`docs/components/zotero-mock-parity.md`
...
- `references/zotero-item-typeMap.xml`
...
差异与风险等级以 `docs/components/zotero-mock-parity.md` 的 drift register 为准。
常见偏差包括：
...
- `tests/fixtures/selection-context/*.json`
- `tests/fixtures/selection-context/attachments/`
