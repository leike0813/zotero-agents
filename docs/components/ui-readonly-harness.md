...
Synthesis data is read from the isolated `synthesisDbPath` when provided, otherwise from `pluginDbPath`; no production service/repository owner, canonical writer, or native mutation client is constructed (enforced by the static boundary tests in `tests/ui/156`).
