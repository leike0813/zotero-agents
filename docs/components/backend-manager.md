...
`src/modules/workflow/settings/backendManager.ts` 是插件的后端连接管理入口。提供 CRUD 对话框管理三种 provider 类型的后端配置（BackendInstance），协调持久化、UI 刷新和副作用清理。

配套文档：
- `docs/components/providers.md` — Provider 系统总览
- `docs/components/acp-backend-presets.md` — ACP 预设集成
