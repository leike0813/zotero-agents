...
| Module | File | Phase |
|--------|------|-------|
| Skill Registry | `src/modules/workflow/catalog/pluginSkillRegistry.ts` | Scan, validate, register |
| Skill Materializer | `src/modules/acp/skillRun/acpSkillMaterializer.ts` | Orchestrate materialization |
| Thin Proxy Materializer | `src/modules/acp/skillRun/acpThinProxySkillMaterializer.ts` | Generate proxy SKILL.md |
| Reference Rewriter | `src/modules/acp/skillRun/acpSkillReferenceRewriter.ts` | Path rewriting + patch blocks |
| Schema Asset Resolver | `src/modules/acp/skillRun/acpSkillSchemaAssets.ts` | Input/parameter schema resolution |
| Output Convergence | `src/modules/acp/skillRun/acpSkillOutputConvergence.ts` | JSON extraction, classification, result write |
| Output Validator | `src/modules/acp/skillRun/acpSkillOutputValidator.ts` | Final payload schema check + repair prompt |
| Result File Fallback | `src/modules/acp/skillRun/acpSkillResultFileFallback.ts` | Disk-based result recovery |
...
`resultJsonPath` and `inputManifestPath` are concrete absolute paths allocated
...
Writes the final result JSON as a pretty-printed file to the run's recorded
`resultJsonPath`. This is the runner-owned envelope; skill business code should
