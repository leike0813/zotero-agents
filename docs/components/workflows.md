...
配套 API 细节请同时参见 `docs/components/workflow-hook-helpers.md`。
...
workflow 清单：`scripts/host-bridge/host-bridge-workflow-catalog.ts` 从
...
- `handlers`：通用操作处理器
- `helpers`：Hook 辅助函数（见 `docs/components/workflow-hook-helpers.md`）
