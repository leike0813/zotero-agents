...
`contracts/host-bridge/surfaces.json` is the canonical composition manifest. It declares
...
research task. `contracts/host-bridge/capabilities.v2.json` owns capability
input/output, effect, approval, and exposure facts.
`contracts/host-bridge/cli-commands.v2.json` owns each command target, closed
...
`artifacts/host-bridge-review/` is the Chinese human-review view of the three
surfaces. Its inventory resolves the same `contracts/host-bridge/surfaces.json` graph as
