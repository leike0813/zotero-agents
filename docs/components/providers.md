...
- 状态机 SSOT：
  - 统一消费模块：`src/modules/skillRunner/run/skillRunnerProviderStateMachine.ts`
  - 详细文档见：`docs/components/skillrunner-provider-state-machine-ssot.md`
...
- `skillrunner.sequence.v1` 语义：
  ...
  - 详细合同见 `docs/skillrunner-sequence-recovery-state-machine.md`
- Runtime options：
  - `acpModeId`：ACP 模式选择
  - `acpModelId`：ACP 模型选择
