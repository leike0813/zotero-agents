...
3. Shared non-test helpers remain under `tests/zotero` during migration window.
...
1. Shared fixtures remain under `tests/fixtures/selection-context`.
2. Workflow-specific fixtures move to `tests/fixtures/workflow-<workflow-id>`.
...
- `tests/fixtures/selection-context/*` -> shared (`core` + multiple workflows)
- `tests/fixtures/literature-analysis/*` -> workflow-literature-analysis
- deprecated reference-matching fixtures are archived under `deprecated/tests/fixtures/`
- `tests/fixtures/workflow-loader-*/*` -> core (loader validation domain)
...
Source root: `tests/zotero/*.test.ts`
Target root: `tests/core/*.test.ts`
...
Additional core domain files (created in-place in `tests/core/`, not migrated from `tests/zotero/`):
...
Source root: `tests/zotero/*`
Target root: `tests/workflow-literature-analysis/*`
...
- `tests/zotero/39-workflow-mineru.test.ts`
...
1. No `*.test.ts` remains under `tests/zotero`.
