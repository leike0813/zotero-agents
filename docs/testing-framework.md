...
详细迁移映射见 `docs/components/test-taxonomy-domain-map.md`。
...
Zotero runner 不直接把 `tests/core`、`tests/ui` 或 `tests/workflow-*`
目录交给 scaffold 打包。`zotero-plugin.config.ts` 只指向
`tests/zotero/<domain>/<mode>` 下的显式入口文件，由这些入口 import
...
- 当前 Zotero lite 入口保留：
  - `tests/core/00-startup.test.ts`
  - `tests/core/11-selection-context-rebuild.test.ts`
  - `tests/core/42-hooks-startup-template-cleanup.test.ts`
  - `tests/core/47-workflow-log-instrumentation.test.ts`
  - `tests/core/52-runtime-bridge.test.ts`
  - `tests/core/87-workflow-package-runtime-diagnostics.test.ts`
  - `tests/core/88-workflow-runtime-scope-diagnostics.test.ts`
  - `tests/core/104-acp-zotero-opencode.integration.test.ts`
  - `tests/core/182-host-bridge-socket.integration.test.ts`
  - `tests/core/185-zotero-library-page-query.zotero.test.ts`
  - `tests/core/186-acp-runtime-file-io.zotero.test.ts`
  - `tests/core/187-runtime-log-persistence.zotero.test.ts`
  - `tests/ui/35-workflow-settings-execution.test.ts`
  - `tests/workflow-literature-workbench-package/45-workflow-note-import-export.test.ts`
  - `tests/workflow-mineru/39-workflow-mineru.test.ts`
- 下列套件不进入 Zotero lite：
  - `tests/core/10-selection-context-schema.test.ts`
  - `tests/core/12-handlers.test.ts`
  - `tests/core/32-job-queue-transport-integration.test.ts`
  - `tests/core/34-generic-http-provider-e2e.test.ts`
  - `tests/core/41-workflow-scan-registration.test.ts`
  - `tests/core/45-runtime-log-manager.test.ts`
  - `tests/core/89-workflow-debug-probe.test.ts`
  - `tests/ui/01-startup-workflow-menu-init.test.ts`
  - `tests/ui/40-gui-preferences-menu-scan.test.ts`
  - `tests/ui/50-workflow-settings-dialog-model.test.ts`
  - `tests/workflow-literature-analysis/21-workflow-literature-analysis.test.ts`
  - `tests/workflow-literature-analysis/23-workflow-literature-analysis-fixtures.test.ts`
  - `tests/workflow-literature-analysis/50-workflow-literature-analysis-mock-e2e.test.ts`
  - `tests/workflow-literature-explainer/21-workflow-literature-explainer.test.ts`
  - `tests/workflow-tag-regulator/64a-workflow-tag-regulator-request-building.test.ts`
  - `tests/workflow-tag-regulator/64b-workflow-tag-regulator-apply-intake.test.ts`
- `tests/core/11-selection-context-rebuild.test.ts` 仅运行 `selection-context-mix-all-top3-parents` 子夹具
...
- 在 workflow/ui 的高复杂度测试文件内，部分边界/兼容性用例通过 `itFullOnly` 下沉到 `full`
  - 代表性文件：`tests/workflow-literature-analysis/21-workflow-literature-analysis.test.ts`、`tests/workflow-mineru/39-workflow-mineru.test.ts`、`tests/ui/40-gui-preferences-menu-scan.test.ts`
...
- `tests/fixtures/selection-context/selection-context-mix-all-top3-parents.ts`
...
- `docs/components/test-taxonomy-domain-map.md`
- `docs/components/test-suite-governance.md`
- `docs/components/zotero-mock-parity.md`
...
- diagnostic outputs default to `artifacts/test-diagnostics/`
...
- `tests/core/175-acp-runtime-performance-profiler.test.ts` locks activation,
  lifecycle, aggregation, bounds, drift sampling, immutability, and failure
  isolation.
- `tests/core/176-acp-silent-runtime-performance-baseline.test.ts` runs a
...
- `tests/core/178-acp-runtime-semantic-trace.test.ts` locks lossless payloads,
...
- `tests/core/180-acp-runtime-replay-controller.test.ts` locks trace preflight
...
- `tests/core/181-acp-runtime-replay-logical-time.test.ts` locks logical deadline
...
- `tests/core/182-acp-runtime-replay-publication-sidecar.test.ts` is a Node-only
...
- `tests/ui/183-acp-runtime-replay-publication-zotero-runtime.test.ts` exercises
  the real privileged host-to-shell-to-child frame chain for rendered ACP Chat
  and ACP Skills publication in Zotero.
- `tests/core/97-acp-ui-smoke.test.ts` locks the complementary source protocol:
...
测试成员集合。`tests/zotero/compatibility-matrix.json` 是版本、下载地址、
...
- `references/Zotero-7`：tag `7.0.32`，commit `188c54c186fbbaa6889145986d43ba64160a44fa`；
- `references/Zotero-9`：tag `9.0.6`，commit `7132587c2d6d56725debe64908733a8140bc6be3`；
- `references/Zotero-10`：tag `10.0.1`，commit `36749bd0bd4fdac9ee46c16f7aa7bed094a0851f`。
...
git submodule update --init --depth 1 \
  references/Zotero-7 references/Zotero-9 references/Zotero-10
git -C references/Zotero-10 describe --tags --exact-match
git -C references/Zotero-10 rev-parse HEAD
```

更新基线时只选择稳定 tag，先核对 tag 解析出的 commit，再更新 gitlink；同一
变更必须复查 `tests/zotero/compatibility-matrix.json`、本节和公开支持声明。嵌套
