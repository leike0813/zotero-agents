# `zotero-bridge navigation open-item`
...
```console
zotero-bridge navigation open-item [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--input <JSON_OR_FILE>]
```
...
```console
zotero-bridge navigation open-item --input '{"key":"ABCDEFGH","libraryId":1}'
```
...
```json
{
  ...
  "summary": "navigation open-item",
  "targets": [
    {
      "kind": "capability",
      "target": "navigation.open_item"
    }
  ]
}
```
