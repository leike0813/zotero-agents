(function () {
  ...
  function childModule(name, method) {
  ...
  function createChildRuntime(source) {
    ...
    const panelRenderer = childModule(
      "AssistantPanelRenderer",
      "renderAssistantPanelSnapshot",
    );
    const transcriptRenderer = childModule(
    ...
    function showTranscriptState(kind, message) {
      ...
      if (
        existing === kind &&
        text(elements.transcript.firstElementChild.textContent) ===
          text(message)
      ) {
        return true;
      }
    ...
    function ready() {
      return sendAction("ready", { documentGeneration: generation }, null);
    }
    ...
    window.addEventListener("message", function (event) {
      ...
      if (data.type === "assistant-workspace:acp-publication") {
  ...
  window.AssistantWorkspaceAcpChild = {
    boot,
    createClient,
    createController,
    createPageRequest,
