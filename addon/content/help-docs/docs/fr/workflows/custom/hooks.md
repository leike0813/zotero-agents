...
> **Note sur le filtrage d'entrée :** L'ancien hook `filterInputs` a été remplacé par le mécanisme déclaratif `validateSelection`. Utilisez `validateSelection` dans `workflow.json` pour définir les contraintes d'entrée sans JavaScript. Voir [Rédaction du Manifeste](#doc/workflows%2Fcustom%2Fmanifest#selection-validation) pour plus de détails.
...
- [Contexte de Sélection](#doc/workflows%2Fcustom%2Fselection-context) — Structure détaillée de selectionContext
- [Référence API Hôte](#doc/workflows%2Fcustom%2Fhost-api) — Référence API complète
- [Empaquetage & Déploiement](#doc/workflows%2Fcustom%2Fpackaging) — Comment empaqueter et déployer les workflows
