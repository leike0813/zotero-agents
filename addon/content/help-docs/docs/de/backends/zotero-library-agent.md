...
3. Das Verbindungsprofil mit der Endpunkt-URL und dem Bearer-Token konfigurieren. Detaillierte Einrichtung siehe [Host-Bridge-Konfiguration](#doc/backends%2Fhost-bridge).

## Nächste Schritte

- [Host Bridge](#doc/backends%2Fhost-bridge) — vollständige Referenz für die `zotero-bridge` CLI und Host-Bridge-Fähigkeiten
- [Hermes Profiles](#doc/backends%2Fhermes-profiles) — persistenter Bibliotheksdienst mit lokaler Indexierung und geplanter Wartung
- [Workflows](#doc/workflows%2Findex) — Übersicht aller eingebauten und benutzerdefinierten Workflows
