...
1. Abre **Tools → [Backend Manager](#doc/backends%2Fbackend-manager)**
...
→ Consulta [Configuración del backend ACP](#doc/backends%2Facp) para más detalles
...
2. Abre **Tools → [Backend Manager](#doc/backends%2Fbackend-manager)**
3. Cambia a la pestaña **Generic HTTP**, haz clic en **Add Generic HTTP**
4. Completa: Display Name `MinerU Official` · Base URL `https://mineru.net` · Authentication `bearer` · Auth Token: pega tu API Token · Timeout `600000`
5. Haz clic en **Save** en la esquina inferior derecha

→ Consulta la [Guía de uso de MineRU](#doc/workflows%2Fmineru) para más detalles

### Alternativa: Skill-Runner desplegado con Docker

Si necesitas ejecución persistente en segundo plano o compartición en red local, puedes [desplegar Skill-Runner con Docker](#doc/backends%2Fskill-runner#recommended-docker-persistent-deployment). Después del despliegue, añade una instancia de backend en la pestaña SkillRunner.

> Para instrucciones de operación detalladas, consulta [Backend Manager](#doc/backends%2Fbackend-manager).
...
→ Consulta [Lector de Markdown integrado](#doc/markdown-reader) para más detalles
...
→ Consulta [Literature Analysis](#doc/workflows%2Fliterature-analysis) para más detalles
...
→ Consulta [Literature Explainer](#doc/workflows%2Fliterature-explainer) para más detalles
...
→ Consulta [Deep Reading](#doc/workflows%2Fliterature-deep-reading) para más detalles
...
→ Consulta [Topic Synthesis](#doc/workflows%2Ftopic-synthesis) para más detalles

## Siguientes pasos

- **Procesamiento por lotes**: Ejecuta [Literature Analysis](#doc/workflows%2Fliterature-analysis) en artículos de tu biblioteca de forma masiva para construir la base de Synthesis
- **Sistema de etiquetas**: Usa [Tag Bootstrapper](#doc/workflows%2Ftag-bootstrapper) para crear un vocabulario controlado y estandarizar tus metadatos
- **Exploración de grafos**: Visualiza tu red de citaciones en el [Synthesis Workbench](#doc/synthesis%2Findex)
- **Desarrollo personalizado**: Consulta [Custom Workflows](#doc/workflows%2Fcustom%2Findex) para crear tus propios flujos de trabajo
