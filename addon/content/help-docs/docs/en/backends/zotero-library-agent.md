...
Zotero Library Agent is the bounded, on-demand task surface of the [Host Bridge](#doc/backends%2Fhost-bridge). It enables AI agents to operate a Zotero library for finite requests — inspecting items, retrieving context, reading literature and synthesis data, executing workflows, applying approved mutations, transferring files, and handing off evidence — without becoming a resident library-maintenance service.
...
3. Configure the connection profile with the endpoint URL and Bearer token. See [Host Bridge Configuration](#doc/backends%2Fhost-bridge) for detailed setup.

## Next Steps

- [Host Bridge](#doc/backends%2Fhost-bridge) — complete reference for the `zotero-bridge` CLI and Host Bridge capabilities
- [Hermes Profiles](#doc/backends%2Fhermes-profiles) — resident library service with local indexing and scheduled maintenance
