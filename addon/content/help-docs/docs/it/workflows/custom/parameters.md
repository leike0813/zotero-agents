...
Impostare `required: true` quando un workflow non può essere eseguito senza un parametro:
...
Le stringhe richieste devono essere non vuote, i numeri richiesti devono essere finiti ed entrambi i valori di un booleano richiesto sono validi. I valori richiesti mancanti vengono rifiutati prima del dispatch del provider. I parametri rimangono facoltativi quando `required` è `false` o omesso.
...
In `preflight`, `buildRequest` e `applyResult`, puoi leggere i valori dei parametri impostati dall'utente tramite `executionOptions.workflowParams`:
...
Vedere la pagina [Localizzazione](#doc/workflows%2Fcustom%2Flocalization) per il meccanismo completo di localizzazione.

## Prossimi passi

- [Contesto di selezione](#doc/workflows%2Fcustom%2Fselection-context) — Comprendere come la selezione degli elementi dell'utente viene passata al Workflow
- [Tipi di richiesta](#doc/workflows%2Fcustom%2Frequest-kinds) — Metodi di passaggio dei parametri per diversi tipi di richiesta
