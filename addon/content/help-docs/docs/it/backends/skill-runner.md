...
> **🏆 Priorità di raccomandazione**: Se hai già uno strumento agent compatibile con ACP sul tuo computer (Codex, OpenCode, Claude Code, ecc.), usa prima il [backend ACP](#doc/backends%2Facp), che non richiede alcuna configurazione aggiuntiva. Skill-Runner è adatto a scenari che richiedono un servizio persistente in background o la condivisione in LAN.
...
#### docker compose (Consigliato) {#recommended-docker-persistent-deployment}
...
> ⚠️ Questa modalità è adatta solo agli utenti che **non sanno come installare gli strumenti agent e non possono usare Docker**. Se sei in grado di installare CLI agent o usare Docker, preferisci il [backend ACP](#doc/backends%2Facp) o la distribuzione Docker descritta sopra.
...
1. Apri **Strumenti → [Backend Manager](#doc/backends%2Fbackend-manager)**
...
- [Scopri i Workflow](#doc/workflows%2Findex) — Skill-Runner è uno dei principali backend per l'esecuzione dei workflow
- [Introduzione alla Dashboard](#doc/dashboard) — Monitora lo stato di esecuzione delle attività
- [Scheda SkillRunner](#doc/sidebar%2Fskillrunner-tab) — Visualizza e interagisci con le esecuzioni di SkillRunner nella barra laterale
