...
Zotero Library Agent 是 [Host Bridge](#doc/backends%2Fhost-bridge) 的有界按需任务表面。它使 AI agent 能够对 Zotero 文献库执行有限请求——检查条目、检索上下文、读取文献和综合研究数据、执行 workflow、应用已审批的变更、传输文件以及交接证据——而不会成为常驻的文献库维护服务。
...
3. 使用端点 URL 和 Bearer token 配置连接 profile。详细设置请参阅 [Host Bridge 配置](#doc/backends%2Fhost-bridge)。

## 后续阅读

- [Host Bridge](#doc/backends%2Fhost-bridge) — `zotero-bridge` CLI 和 Host Bridge 能力的完整参考
- [Hermes Profiles](#doc/backends%2Fhermes-profiles) — 具有本地索引和定期维护的常驻文献库服务
- [Workflows](#doc/workflows%2Findex) — 所有内建和自定义 workflow 的概述
- [MCP Server](#doc/backends%2Fmcp-server) — MCP 兼容客户端的替代协议接口
