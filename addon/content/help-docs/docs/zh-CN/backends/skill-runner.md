...
> **🏆 推荐优先级**：如果你的本机已有支持 ACP 协议的 Agent 工具（Codex、OpenCode、Claude Code 等），请优先使用 [ACP 后端](#doc/backends%2Facp)，零额外配置。Skill-Runner 适合需要后台常驻服务或局域网共享的场景。
...
#### docker compose（推荐） {#recommended-docker-persistent-deployment}
...
> ⚠️ 此模式仅适合**完全不了解如何安装 Agent 工具、也不会使用 Docker** 的用户。如果你具备安装 Agent CLI 或 Docker 的能力，请优先选择 [ACP 后端](#doc/backends%2Facp) 或上方的 Docker 部署。
...
- [了解 Workflow](#doc/workflows%2Findex) — Skill-Runner 是执行 Workflow 的主要后端之一
- [Dashboard 介绍](#doc/dashboard) — 监控任务运行状态
- [SkillRunner Tab](#doc/sidebar%2Fskillrunner-tab) — 在侧边栏中查看和交互 SkillRunner 运行
