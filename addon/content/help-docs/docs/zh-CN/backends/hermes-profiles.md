...
**zotero-librarian** 是一个可直接安装的 [Hermes](https://github.com/anomalyco/hermes) Profile，让 AI 助手通过 [Host Bridge](#doc/backends%2Fhost-bridge) 来管理你的 Zotero 文献库。Profile 包含 Agent 所需的一切：`zotero-bridge` CLI、Host Bridge 连接配置模板、本地 SQLite 元数据索引、工作流目录缓存、运行监控脚本以及定时维护 cron 任务。

Profile 以独立包的形式从 Zotero Agents 仓库的 `host-bridge/zotero-librarian-profile` 分支发布。
...
- [工作流](#doc/workflows%2Findex) — 所有内置和自定义工作流概览
- [MCP Server](#doc/backends%2Fmcp-server) — 面向 MCP 兼容客户端的替代协议接口
