...
| `create.mode` | 执行模式。`"auto"`（非交互）或 `"interactive"`（交互，需要用户输入） |
...
| `mode` | **必需。** 执行模式：`"auto"`（非交互）或 `"interactive"`（需要用户输入） |
| `workspace` | 工作区策略。`"new"`（创建新工作区）、`"reuse-workflow"`（复用上级工作区） |
...
| `apply_result` | 步骤级结果应用：`workflow_id` 指定调用哪个子 workflow 的 `applyResult`；`on_failure` 控制失败行为（`"continue"` 或 `"fail_sequence"`） |
| `include_if` | 条件步骤执行。`{ kind: "parameter", parameter: "...", equals: ... }` 检查工作流参数，或 `{ kind: "runtime", condition: "..." }` 用于运行时条件 |
...
| `result` | 终止后使用什么作为结果：`"step_output"`（当前步骤的完整输出） |
...
| `step` | 源步骤 ID（从哪个步骤的输出中读取）。省略时从前一个相邻步骤读取 |
| `source` | 源步骤输出 JSON 中的字段名 |
| `target` | JSON 路径，值应写入当前步骤输入的什么位置（如 `"/input/field_name"`） |
| `required` | 为 `true` 时，源值缺失会导致步骤失败。默认 `false` |
| `value` | 用于 `kind: "value"`，传递字面值（当 `step`/`source` 省略时使用） |
...
- [调试与测试](#doc/workflows%2Fcustom%2Fdebugging) — 验证 workflow 的请求和响应
- [打包与部署](#doc/workflows%2Fcustom%2Fpackaging) — 将 workflow 发布给用户使用
