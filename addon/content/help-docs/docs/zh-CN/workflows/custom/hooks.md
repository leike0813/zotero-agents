...
> **关于输入过滤**：旧的 `filterInputs` Hook 已被声明式 `validateSelection` 机制替代。使用 `workflow.json` 中的 `validateSelection` 定义输入约束，无需编写 JavaScript。详见 [工作流清单文件编写](#doc/workflows%2Fcustom%2Fmanifest#selection-validation)。
