...
:::info 编辑分析结果
[文献分析](#doc/workflows%2Fliterature-analysis)生成的笔记内容是从后台数据**渲染**出来的，直接修改笔记内容不会改变后台数据。如果你需要修改分析结果，正确的做法是：**导出笔记** → 修改导出的文件 → 使用**导入笔记**重新导入。
:::
...
- [文献分析](#doc/workflows%2Fliterature-analysis) — 生成可导出的三类笔记
