...
→ 详见 [ACP 后端配置](#doc/backends%2Facp)
...
2. 打开 **工具 → [后端管理器](#doc/backends%2Fbackend-manager)**
...
→ 详见 [MinerU 使用文档](#doc/workflows%2Fmineru)

### 备选：Docker 部署 Skill-Runner

如果需要后台持续执行或局域网共享，可以 [Docker 部署 Skill-Runner](#doc/backends%2Fskill-runner#推荐docker-常驻部署)。部署后在 SkillRunner Tab 中添加后端实例。

> 详细的对话操作指南参见 [后端管理器](#doc/backends%2Fbackend-manager)。
...
→ 详见 [内置 Markdown 阅读器](#doc/markdown-reader)
...
→ 详见 [深度阅读](#doc/workflows%2Fliterature-deep-reading)
...
→ 详见 [Topic 综合创建](#doc/workflows%2Ftopic-synthesis)

## 下一步

- **批量化处理**：对库内文献批量执行 [文献分析](#doc/workflows%2Fliterature-analysis)，为 Synthesis 打基础
- **标签体系**：用 [Tag Bootstrapper](#doc/workflows%2Ftag-bootstrapper) 创建受控词表，让元数据更规范
- **图谱探索**：在 [Synthesis Workbench](#doc/synthesis%2Findex) 中可视化你的引文网络
- **自定义开发**：参考 [自定义 Workflow](#doc/workflows%2Fcustom%2Findex) 创建专属工作流
