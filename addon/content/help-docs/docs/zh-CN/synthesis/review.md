# 审核中心

Review 表面是集中处理 Synthesis 系统中各类待审核事项的地方。包含三个子标签页：**引用匹配**、**概念** 和 **主题图**。

<figure class="zs-doc-figure"><img src="chrome://zotero-skills/content/help-docs/assets/img/docs/synthesis/review.webp" alt="Synthesis Review 审核中心" title="Synthesis Review 审核中心" loading="lazy" /><figcaption>Synthesis Review 审核中心</figcaption></figure>

## 引用匹配审核

当系统自动匹配参考文献与 Zotero 条目时，无法确定性判定的匹配会作为提案进入审核队列。

### 匹配提案状态

| 状态 | 说明 |
|------|------|
| **待审核** | 系统生成的匹配候选，等待用户确认或拒绝 |
| **已接受** | 用户确认绑定，该引用已与 Zotero 条目关联 |
| **已拒绝** | 用户拒绝该绑定 |
| **已重新打开** | 之前处理过的提案重新开放审核 |

### 可执行的操作

- **接受（Accept）**：确认引用与条目的绑定关系
- **拒绝（Reject）**：拒绝该匹配提案
- **批量操作**：勾选多个提案后批量接受或拒绝

### 匹配置信度

参考 [索引与引文图谱](#doc/synthesis%2Findex-and-citation) 中的置信度级别说明。确定性（deterministic）和高置信度（high）的匹配通常自动处理，中等（medium）及以下置信度的匹配会进入审核。

### 筛选与排序

可以按以下条件筛选提案列表：

- 匹配状态（待审核/已接受/已拒绝）
- 匹配策略（DOI/标题/作者等）
- 置信度级别
- 按时间或相关性排序

## 概念审核

概念知识库的自动扩展可能会产生置信度较低的概念匹配建议，需要用户审核确认。

### 审核对象

- **新概念建议**：从文献中自动提取的新概念候选
- **概念义项确认**：为已有概念添加新的含义（义项）时需确认
- **别名建议**：检测到同一概念的替代名称时需确认

### 操作方式

每条建议显示概念名称、提取来源、置信度和支持证据。用户可：

- **接受**：确认该建议，写入概念 KB
- **拒绝**：忽略该建议
- **查看上下文**：查看该概念在文献中的出现位置

## 主题图审核

当系统检测到主题之间可能存在关系时，会生成关系提案供审核。

### 关系类型

| 关系 | 说明 |
|------|------|
| `broader_than` | A 是 B 的上位主题（更广泛） |
| `related_to` | 两个主题相关 |
| `overlaps_with` | 两个主题有内容重叠 |
| `contrasts_with` | 两个主题形成对比 |

### 提案内容

每条提案显示：

- **源主题和目标主题**的名称与描述
- **建议的关系类型**
- **置信度**（基于主题内容的语义分析）
- **支持证据**（共同覆盖的论文等）

### 操作方式

- **接受**：确认该关系，写入 Topic Graph
- **拒绝**：忽略该关系建议
- **重新打开**：将已处理的提案重新开放审核

## 下一步

- [概念知识库](#doc/synthesis%2Fconcepts) — 管理概念、义项、别名
- [Topics](#doc/synthesis%2Ftopic-synthesis) — 管理主题综合
