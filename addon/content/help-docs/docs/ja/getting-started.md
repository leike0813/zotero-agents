...
→ 詳細は[ACPバックエンド設定](#doc/backends%2Facp)を参照
...
永続的なバックグラウンド実行やLAN内共有が必要な場合は、[DockerでSkill-Runnerをデプロイ](#doc/backends%2Fskill-runner#recommended-docker-persistent-deployment)できる。デプロイ後、SkillRunnerタブでバックエンドインスタンスを追加する。

> 詳細な操作手順は[バックエンドマネージャー](#doc/backends%2Fbackend-manager)を参照。
...
→ 詳細は[組み込みMarkdownリーダー](#doc/markdown-reader)を参照
...
→ 詳細は[Literature Analysis](#doc/workflows%2Fliterature-analysis)を参照
...
→ 詳細は[Literature Explainer](#doc/workflows%2Fliterature-explainer)を参照
...
→ 詳細は[Deep Reading](#doc/workflows%2Fliterature-deep-reading)を参照
...
→ 詳細は[Topic Synthesis](#doc/workflows%2Ftopic-synthesis)を参照

## 次のステップ

- **バッチ処理**: ライブラリの論文に対して[Literature Analysis](#doc/workflows%2Fliterature-analysis)を一括実行し、統合の基盤を構築
- **タグシステム**: [Tag Bootstrapper](#doc/workflows%2Ftag-bootstrapper)を使用して制御語彙を作成し、メタデータを標準化
- **グラフ探索**: [Synthesis Workbench](#doc/synthesis%2Findex)で引用ネットワークを可視化
- **カスタム開発**: [Custom Workflows](#doc/workflows%2Fcustom%2Findex)を参照して独自のWorkflowを作成
