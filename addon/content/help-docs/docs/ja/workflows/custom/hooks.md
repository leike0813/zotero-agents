...
> **入力フィルタリングについて：** 古い `filterInputs` フックは宣言的な `validateSelection` メカニズムに置き換えられました。JavaScriptなしで入力制約を定義するには `workflow.json` で `validateSelection` を使用してください。詳細は [マニフェストファイルの作成](#doc/workflows%2Fcustom%2Fmanifest#selection-validation) を参照。
