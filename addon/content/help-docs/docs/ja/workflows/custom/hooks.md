...
> **入力フィルタリングについて：** 古い `filterInputs` フックは宣言的な `validateSelection` メカニズムに置き換えられました。JavaScript を書かずに入力制約を定義するには `workflow.json` で `validateSelection` を使用してください。詳細は [マニフェストファイルの作成](#doc/workflows%2Fcustom%2Fmanifest#selection-validation) を参照。
...
- [選択コンテキスト](#doc/workflows%2Fcustom%2Fselection-context) — selectionContext の詳細な構造
- [ホスト API リファレンス](#doc/workflows%2Fcustom%2Fhost-api) — 完全な API リファレンス
- [パッケージングとデプロイ](#doc/workflows%2Fcustom%2Fpackaging) — ワークフローのパッケージ化とデプロイ方法
