...
> **実行モード**（`auto` / `interactive`）は `request.create.mode` に移動された — [リクエスト種別](#doc/workflows%2Fcustom%2Frequest-kinds)を参照。
...
各 `kind` の詳細については[リクエスト種別](#doc/workflows%2Fcustom%2Frequest-kinds)を参照。
...
詳細については[多言語化](#doc/workflows%2Fcustom%2Flocalization)のページを参照。
...
- [フックシステム](#doc/workflows%2Fcustom%2Fhooks) — 各 Hook の API シグネチャと書き方を学ぶ
- [パラメータシステム](#doc/workflows%2Fcustom%2Fparameters) — パラメータ種別、enum 値、動的オプションソース
- [選択とコンテキスト](#doc/workflows%2Fcustom%2Fselection-context) — ユーザーが選択したアイテムの情報を取得する方法
