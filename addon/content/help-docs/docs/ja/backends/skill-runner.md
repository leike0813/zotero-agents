...
> **🏆 推奨優先度**: マシンにACP対応のエージェントツール（Codex、OpenCode、Claude Codeなど）がすでにある場合は、まず[ACPバックエンド](#doc/backends%2Facp)を使用されたい。追加の設定はゼロで済む。Skill-Runnerは永続的なバックグラウンドサービスやLAN共有が必要なシナリオに適している。
...
#### docker compose（推奨） {#recommended-docker-persistent-deployment}
...
> ⚠️ このモードは**エージェントツールのインストール方法を知らず、Dockerも使用できない**ユーザーにのみ適している。エージェントCLIのインストールやDockerの使用が可能であれば、[ACPバックエンド](#doc/backends%2Facp)または上記のDockerデプロイを優先されたい。
...
- [Workflowについて学ぶ](#doc/workflows%2Findex) — Skill-RunnerはWorkflowを実行する主要なバックエンドの一つ
- [ダッシュボードの概要](#doc/dashboard) — タスクの実行ステータスを監視
- [SkillRunnerタブ](#doc/sidebar%2Fskillrunner-tab) — サイドバーでSkillRunner実行を表示・操作
