...
Zotero Agentsは4つのバックエンドタイプをサポートし、それぞれ異なるユースケースに適している。
...
プラグインのローカルコンテキスト内でのみ実行されるWorkflow用で、リモートバックエンドや外部API呼び出しは関与しない。エクスポート/インポート操作、タグ監査、ファイル処理が代表的な例である。Pass-through Workflowはサーバー設定を一切必要とせず、プラグイン内で完全に実行される。

→ [カスタムWorkflowアーキテクチャ](#doc/workflows%2Fcustom%2Findex)
...
| **Pass-through** | なし（ローカル） | プラグイン内 | 必要に応じて | 純粋なローカル操作（エクスポート、タグ監査、ファイル処理） |
