...
- [MCPサーバー](#doc/backends%2Fmcp-server) — MCP対応クライアント（Claude Desktopなど）のための標準化されたプロトコルインターフェース
- [Hermes Profiles](#doc/backends%2Fhermes-profiles) — AIエージェントでZoteroライブラリを管理するためのすぐにインストール可能なプロファイル
- [環境設定](#doc/preferences) — Host Bridgeの全設定を表示
- [ACPバックエンド](#doc/backends%2Facp) — ACPエージェントの設定について学ぶ
