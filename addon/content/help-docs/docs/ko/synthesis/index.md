...
| 서피스 | 기능 | 문서 |
|---------|----------|------|
| **Home** | 라이브러리 개요 대시보드: 라이브러리 인사이트 (등록된 논문 수 / 토픽 수 / 그래프 노드), Git 동기화 상태 패널, 인기 토픽 카드 리스트 | [상세](#doc/synthesis%2Fhome) |
| **Topics** | 토픽 리스트 및 관리: 3가지 뷰 모드 (그래프 / 그리드 / 리스트), 토픽 생성 및 업데이트, 토픽 검색 및 정렬 | [상세](#doc/synthesis%2Ftopic-synthesis) |
| **Index** | 표준 참고문헌 인덱스: 논문 등록 뷰 (논문 리스트 + 인용 행 + 바인딩 상태), 표준 참고문헌 뷰 (검색 / 병합 / 리다이렉트 / 중복 제거) | [상세](#doc/synthesis%2Findex-and-citation) |
| **Review** | 리뷰 허브: 3개의 서브 탭 — 인용 매치 리뷰 (바인딩 제안 수락/거절), 개념 리뷰, 토픽 그래프 관계 리뷰 | [상세](#doc/synthesis%2Freview) |
| **Graph** | 인용 그래프 시각화 (force-directed / radial / component — 3가지 레이아웃), 토픽 필터링 및 노드/엣지 인터랙션 지원 | [상세](#doc/synthesis%2Findex-and-citation) |
| **Tags** | 제어된 태그 어휘 관리 + 자동 태그 제안 승인 | [상세](#doc/synthesis%2Ftags) |
| **Concepts** | 개념 지식 베이스 관리: 개념 / 의미 / 별칭 / 관계의 4계층 구조, 토픽 그래프와 리더에 오버레이 가능 | [상세](#doc/synthesis%2Fconcepts) |
| **Reader** | 토픽 리더: 8개의 서브 페이지(Overview, Taxonomy, Claims, Compare, Future Directions, Coverage, References, Report)를 갖춘 전체 토픽 상세 페이지 | [상세](#doc/synthesis%2Ftopic-synthesis) |
...
- 구성된 [Skill-Runner](#doc/backends%2Fskill-runner) 백엔드 (통합 Workflow 실행용)
- 라이브러리에 존재하는 논문 항목

## 다음 단계

- [홈 대시보드](#doc/synthesis%2Fhome) — 라이브러리 개요 및 동기화 상태 확인
- [태그 관리](#doc/synthesis%2Ftags) — 제어된 태그 어휘 관리
- [인덱스 & 인용 그래프](#doc/synthesis%2Findex-and-citation) — 참고문헌 인덱싱 및 인용 네트워크 알아보기
- [토픽 통합 생성](#doc/synthesis%2Ftopic-synthesis) — 토픽 분석 생성
- [리뷰 허브](#doc/synthesis%2Freview) — 인용 매치, 개념 및 토픽 그래프 제안 리뷰
- [개념 지식 베이스](#doc/synthesis%2Fconcepts) — 핵심 개념 관리
- [WebDAV 동기화](#doc/synthesis%2Fwebdav-sync) — 영구 데이터 동기화 구성
