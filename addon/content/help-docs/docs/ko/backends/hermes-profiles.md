...
**zotero-librarian**은 [Host Bridge](#doc/backends%2Fhost-bridge)를 통해 AI 에이전트가 Zotero 라이브러리를 관리할 수 있게 해주는 즉시 설치 가능한 [Hermes](https://github.com/anomalyco/hermes) 프로필입니다. 에이전트가 필요한 모든 것을 번들로 제공합니다: `zotero-bridge` CLI, Host Bridge 연결 프로필 템플릿, 로컬 SQLite 메타데이터 인덱스, 워크플로 카탈로그 캐시, 실행 모니터링 스크립트, 예약된 유지보수 cron 작업.
...
- [Host Bridge](#doc/backends%2Fhost-bridge) — `zotero-bridge` CLI 및 Host Bridge 기능에 대한 완전한 참조
- [워크플로](#doc/workflows%2Findex) — 모든 내장 및 사용자 정의 워크플로 개요
