...
Zotero Library Agent는 [Host Bridge](#doc/backends%2Fhost-bridge)의 유계(bounded) 온디맨드 작업 표면입니다. AI 에이전트가 유한한 요청에 대해 Zotero 라이브러리를 운영할 수 있게 합니다 — 항목 검사, 컨텍스트 검색, 문헌 및 합성 데이터 읽기, Workflow 실행, 승인된 변경 적용, 파일 전송, 증거 전달 — 상주 라이브러리 유지보수 서비스가 되지 않으면서.
...
3. 엔드포인트 URL과 Bearer 토큰으로 연결 프로파일을 구성합니다. 자세한 설정은 [Host Bridge 구성](#doc/backends%2Fhost-bridge)을 참조하십시오.

## 다음 단계

- [Host Bridge](#doc/backends%2Fhost-bridge) — `zotero-bridge` CLI와 Host Bridge 기능의 전체 참조
- [Hermes Profiles](#doc/backends%2Fhermes-profiles) — 로컬 인덱싱과 정기 유지보수를 갖춘 상주 라이브러리 서비스
- [Workflows](#doc/workflows%2Findex) — 모든 내장 및 사용자 정의 Workflow 개요
- [MCP Server](#doc/backends%2Fmcp-server) — MCP 호환 클라이언트를 위한 대체 프로토콜 인터페이스
