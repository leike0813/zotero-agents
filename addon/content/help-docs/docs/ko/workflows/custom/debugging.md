...
hostApi.notifications.toast({
  text: `buildRequest가 ${selectionContext.items.parents.length}개의 상위 항목을 수신했습니다`,
  type: "default",
});
```
...
- [전체 워크플로우 매니페스트 참조](#doc/workflows%2Fcustom%2Fmanifest) — workflow.json의 모든 필드
- [Host API 참조](#doc/workflows%2Fcustom%2Fhost-api) — 훅에서 사용 가능한 모든 API
