...
describe("workspace toolbar running tasks popover", function () {
  it("shares Dashboard active task filtering with the toolbar popover", async function () {
    ...
    const popover = await readProjectFile("src/modules/workspaceToolbarTaskPopover.ts");
  ...
  it("wires the Workspace toolbar button to a hover/focus popover without changing click behavior", async function () {
    const toolbar = await readProjectFile("src/modules/dashboardToolbarButton.ts");
    const popover = await readProjectFile("src/modules/workspaceToolbarTaskPopover.ts");
    ...
    assert.include(popover, "focus");
    assert.include(popover, "blur");
    ...
    assert.include(popover, 'xulElement(doc, "box", "zs-workspace-running-popover-separator")');
    ...
    assert.include(popover, 'xulElement(doc, "hbox", "zs-workspace-running-popover-task")');
    ...
    assert.include(popover, 'background-color: ${isPlainRunning ? "#2563eb" : "#f59e0b"}');
