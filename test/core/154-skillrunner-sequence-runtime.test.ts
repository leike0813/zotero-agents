...
describe("skillrunner.sequence.v1 runtime", function () {
  ...
  it("passes previous handoff by default and applies explicit mapping to downstream input", async function () {
    ...
    assert.deepInclude(
      (launched[1].input as Record<string, unknown>).handoff as Record<
        string,
        unknown
      >,
      {
    ...
    assert.deepInclude(launched[2].input as Record<string, unknown>, {
      static_value: "kept",
      manifest_path: "core-skill.handoff.json",
    });
    assert.deepInclude(launched[2].parameter as Record<string, unknown>, {
      language: "zh-CN",
      operation: "create",
    });
    assert.deepEqual(
      (launched[2].runtime_options as any).workflow_workspace,
      {
        mode: "reuse",
  ...
  it("builds literature-digest one-step and auto-tag two-step sequence requests", async function () {
    const tempRoot = await mkTempDir("zotero-skills-literature-digest-sequence");
    const parentItem = {
      id: 42,
      key: "PARENT42",
    ...
    assert.equal(digestOnly.steps[0].input.source_path, "D:/papers/sequence.md");
    ...
    assert.match(String(tagStep.input.valid_tags || ""), /valid_tags-parent-42/);
