...
function structuredArtifact(overrides: Record<string, unknown> = {}) {
  return {
    ...
    topic: {
      ...
      definition: "Object detection locates and classifies object instances in images.",
    ...
    summary: {
      brief: "对象检测主题综合。",
      overview: "该示例说明对象检测从传统 pipeline 走向端到端集合预测的最小结构化综合。",
    ...
    claims: [
      {
        ...
        analysis: "该判断来自 DETR 对集合预测的范式引入以及后续路线围绕查询、匹配和收敛展开的连续改进。",
      ...
      events: [
        {
          ...
          year: 2020,
          label: "DETR",
          description: "DETR 将检测问题转化为集合预测，为后续 query-based 路线提供起点。",
          phase: "paradigm_shift",
          why_it_matters: "它证明检测可以被重新表述为集合预测问题。",
          progression_logic: "后续工作主要围绕收敛速度、小目标性能和部署效率修正 DETR 的初始瓶颈。",
          follow_on_effect: "后续路线围绕 query、matching 和 efficiency 继续扩展。",
          evidence_refs: ["paper:1:DETR"],
          evidence_map_refs: ["claim:detector-evolution"],
        },
      ],
    ...
    positioning: {
      ...
      field_position: "该 topic 位于 computer vision detection 与 transformer vision models 的交叉处。",
      review_position: "可用于说明检测器从传统 pipeline 到 query-based set prediction 的方法迁移。",
      scope_boundary: {
    ...
    taxonomy: {
      ...
      axis_rationale: "对象检测方法的关键差异主要体现在候选生成、查询机制和匹配策略。",
      summary: {
      ...
      nodes: [
        {
          ...
          mechanism: "Transformer decoder query 与 Hungarian matching 建立预测集合和目标集合的一一对应。",
          representative_papers: ["paper:1:DETR"],
          main_contributions: ["证明 set prediction detection 可行。"],
          strengths: ["结构统一"],
          limitations: ["早期收敛慢"],
          maturity: "概念验证后进入效率优化阶段",
          relation_to_other_routes: "与 anchor/proposal 路线构成端到端建模对照。",
          review_angle: "可用于 Related Work 中解释 query-based detection 的范式起点。",
          paper_refs: ["1:DETR"],
      ...
      themes: [
        {
          ...
          analysis: "Transformer attention 为 DETR 的 query/key/value 建模提供方法背景。",
...
describe("Topic synthesis structured artifact contract", function () {
  it("accepts complete section manifests without run-workspace markdown preview inputs", async function () {
    ...
    const missingTitle = validateTopicSynthesisArtifact(
      structuredArtifact({
        synthesis_report: {
          ...
          body:
            "对象检测报告正文已经覆盖 topic 定义、研究路线、历史递进、核心结论、比较争议、覆盖缺口和库外文献建议，但缺少标题字段，因此不能进入持久化链路。该段文字故意写得足够长，用来确保失败原因来自 title，而不是 body 长度。".repeat(3),
        },
      }),
    );
