...
const ZOTERO_LITE_ALLOWLIST: Record<Exclude<TestDomain, "all">, RegExp[]> = {
  core: [
    ...
    /(^|\/)test\/core\/275-managed-note-transaction\.zotero\.test\.ts$/,
    /(^|\/)test\/core\/188-zotero-navigation\.zotero\.test\.ts$/,
  ],
  ui: [
    ...
    /(^|\/)test\/ui\/184-acp-skills-composer-continuation\.zotero\.test\.ts$/,
  ],
  workflow: [
