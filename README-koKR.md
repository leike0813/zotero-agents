...
| **Kilo Code Auto Free** | 무료 | 내장 Auto Free 모드가 각 요청을 적절한 무료 모델로 자동 라우팅합니다. API 키나 계정이 필요 없습니다 |
| **OpenCode Zen / OpenRouter 무료** | 무료 | OpenCode Zen은 내장 무료 모델을 제공합니다; OpenRouter도 무료 등급 모델(Gemini 2.5 Flash, DeepSeek V3 등)을 제공합니다. 속도 제한이 있지만 비용은 없습니다 |

### 무료 버전 제한 사항

무료 모델은 시작하기에 좋지만 다음과 같은 제한이 있습니다:

| 제한 사항 | 내용 |
|-----------|------|
| **속도 제한** | 제공업체 부하에 따라 분당 5~20회 요청으로 제한될 수 있습니다. 일괄 처리 시 현저히 느려질 수 있습니다 |
| **동시성** | 일반적으로 단일 동시 요청만 가능합니다. 여러 워크플로를 동시에 제출하면 대기열에 쌓이거나 실패할 수 있습니다 |
| **모델 가용성** | 피크 시간대에 무료 모델 풀이 소진될 수 있습니다. "모델을 사용할 수 없음" 또는 "용량 초과" 오류가 발생할 수 있습니다 |
| **모델 교체** | 제공업체가 사전 통지 없이 무료 모델을 조용히 교체(업그레이드 또는 다운그레이드)할 수 있습니다. 실행 간 출력 품질이 달라질 수 있습니다 |
| **SLA 없음** | 무료 등급은 가동 시간을 보장하지 않습니다. 서비스가 일시적으로 중단되거나 종료될 수 있습니다 |

> 안정적인 일괄 처리나 프로덕션 사용이 필요하시다면 유료 플랜(OpenCode Go 또는 Coding Plan)을 고려해 주세요 — 논문 한 편당 비용은 절약되는 시간에 비해 미미합니다.
...
| **Kilo Code** | 내장 Auto Free 모드 — 설정 없이 사용 가능한 무료 모델로 자동 라우팅. XDG 환경 변수를 통한 설정 분리 지원. 유료 API 키로도 사용 가능 | **무료** (Auto Free) | ⭐⭐⭐ 훌륭한 무료 옵션 |
| **Opencode** | Qwen3.5-Plus / Kimi-K2.5 / GLM-5 등 모델이 문헌 작업에서 뛰어난 성능을 발휘합니다. [OpenCode Go](https://opencode.ai/go?ref=SZDFT9GZKW)는 저렴한 할당량을 제공; Zen 에디션은 내장 무료 모델 포함; OpenRouter의 무료 모델도 사용 가능 | 무료 (Zen / OpenRouter) 또는 저비용 (Go) | ⭐⭐⭐ 강력 추천 |
