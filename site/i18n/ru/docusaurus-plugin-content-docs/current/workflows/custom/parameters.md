...
Установите `required: true`, когда workflow не может быть запущен без параметра:
...
Обязательные строки должны быть непустыми, обязательные числа должны быть конечными, оба значения обязательного булева типа допустимы. Отсутствующие обязательные значения отклоняются до диспетчеризации провайдера. Параметры остаются необязательными, когда `required` имеет значение `false` или опущен.
...
Полный механизм локализации см. на странице [Локализация](localization).

## Следующие шаги

- [Контекст выделения](selection-context) — Поймите, как выбор элементов пользователем передаётся в workflow
- [Типы запросов](request-kinds) — Методы передачи параметров для различных типов запросов
