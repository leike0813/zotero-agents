...
用户不需要运行 Tag Bootstrapper 来创建它们。Tag Bootstrapper 只新增自定义词表项；[标签规范化](../workflows/tag-regulator) 可以审计普通受控标签，但不能在文献条目上添加或删除内建 workflow 状态。两者都不得根据论文主题、语言、元数据或正文推断内建状态。
