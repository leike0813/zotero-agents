...
| `maxTopics` | 否 | 包含的相关主题最大数量，范围 0–10（默认：5）。 |
| `maxCorePapers` | 否 | 核心论文最大数量，范围 1–50（默认：20）。 |
| `maxRelatedPapers` | 否 | 非 Topic 额外论文最大数量，范围 1–200（默认：80）；选中 Topic 解析出的论文不受此上限淘汰。 |
...
1. 从用户接收论文意图参数。
2. 从现有 Synthesis Topics 和有界 Zotero 元数据锚点中发现候选材料。库检索匹配题名、作者、年份、刊名和标签等已索引元数据，不执行全文语义检索。
...
主题、图、分析产物或源不可用时，workflow 会使用仍可读取的证据继续，并记录诊断和警告。候选为零但发现证据不可用或格式异常时，Stage 40 会保留为可重试错误，不会报告 `no_related_literature`。只有确认发现结果为空，或完成评估后仍无论文满足条件，运行才会结束且不注册产品。
...
- [Literature Analysis](literature-analysis) — 生成可包含在包中的摘要和引文分析产物
- [Literature Search & Ingest](literature-search-ingest) — 在组装包之前搜索并入库缺失的文献
- [导出/导入文献包](export-import-literature-bundle) — 导出 Zotero 条目的便携式 ZIP 包（不同用途）
