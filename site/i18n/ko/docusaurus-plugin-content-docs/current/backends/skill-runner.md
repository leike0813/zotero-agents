...
> **🏆 Recommendation Priority**: If you already have an ACP-compatible agent tool on your machine (Codex, OpenCode, Claude Code, etc.), please use the [ACP backend](./acp) first, which requires zero additional configuration. Skill-Runner is suitable for scenarios requiring a persistent background service or LAN sharing.
...
> ⚠️ This mode is only suitable for users who **have no knowledge of how to install agent tools and cannot use Docker**. If you are capable of installing agent CLIs or using Docker, please prefer the [ACP backend](./acp) or the Docker deployment above.
...
1. Open **Tools → [Backend Manager](backend-manager)**
...
- [Learn about Workflows](../workflows/) — Skill-Runner is one of the main backends for executing workflows
- [Dashboard Introduction](../dashboard) — Monitor task execution status
- [SkillRunner Tab](../sidebar/skillrunner-tab) — View and interact with SkillRunner runs in the sidebar
