...
> **🏆 Priorité de recommandation** : Si vous avez déjà un outil d'agent compatible ACP sur votre machine (Codex, OpenCode, Claude Code, etc.), veuillez d'abord utiliser le [backend ACP](./acp), qui ne nécessite aucune configuration supplémentaire. Skill-Runner convient aux scénarios nécessitant un service persistant en arrière-plan ou un partage sur le réseau local.
...
#### docker compose (recommandé) {#recommended-docker-persistent-deployment}
...
> ⚠️ Ce mode convient uniquement aux utilisateurs qui **n'ont aucune connaissance de l'installation des outils d'agents et ne peuvent pas utiliser Docker**. Si vous êtes capable d'installer des CLIs d'agents ou d'utiliser Docker, veuillez préférer le [backend ACP](./acp) ou le déploiement Docker ci-dessus.
...
1. Ouvrez **Outils → [Gestionnaire de backends](backend-manager)**
...
- [Découvrir les workflows](../workflows/) — Skill-Runner est l'un des principaux backends pour exécuter des workflows
- [Introduction au tableau de bord](../dashboard) — Surveiller le statut d'exécution des tâches
- [Onglet SkillRunner](../sidebar/skillrunner-tab) — Consulter et interagir avec les exécutions SkillRunner dans la barre latérale
