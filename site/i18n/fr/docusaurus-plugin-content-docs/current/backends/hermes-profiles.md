...
Le modèle de profil utilise `auth.tokenEnv: "ZOTERO_BRIDGE_TOKEN"`, donc la CLI récupère automatiquement le jeton depuis l'environnement. Consultez la [Configuration de Host Bridge](host-bridge) pour une documentation détaillée sur le point de terminaison, le jeton et les fichiers de profil.
...
- [Host Bridge](host-bridge) — référence complète de la CLI `zotero-bridge` et des fonctionnalités de Host Bridge
- [Workflows](../workflows) — aperçu de tous les workflows intégrés et personnalisés
- [Serveur MCP](mcp-server) — interface de protocole alternative pour les clients compatibles MCP
