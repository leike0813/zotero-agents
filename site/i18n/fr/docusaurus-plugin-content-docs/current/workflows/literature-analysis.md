...
- **Extraire le Markdown d'abord** : Avant d'exécuter Literature Analysis, il est recommandé d'utiliser [MinerU](mineru) pour convertir d'abord le PDF en Markdown. Le Markdown original améliore significativement la compréhension par l'IA de la structure de l'article.
- **Initialiser le vocabulaire de balises d'abord** : Il est recommandé d'exécuter [Tag Bootstrapper](tag-bootstrapper) pour initialiser un vocabulaire de balises contrôlé avant votre première Literature Analysis. Cela permet à la régulation automatique des balises dans le pipeline d'analyse d'atteindre une efficacité maximale.
:::
...
![Note Résumé de Literature Analysis](/img/docs/workflows/literature-analysis_digest.png)

:::info À Propos du Contenu des Notes
Le contenu affiché dans la note est **rendu** à partir des données du backend. Modifier directement le contenu de la note dans Zotero **ne changera pas** les données réelles du backend. Pour modifier les résultats d'analyse, utilisez la fonction [Export/Import Notes](export-import-notes) pour exporter, modifier puis réimporter.
:::
...
![Note Références de Literature Analysis](/img/docs/workflows/literature-analysis_references.png)
...
![Note Analyse de Citation de Literature Analysis](/img/docs/workflows/literature-analysis_citation-analysis.png)
...
| `auto_tag_regulator` | boolean | S'il faut automatiquement enchaîner [Tag Regulator](tag-regulator) après l'analyse littéraire. **Activation recommandée** | `true` |
...
- [Tag Bootstrapper](tag-bootstrapper) — Initialiser un vocabulaire de balises contrôlé avant votre première analyse
- [MinerU](mineru) — Convertir d'abord le PDF en Markdown pour une qualité d'analyse optimale
- [Interactive Literature Explainer](literature-explainer) — Dialoguer avec l'IA pour une compréhension approfondie de la littérature
- [Export/Import Notes](export-import-notes) — Exporter les artefacts d'analyse pour édition, ou migrer entre instances Zotero
- [Tag Regulator](tag-regulator) — Exécuter la régulation de balises indépendamment (Literature Analysis peut s'enchaîner automatiquement)
