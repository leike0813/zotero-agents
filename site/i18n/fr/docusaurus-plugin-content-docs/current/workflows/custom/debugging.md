...
hostApi.notifications.toast({
  text: `buildRequest a reçu ${selectionContext.items.parents.length} éléments parents`,
  type: "default",
});
```
...
- [Référence Complète du Manifeste de Workflow](manifest) — Tous les champs dans workflow.json
