...
→ Voir [Configuration du backend ACP](backends/acp) pour plus de détails
...
2. Ouvrez **Outils → [Gestionnaire de backends](backends/backend-manager)**
...
→ Voir [Guide d'utilisation de MinerU](workflows/mineru) pour plus de détails

### Alternative : Skill-Runner déployé avec Docker

Si vous avez besoin d'une exécution persistante en arrière-plan ou d'un partage sur le réseau local, vous pouvez [déployer Skill-Runner avec Docker](backends/skill-runner#recommended-docker-persistent-deployment). Après le déploiement, ajoutez une instance de backend dans l'onglet SkillRunner.

> Pour les instructions détaillées, voir [Gestionnaire de backends](backends/backend-manager).
...
→ Voir [Lecteur Markdown intégré](markdown-reader) pour plus de détails
...
→ Voir [Literature Analysis](workflows/literature-analysis) pour plus de détails
...
→ Voir [Literature Explainer](workflows/literature-explainer) pour plus de détails
...
→ Voir [Lecture approfondie](workflows/literature-deep-reading) pour plus de détails
...
→ Voir [Synthèse de sujets](workflows/topic-synthesis) pour plus de détails

## Prochaines étapes

- **Traitement par lots** : Exécutez [Literature Analysis](workflows/literature-analysis) en masse sur les articles de votre bibliothèque pour construire les fondations de la synthèse
- **Système de balises** : Utilisez [Tag Bootstrapper](workflows/tag-bootstrapper) pour créer un vocabulaire contrôlé et standardiser vos métadonnées
- **Exploration du graphe** : Visualisez votre réseau de citations dans le [Synthesis Workbench](synthesis)
- **Développement personnalisé** : Consultez [Workflows personnalisés](workflows/custom/) pour créer vos propres workflows
