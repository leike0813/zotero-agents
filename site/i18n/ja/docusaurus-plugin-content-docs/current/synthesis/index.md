...
![Synthesis Workbench ホーム](/img/docs/synthesis/home.png)
...
| サーフェス | 機能 | ドキュメント |
|------------|------|-------------|
| **Home** | ライブラリ概要ダッシュボード：ライブラリインサイト（登録論文数/トピック数/グラフノード数）、Git 同期状態パネル、トレンドトピックカードリスト | [詳細](home) |
| **Topics** | トピックリストと管理：3 つのビューモード（グラフ/グリッド/リスト）、トピックの作成と更新、トピック検索と並び替え | [詳細](topic-synthesis) |
| **Index** | 正規参考文献インデックス：論文レジストリビュー（論文リスト + 引用行 + バインディング状態）、正規参考文献ビュー（検索/統合/リダイレクト/重複排除） | [詳細](index-and-citation) |
| **Review** | レビューハブ：3 つのサブタブ — 引用マッチレビュー（バインディング提案の承認/却下）、概念レビュー、トピックグラフ関係レビュー | [詳細](review) |
| **Graph** | 引用グラフの可視化（力指向/放射状/コンポーネント — 3 つのレイアウト）、トピックフィルタリングとノード/エッジ操作 | [詳細](index-and-citation) |
| **Tags** | 統制タグ語彙管理 + 自動タグ提案の承認 | [詳細](tags) |
| **Concepts** | 概念知識ベース管理：概念/意味/別名/関係の 4 層構造、トピックグラフやリーダーにオーバーレイ可能 | [詳細](concepts) |
| **Reader** | トピックリーダー：8 つのサブページ（Overview、Taxonomy、Claims、Compare、Future Directions、Coverage、References、Report）を持つ完全なトピック詳細ページ | [詳細](topic-synthesis) |
...
- 設定済みの [Skill-Runner](../backends/skill-runner) バックエンド（統合 Workflow の実行用）
- ライブラリに論文アイテムが既に存在していること

## 次のステップ

- [Home ダッシュボード](home) — ライブラリ概要と同期状態を確認
- [タグ管理](tags) — 統制タグ語彙を管理
- [インデックスと引用グラフ](index-and-citation) — 参考文献インデックスと引用ネットワークについて学ぶ
