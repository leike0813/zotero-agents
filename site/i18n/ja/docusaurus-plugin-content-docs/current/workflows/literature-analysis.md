...
:::tip ベストプラクティス
- **先にMarkdownを抽出**: Literature Analysisを実行する前に、[MinerU](mineru)を使用してPDFをMarkdownに変換することを推奨します。元のMarkdownは、論文構造のAI理解を大幅に向上させます。
- **先にタグ語彙を初期化**: 最初のLiterature Analysisの前に、[Tag Bootstrapper](tag-bootstrapper)を実行して統制されたタグ語彙を初期化することを推奨します。これにより、分析パイプライン内の自動タグ正規化が最大の効果を発揮します。
:::
...
![Literature Analysisダイジェストノート](/img/docs/workflows/literature-analysis_digest.png)

:::info ノート内容について
ノートに表示される内容は、バックエンドデータから**レンダリング**されたものです。Zoteroでノートの内容を直接変更しても、実際のバックエンドデータは**変更されません**。分析結果を編集するには、[Export/Import Notes](export-import-notes)機能を使用して、エクスポート→修正→再インポートを行ってください。
:::
...
![Literature Analysis参考文献ノート](/img/docs/workflows/literature-analysis_references.png)
...
![Literature Analysis引用分析ノート](/img/docs/workflows/literature-analysis_citation-analysis.png)
...
| `auto_tag_regulator` | boolean | 文献分析後に[Tag Regulator](tag-regulator)を自動的にカスケードするかどうか。**有効化を推奨** | `true` |
...
- [Tag Bootstrapper](tag-bootstrapper) — 最初の分析前に統制されたタグ語彙を初期化
- [MinerU](mineru) — PDFを先にMarkdownに変換して最高の分析品質を得る
- [Interactive Literature Explainer](literature-explainer) — AIとの対話による文献の深い理解
- [Export/Import Notes](export-import-notes) — 分析成果物をエクスポートして編集、またはZoteroインスタンス間で移行
- [Tag Regulator](tag-regulator) — タグ正規化を独立して実行（Literature Analysisから自動的にカスケード可能）
