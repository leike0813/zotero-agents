...
Die Profilvorlage verwendet `auth.tokenEnv: "ZOTERO_BRIDGE_TOKEN"`, sodass die CLI das Token automatisch aus der Umgebung übernimmt. Siehe [Host Bridge-Konfiguration](host-bridge) für detaillierte Dokumentation zu Endpunkt, Token und Profildateien.
...
- [Host Bridge](host-bridge) — vollständige Referenz für die `zotero-bridge`-CLI und Host Bridge-Funktionen
- [Workflows](../workflows) — Übersicht aller integrierten und benutzerdefinierten Workflows
