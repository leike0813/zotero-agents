...
3. Configurare il profilo di connessione con l'URL dell'endpoint e il token Bearer. Vedere [Configurazione del Host Bridge](host-bridge) per la configurazione dettagliata.

## Prossimi passi

- [Host Bridge](host-bridge) — riferimento completo della CLI `zotero-bridge` e delle capacità del Host Bridge
- [Hermes Profiles](hermes-profiles) — servizio di libreria residente con indicizzazione locale e manutenzione programmata
- [Workflows](../workflows) — panoramica di tutti i workflow integrati e personalizzati
- [MCP Server](mcp-server) — interfaccia di protocollo alternativa per client compatibili MCP
