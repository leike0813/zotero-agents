...
**zotero-librarian** es un perfil [Hermes](https://github.com/anomalyco/hermes) listo para instalar que permite a los agentes de IA gestionar su biblioteca Zotero a través del [Host Bridge](host-bridge). Incluye todo lo que un agente necesita: la CLI `zotero-bridge`, una plantilla de perfil de conexión Host Bridge, un índice local de metadatos SQLite, una caché de catálogo de workflows, scripts de monitoreo de ejecuciones y trabajos cron de mantenimiento programado.
...
- [Host Bridge](host-bridge) — referencia completa de la CLI `zotero-bridge` y las capacidades de Host Bridge
- [Workflows](../workflows) — descripción general de todos los workflows integrados y personalizados
- [Servidor MCP](mcp-server) — interfaz de protocolo alternativa para clientes compatibles con MCP
