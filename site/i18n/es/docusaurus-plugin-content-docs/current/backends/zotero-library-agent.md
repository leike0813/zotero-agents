...
3. Configurar el perfil de conexión con la URL del endpoint y el token Bearer. Ver [Configuración de Host Bridge](host-bridge) para configuración detallada.

## Próximos pasos

- [Host Bridge](host-bridge) — referencia completa de la CLI `zotero-bridge` y capacidades de Host Bridge
- [Hermes Profiles](hermes-profiles) — servicio de biblioteca residente con indexación local y mantenimiento programado
- [Workflows](../workflows) — resumen de todos los workflows integrados y personalizados
- [MCP Server](mcp-server) — interfaz de protocolo alternativa para clientes compatibles con MCP
