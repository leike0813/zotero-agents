...
`visible_if` 控制参数在设置对话框中的显示/隐藏。当 `equals` 为 `true` 时，目标参数值为 truthy 才显示；为 `false` 时，目标参数值为 falsy 才显示。
...
| `synthesis.topics` | Synthesis 工作台中的主题列表 | `filter`（all/updatable） |
...
在 `buildRequest`、`filterInputs` 和 `applyResult` 中，可以通过 `executionOptions.workflowParams` 读取用户设置的参数值：
