...
You do not need to run Tag Bootstrapper to create them. Tag Bootstrapper only adds custom vocabulary entries, while [Tag Regulator](../workflows/tag-regulator) may audit ordinary controlled tags but cannot add or remove builtin workflow statuses on literature items. Neither workflow may infer a builtin status from a paper's topic, language, metadata, or full text.
