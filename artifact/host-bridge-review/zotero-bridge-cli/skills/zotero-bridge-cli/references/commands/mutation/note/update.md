...
| --input | input | option | yes | — | JSON_OR_FILE | no | — | — | Note payload JSON 对象 |
...
```console
zotero-bridge mutation note update --input '{"content":{"format":"text","value":"Updated note"}}'
```
...
- 规范 argv 路径：`mutation` `note` `update`。
- 输出边界：`fixed`；受管辖细节：{"strategy":"fixed"}。
- 分页：`none`。
- 类别：`write`；危险级别：`review`。
- 结构化绑定模式：`overlay`。
- 意图可见性：`visible`。
- 操作别名：`mutation note update`、`mutation`、`note`、`update`、`NOTE`、`input`、`JSON_OR_FILE`。
