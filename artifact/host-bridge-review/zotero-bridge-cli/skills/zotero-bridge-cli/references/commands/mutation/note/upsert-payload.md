...
| Token | Id | Kind | Required | Conditional requirement | Values / arity | Repeatable | Environment | Conflicts | Help |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| --note | note | option | yes | — | NOTE | no | — | — | 目标 Zotero note ref |
| --input | input | option | yes | — | JSON_OR_FILE | no | — | — | Payload JSON 对象 |
...
```console
zotero-bridge mutation note upsert-payload --input '{"payload":{"format":"json","noteKind":"example","payloadType":"example","schemaVersion":"v1","value":{}}}'
```
...
- 规范 argv 路径：`mutation` `note` `upsert-payload`。
- 输出边界：`fixed`；受管辖细节：{"strategy":"fixed"}。
- 分页：`none`。
- 类别：`write`；危险级别：`review`。
- 结构化绑定模式：`overlay`。
- 意图可见性：`visible`。
- 操作别名：`mutation note upsert-payload`、`mutation`、`note`、`upsert-payload`、`NOTE`、`input`、`JSON_OR_FILE`。
