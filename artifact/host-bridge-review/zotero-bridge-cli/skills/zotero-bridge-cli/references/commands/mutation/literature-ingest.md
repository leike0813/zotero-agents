...
| Token | Id | Kind | Required | Conditional requirement | Values / arity | Repeatable | Environment | Conflicts | Help |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| --input | input | option | yes | — | JSON_OR_FILE | no | — | — | 文献摄取 payload。使用内联 JSON、包含 JSON 的文件路径、@file 语法或 '-' 从 stdin 读取 JSON。payload 必须是包含一个类型化 paper（itemType、fields、creators、identifiers）与可选 collection 的对象。 |
