...
- 规范 argv 路径：`debug` `synthesis` `snapshot`。
- 输出边界：`limit`；受管辖细节：{"defaultLimit":25,"fileField":"data.delivery.bundle","maxLimit":100,"section":"data.cacheBasis","strategy":"limit","truncatedField":"data.truncated"}。
- 分页：`none`。
- 类别：`debug`；危险级别：`none`。
- 结构化绑定模式：`passthrough`。
- 意图可见性：`hidden`。
- 操作别名：`debug synthesis snapshot`、`debug`、`synthesis`、`snapshot`、`input`、`JSON_OR_FILE`。
