...
- 规范 argv 路径：`synthesis` `resolver` `resolve`。
- 输出边界：`cursor`；受管辖细节：{"continuation":["data.nextCursor","data.hasMore","data.returned","data.total","data.limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"data.candidates","strategy":"cursor"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`passthrough`。
- 意图可见性：`visible`。
- 操作别名：`synthesis resolver resolve`、`synthesis`、`resolver`、`resolve`、`query`、`JSON_OR_FILE`。
