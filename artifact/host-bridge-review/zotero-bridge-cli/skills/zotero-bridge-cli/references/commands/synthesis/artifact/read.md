...
该命令返回解码后的 payload 内容。将 `error/decode_error` 行视为不可用证据，并保留其诊断，而不是解释原始 note 标记。
...
- 规范 argv 路径：`synthesis` `artifact` `read`。
- 输出边界：`file`；受管辖细节：{"fileField":"data.delivery.file","strategy":"file"}。
- 分页：`file`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`passthrough`。
- 意图可见性：`visible`。
- 操作别名：`synthesis artifact read`、`synthesis`、`artifact`、`read`、`query`、`JSON_OR_FILE`。
