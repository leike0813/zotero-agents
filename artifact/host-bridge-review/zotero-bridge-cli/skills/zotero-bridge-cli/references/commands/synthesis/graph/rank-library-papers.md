...
- 规范 argv 路径：`synthesis` `graph` `rank-library-papers`。
- 输出边界：`cursor`；受管辖的详情：{"continuation":["data.nextCursor","data.hasMore","data.returned","data.total","data.limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"data.papers","strategy":"cursor"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`passthrough`。
- 意图可见性：`visible`。
- 操作别名：`synthesis graph rank-library-papers`、`synthesis`、`graph`、`rank-library-papers`、`query`、`JSON_OR_FILE`。
