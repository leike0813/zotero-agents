...
- 规范 argv 路径：`synthesis` `concept` `query`。
- 输出边界：`limit`；受管辖细节：{"defaultLimit":25,"maxLimit":100,"section":"data.matches","strategy":"limit","truncatedField":"data.truncated"}。
- 分页：`none`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`passthrough`。
- 意图可见性：`visible`。
- 操作别名：`synthesis concept query`、`synthesis`、`concept`、`query`、`JSON_OR_FILE`。
