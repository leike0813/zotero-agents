...
列出缺少 PDF attachment 的 Zotero items
...
- 规范 argv 路径：`library` `readiness` `missing-pdf`。
...
- 结构化绑定模式：`overlay`。
- 意图可见性：`visible`。
- 操作别名：`library readiness missing-pdf`、`library`、`readiness`、`missing-pdf`、`query`、`JSON_OR_FILE`。
