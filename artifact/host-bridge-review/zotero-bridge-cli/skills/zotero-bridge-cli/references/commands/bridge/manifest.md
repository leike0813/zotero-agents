...
- 规范 argv 路径：`bridge` `manifest`。
- 输出边界：`cursor`；受管辖的详情：{"continuation":["nextCursor","hasMore","returned","total","limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"capabilities","strategy":"cursor"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`none`。
- 意图可见性：`visible`。
- 操作别名：`bridge manifest`、`bridge`、`manifest`、`cursor`、`CURSOR`、`limit`、`LIMIT`。
