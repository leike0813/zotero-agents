...
- 规范 argv 路径： `run` `permission` `pending`.
- 输出边界： `cursor`; governed details: {"strategy":"cursor","section":"permissions","defaultLimit":25,"maxLimit":100,"cursorInput":"cursor","continuation":["nextCursor","hasMore","returned","total","limit"]}.
- 分页： `cursor`.
- 类别： `read`; danger: `none`.
- 意图可见性： `visible`.
- 操作别名： `run permission pending`, `run`, `permission`, `pending`, `cursor`, `CURSOR`, `limit`, `LIMIT`.
### Effects
