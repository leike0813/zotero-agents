...
| Token | Id | Kind | Required | Conditional requirement | Values / arity | Repeatable | Environment | Conflicts | Help |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RUN_ID | run_id | positional | yes | — | RUN_ID | no | — | — | workflow run 的 id |
...
- 规范 argv 路径：`run` `get`。
- 输出边界：`cursor`；受管辖的详情：{"continuation":["pagination.skillRuns.nextCursor","pagination.skillRuns.hasMore","pagination.skillRuns.returned","pagination.skillRuns.total","pagination.skillRuns.limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"skillRuns","strategy":"cursor"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`none`。
- 意图可见性：`visible`。
- 操作别名：`run get`、`run`、`get`、`run_id`、`RUN_ID`、`cursor`、`CURSOR`、`limit`、`LIMIT`。
