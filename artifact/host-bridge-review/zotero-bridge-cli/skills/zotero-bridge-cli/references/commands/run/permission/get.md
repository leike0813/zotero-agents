...
读取一个 Zotero 侧 permission request
...
| Token | Id | Kind | Required | Conditional requirement | Values / arity | Repeatable | Environment | Conflicts | Help |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PERMISSION_REQUEST_ID | permission_request_id | positional | yes | — | PERMISSION_REQUEST_ID | no | — | — | permission request 的 id |
...
- 规范 argv 路径：`run` `permission` `get`。
- 输出边界：`fixed`；受管辖细节：{"strategy":"fixed"}。
- 分页：`none`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`none`。
- 意图可见性：`visible`。
- 操作别名：`run permission get`、`run`、`permission`、`get`、`permission_request_id`、`PERMISSION_REQUEST_ID`。
