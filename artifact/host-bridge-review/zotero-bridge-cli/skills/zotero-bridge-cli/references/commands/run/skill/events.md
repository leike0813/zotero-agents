...
| --since-updated-at | since_updated_at | option | no | — | SINCE_UPDATED_AT | no | — | — | 返回此 updatedAt 时间戳之后的事件 |
| --limit | limit | option | no | — | LIMIT | no | — | — | 事件数量上限 |
...
- 输出边界：`cursor`；受管辖详情：{"continuation":["nextCursor","hasMore","returned","total","limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"events","strategy":"cursor"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`none`。
- 意图可见性：`visible`。
