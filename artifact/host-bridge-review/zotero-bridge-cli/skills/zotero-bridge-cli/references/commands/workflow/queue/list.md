...
列出待处理的 Zotero 受管理 workflow 队列单元
...
| Token | Id | Kind | Required | Conditional requirement | Values / arity | Repeatable | Environment | Conflicts | Help |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| --backend-type | backend_type | option | no | — | BACKEND_TYPE | no | — | — | 按 backend 类型筛选：acp 或 skillrunner |
| --backend | backend | option | no | — | BACKEND | no | — | — | 按 backend id 筛选 |
| --cursor | cursor | option | no | — | CURSOR | no | — | — | 不透明的续接 cursor |
| --limit | limit | option | no | — | LIMIT | no | — | — | queue unit 最大数量（1-100） |
