...
当用户使用中文简称、领域缩写或可能映射到多个 Host Bridge 概念的 artifact 名称时，使用此参考。除非用户明确定义了不同的上下文，否则优先使用这些规范含义。

| 术语 | 规范含义 |
| --- | --- |
| Provider | 解释 provider 特定选项的 workflow 运行时族，如 ACP。 |
| Backend | 通过 `backendId` 为 provider 选择的已配置具体连接或可执行实例。 |
| Provider profile | 外部 Agent 的请求级预设，将 backend 选择与非敏感的 provider 特定 `providerOptions` 组合。Host Bridge 仅将其验证并应用于该请求；不存储或管理它。 |
