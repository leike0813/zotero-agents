...
| --- | --- | --- | --- |
| Minimum-core | `zotero-bridge-cli` | Exact CLI operations, schemas, handles, approvals, recovery, and generated command reference | Research-task selection and resident automation policy |
...
Review composition and rendering changes in `host-bridge/surfaces.json`, `scripts/host-bridge-surface-model.ts`, `scripts/render-host-bridge-surfaces.ts`, `scripts/materialize-host-bridge-surfaces.ts`, `scripts/check-host-bridge-skill-packages.ts`, release-set scripts and schemas, and the Host Bridge release workflow.
...
- Generic coordinator: `skills_src/zotero-library-agent/skills/zotero-library-agent/SKILL.md`, its direct reference, and its runner contract.
...
6. For minimum-core, verify exact command facts come from the CLI contract, every public descriptor field reaches the generated reference, and no research workflow is presented as an operational requirement.
7. For Generic, verify the coordinator routes one bounded objective to the appropriate task Skill. Each of the six Skills must independently define `zotero-library-task.result.v1`, inline evidence, the three status meanings, and the LLM/tool responsibility boundary. Cross-task workflow and handoff policy belongs to the coordinator reference.
...
A generated equivalent must expose the complete authoritative data needed by the agent. For the CLI command reference, this includes argv bindings, invocation/payload/result schemas, pagination, effects and state change, approval scope, handle transitions, recovery/next command, targets, aliases, and intent-search visibility.
