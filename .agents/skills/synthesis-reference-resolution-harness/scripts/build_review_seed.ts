...
function loadOptionalJson<T>(fixture: string, fileName: string): T | undefined {
  const filePath = path.join(fixture, fileName);
  if (!fs.existsSync(filePath)) {
    return undefined;
  }
  return readJson<T>(fixture, fileName);
}
...
function main() {
  ...
  const trustedLabelsFile = argValue(
    "--trusted-labels",
    "gold-labels.trusted-citekey.review.json",
  );
  const outFile = argValue("--out", "review-seed.json");
  const policy = (argValue("--policy", "production") ||
    "production") as ReferenceMatcherPolicyId;
  const seed = buildReviewSeedData({
    fixture,
    policy,
    sourceLabels: trustedLabelsFile,
    library: readJson(fixture, "library.json"),
    references: readJson(fixture, "references.json"),
    trustedLabels: loadOptionalJson(fixture, trustedLabelsFile),
    dangerPairs: loadOptionalJson(fixture, "danger-pairs.json"),
  });
  const outPath = path.isAbsolute(outFile)
    ? outFile
    : path.join(fixture, outFile);
  writeJson(outPath, seed);
  console.log(
    JSON.stringify(
      {
        ok: true,
        ...
        counts: {
          ...
          sourcePapers: seed.source_papers.length,
          references: seed.references.length,
          confirmedEdges: seed.edges.filter((edge) => edge.kind === "confirmed")
            .length,
          candidateEdges: seed.edges.filter((edge) => edge.kind === "candidate")
            .length,
          diagnostics: seed.diagnostics.length,
        },
      },
      null,
      2,
