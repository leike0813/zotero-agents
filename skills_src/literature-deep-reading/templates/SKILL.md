...
- 远程 SkillRunner profile 下，Host Bridge 的 `synthesis artifact export-filtered` 可能返回 `delivery.mode="bridge-download"` 而不是直接写本地 artifact 文件。runtime 会 fail closed 并写出 `runtime/payloads/*paper-artifacts-export-delivery.json`；遇到该错误时，读取这个 delivery JSON，执行其中的 `delivery.downloadCommand`，再执行 `delivery.unpackHint`，确认下载包已解包到当前 run workspace 后，重跑同一个 runtime command。
