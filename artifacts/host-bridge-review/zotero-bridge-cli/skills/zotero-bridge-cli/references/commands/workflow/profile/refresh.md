# `zotero-bridge workflow profile refresh`

刷新 ACP backend provider catalog
...
| Token | Id | Kind | Required | Conditional requirement | Values / arity | Repeatable | Environment | Conflicts | Help |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| --backend | backend | option | yes | — | BACKEND | no | — | — | 探测并刷新的已配置 ACP backend id |
...
```json
{
  ...
  "properties": {
    "backend": {
      "description": "Configured ACP backend id to probe and refresh",
      "type": "string"
    }
  },
  "required": [
...
```json
{
  ...
  "properties": {
    "response": {
      "additionalProperties": true,
      "description": "Response object returned by POST /bridge/v2/workflows/provider-profiles/refresh.",
      "type": "object",
      "x-openPropertiesReason": "The ACP refresh endpoint owns the response fields."
    }
  },
  "type": "object",
...
```json
{
  "approvalContract": {
    "kind": "none",
    "scope": "No Zotero library write approval; the ACP backend probe may start a provider session.",
    "timing": "none"
  },
  "arguments": [
    {
      ...
      "defaultValues": [],
      "global": false,
      "help": "Configured ACP backend id to probe and refresh",
      "id": "backend",
      "kind": "option",
      "possibleValues": [],
      "repeatable": false,
  ...
  "binding": "object",
  "category": "write",
  "command": "workflow profile refresh",
  "composition": null,
  "danger": "review",
  "effects": [
    {
      "description": "Refreshes the persisted ACP runtime option catalog for one backend.",
  ...
  "recovery": [
    {
      "action": "Inspect the structured refresh error and retry only when retryable is true.",
      "nextCommand": "workflow profile describe",
      "requiresHandles": [],
      "stateCheck": "none",
      "when": "The probe fails or its result is uncertain."
    }
  ],
  "resultSchema": {
    "additionalProperties": true,
  ...
  "summary": "Refresh an ACP backend provider catalog",
  "targets": [
    {
      "kind": "endpoint",
      "target": "POST /bridge/v2/workflows/provider-profiles/refresh"
    }
  ]
}
```
...
- 规范 argv 路径：`workflow` `profile` `refresh`。
- 输出边界：`fixed`；受管辖细节：{"strategy":"fixed"}。
- 分页：`none`。
- 类别：`write`；危险级别：`review`。
- 结构化绑定模式：`object`。
- 意图可见性：`visible`。
- 操作别名：`workflow profile refresh`、`workflow`、`profile`、`refresh`、`backend`、`BACKEND`。
