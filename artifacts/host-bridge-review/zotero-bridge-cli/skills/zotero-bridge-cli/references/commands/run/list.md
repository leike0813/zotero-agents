...
| Token | Id | Kind | Required | Conditional requirement | Values / arity | Repeatable | Environment | Conflicts | Help |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| --workflow | workflow | option | no | — | WORKFLOW | no | — | — | 按 workflow id 筛选 |
| --backend | backend | option | no | — | BACKEND | no | — | — | 按 backend id 筛选 |
| --backend-type | backend_type | option | no | — | BACKEND_TYPE | no | — | — | 按 backend 类型筛选 |
| --request | request | option | no | — | REQUEST | no | — | — | 按 provider request id 筛选 |
| --submission | submission | option | no | — | SUBMISSION | no | — | — | 按原生 workflow submission id 筛选 |
| --run | run | option | no | — | RUN | no | — | — | 按 workflow run id 筛选 |
| --state | state | option | no | — | STATE | no | — | — | 按任务状态筛选 |
| --active-only | active_only | option | no | — | ACTIVE_ONLY; values: true, false | no | — | — | 只返回活动的 task 运行时行 |
| --cursor | cursor | option | no | — | CURSOR | no | — | — | 不透明的续接 cursor |
| --limit | limit | option | no | — | LIMIT | no | — | — | 最大任务数（1-100） |
...
- 规范 argv 路径：`run` `list`。
- 输出边界：`cursor`；受管辖详情：{"continuation":["nextCursor","hasMore","returned","total","limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"items","strategy":"cursor"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`none`。
- 意图可见性：`visible`。
