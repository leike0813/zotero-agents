...
| --limit | limit | option | no | — | LIMIT | no | — | — | 最大 run 数 |
...
- 规范 argv 路径：`run` `workflow` `recent`。
- 输出边界：`cursor`；受管辖详情：{"continuation":["nextCursor","hasMore","returned","total","limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"runs","strategy":"cursor"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`none`。
- 意图可见性：`visible`。
- 操作别名：`run workflow recent`、`run`、`workflow`、`recent`、`WORKFLOW`、`limit`、`LIMIT`、`cursor`、`CURSOR`。
