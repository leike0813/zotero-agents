...
列出缺少文献分析生成 artifacts 的 Zotero items
...
- 规范 argv 路径：`library` `readiness` `missing-analysis`。
...
- 结构化绑定模式：`overlay`。
- 意图可见性：`visible`。
- 操作别名：`library readiness missing-analysis`、`library`、`readiness`、`missing-analysis`、`query`、`JSON_OR_FILE`。
