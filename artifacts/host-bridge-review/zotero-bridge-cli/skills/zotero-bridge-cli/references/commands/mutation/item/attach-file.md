...
将通过 Zotero Bridge 上传的文件附加到 Zotero item
...
| --file-id | file_id | option | yes | — | FILE_ID | no | — | — | Bridge 签发的已上传文件 id |
| --display-name | display_name | option | no | — | DISPLAY_NAME | no | — | — | 附件显示名称 |
| --content-type | content_type | option | no | — | CONTENT_TYPE | no | — | — | Attachment 内容类型 |
