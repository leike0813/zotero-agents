...
- 规范 argv 路径：`mutation` `collection` `remove-items`。
- 输出边界：`fixed`；受管辖细节：{"strategy":"fixed"}。
- 分页：`none`。
- 类别：`write`；危险级别：`review`。
- 结构化绑定模式：`object`。
- 意图可见性：`visible`。
- 操作别名：`mutation collection remove-items`、`mutation`、`collection`、`remove-items`、`COLLECTION`、`items`、`ITEMS`。
