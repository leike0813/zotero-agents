...
- 规范 argv 路径：`synthesis` `topic` `get-report`。
- 输出边界：`offset`；受管辖细节：{"continuation":["data.nextOffset","data.hasMore","data.totalChars","data.truncated","data.maxChars"],"cursorInput":"offset","defaultLimit":8000,"maxLimit":16000,"section":"data.text","strategy":"offset"}。
- 分页：`cursor`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`passthrough`。
- 意图可见性：`visible`。
- 操作别名：`synthesis topic get-report`、`synthesis`、`topic`、`get-report`、`query`、`JSON_OR_FILE`。
