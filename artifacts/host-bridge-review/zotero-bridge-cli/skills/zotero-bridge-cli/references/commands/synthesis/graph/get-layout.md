...
- 规范 argv 路径：`synthesis` `graph` `get-layout`。
- 输出边界：`limit`；受管辖详情：{"defaultLimit":25,"maxLimit":100,"section":"data.nodes","strategy":"limit","truncatedField":"data.diagnostics.truncated"}。
- 分页：`none`。
- 类别：`read`；危险级别：`none`。
- 结构化绑定模式：`passthrough`。
- 意图可见性：`visible`。
- 操作别名：`synthesis graph get-layout`、`synthesis`、`graph`、`get-layout`、`query`、`JSON_OR_FILE`。
