...
async function resolveAppliedNote(note: {
  ref: { libraryId: number; key: string };
}) {
  return Zotero.Items.getByLibraryAndKey(note.ref.libraryId, note.ref.key);
}
...
describe("workflow: literature-explainer", function () {
  ...
  it("creates a conversation note when note_path is bundle-relative", async function () {
    ...
    const note = await resolveAppliedNote((applied.notes || [])[0]);
    ...
    assert.isOk(payload);
    assert.equal(payload?.path, "mcp/conversation-note.md");
  ...
  itZoteroFullOrNode(
    ...
    async function () {
      ...
      if (
        applied.created_note_ref?.key !== (applied.notes || [])[0]?.ref?.key
      ) {
        throw new Error(
          `created_note_ref mismatch; created=${String(applied.created_note_ref?.key || "")}; noteKey=${String((applied.notes || [])[0]?.ref?.key || "")}`,
        );
      }
      const note = await resolveAppliedNote((applied.notes || [])[0]);
      ...
      if (payload.path !== "mcp/conversation-note.md") {
        throw new Error(
          `conversation payload path mismatch; expected=mcp/conversation-note.md; actual=${String(payload.path || "")}; noteId=${note.id}; noteHtml=${note.getNote()}`,
        );
      }
