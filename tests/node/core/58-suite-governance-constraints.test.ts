...
import { resolveSynthesisNativeStage1Suite } from "../../../scripts/synthesis/synthesis-native-stage1-suite";
...
describe("suite governance constraints", function () {
  ...
  it("Risk: node raw suite excludes Zotero runner aggregate suites", function () {
    ...
    assert.match(
      ...
      /--ignore\s+"tests\/zotero\/\*\*\/\*\.test\.ts"/i,
    );
  });
  ...
  it("Risk: the Synthesis Stage 1 milestone inventory stays complete and fail-closed", function () {
    const files = readdirSync(join(process.cwd(), "tests", "core"), {
      withFileTypes: true,
    })
      .filter((entry) => entry.isFile() && entry.name.endsWith(".test.ts"))
      .map((entry) => `tests/core/${entry.name}`);
    const suite = resolveSynthesisNativeStage1Suite(files);

    assert.equal(suite.files.length, 36);
    assert.equal(
      suite.files[0],
      "tests/core/175-synthesis-client-foundation.test.ts",
    );
    assert.equal(
      suite.files.at(-1),
      "tests/core/239-synthesis-sidecar-build-promotion.test.ts",
    );
    assert.deepEqual(
    ...
    assert.deepEqual(suite.segments[1].files, [
      "tests/core/193-synthesis-sidecar-runtime-packaging.test.ts",
      "tests/core/218-synthesis-cross-language-sidecar-contract.test.ts",
    ]);
      ...
      resolveSynthesisNativeStage1Suite(
        files.map((filePath) =>
          filePath.includes("/175-")
            ? "tests/core/175-unrelated-foundation.test.ts"
            : filePath,
        ),
      ),
    );
