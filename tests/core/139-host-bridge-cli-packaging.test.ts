...
} from "../../src/modules/hostBridge/cli/hostBridgeCliResolver";
import {
  installHostBridgeCli,
  resolveHostBridgeCliInstallTarget,
  hostBridgeCliInstallerInternalsForTests,
} from "../../src/modules/hostBridge/cli/hostBridgeCliInstaller";
...
} from "../../src/modules/hostBridge/cli/hostBridgeProfileStore";
...
} from "../../src/modules/hostBridge/cli/hostBridgeCliInstallPrompt";
import {
  syncHostBridgeCliPrebuildInternalsForTests,
  syncHostBridgeCliPrebuilds,
} from "../../scripts/host-bridge/sync-host-bridge-cli-prebuilds";
import { stageHostBridgeCliPrebuildSet } from "../../scripts/host-bridge/stage-host-bridge-cli-prebuilds";
import { buildHostBridgeAgentSurfaceDescriptor } from "../../scripts/host-bridge/host-bridge-agent-surface";
import {
  assertLockedHostBridgeCliIdentity,
  assertPrebuildSourceState,
  parsePrebuildCliArgs,
  prebuildZoteroBridgeCli,
} from "../../scripts/host-bridge/prebuild-zotero-bridge-cli";
...
async function createFreshnessFixture() {
  ...
  await writeTextFile(
    root,
    "rust/zotero-bridge/cli-build-recipe.json",
  ...
  await writeTextFile(
    root,
    "rust/zotero-bridge/Cargo.toml",
    '[package]\nname = "zotero-bridge"\nversion = "0.1.0"\n',
  );
  await writeTextFile(
    root,
    "rust/zotero-bridge/Cargo.lock",
    '[[package]]\nname = "zotero-bridge"\nversion = "0.1.0"\n',
  );
  await writeTextFile(root, "rust/zotero-bridge/src/main.rs", "fn main() {}\n");
  const governance =
    await import("../../scripts/host-bridge/host-bridge-cli-release-governance.mjs");
  const freshness =
    await import("../../scripts/host-bridge/check-host-bridge-cli-prebuild-freshness.mjs");
...
describe("host bridge cli packaging and install", function () {
  ...
  it("rolls back managed binaries and manifests after a partial install failure", async function () {
    ...
    try {
      ...
      assert.strictEqual(
        await fs.readFile(
          path.join(root, "releases", "host-bridge", "cli-release.json"),
          "utf8",
        ),
        oldManifest,
      );
  ...
  it("covers library commands in the generated command reference", async function () {
    const commands = await fs.readFile(
      path.join(process.cwd(), "rust/zotero-bridge/src/commands.rs"),
      "utf8",
    );
  ...
  it("covers synthesis topic context in the generated command reference", async function () {
    const cliArgs = await fs.readFile(
      path.join(process.cwd(), "rust/zotero-bridge/src/args.rs"),
      "utf8",
    );
    const wrapperReference = await fs.readFile(
  ...
  it("declares remote Host Bridge profile and master token preference controls", async function () {
    ...
    const docs = await fs.readFile(
      path.join(process.cwd(), "docs/host-bridge-cli.md"),
      "utf8",
    );
  ...
  it("keeps tracked POSIX Host Bridge artifacts executable", async function () {
    ...
    const release = JSON.parse(
      await fs.readFile(
        path.join(process.cwd(), "releases/host-bridge/cli-release.json"),
        "utf8",
      ),
    ) as {
  ...
  it("declares CLI release packaging workflow and addon bin directories", async function () {
    ...
    const recipe = JSON.parse(
      await fs.readFile("rust/zotero-bridge/cli-build-recipe.json", "utf8"),
    );
    ...
    assert.include(
      prebuildWorkflow,
      "node scripts/host-bridge/check-zotero-bridge-cli-binary-identity.mjs",
    );
    assert.include(
      prebuildWorkflow,
    ...
    assert.strictEqual(
      ...
      "tsx scripts/host-bridge/sync-host-bridge-cli-prebuilds.ts",
    );
    assert.strictEqual(
      packageJson.scripts["prebuild:zotero-bridge-cli"],
      "tsx scripts/host-bridge/prebuild-zotero-bridge-cli.ts",
    );
    assert.strictEqual(
      packageJson.scripts["build:local:zotero-bridge-cli"],
      "node scripts/host-bridge/build-zotero-bridge-cli.mjs",
    );
    assert.notInclude(
      packageJson.scripts["build:local:zotero-bridge-cli"],
      "github",
    ...
    assert.strictEqual(
      ...
      "node scripts/host-bridge/host-bridge-cli-release-governance.mjs status --json",
    );
    assert.strictEqual(
      packageJson.scripts["check:host-bridge-cli-prebuild-freshness"],
      "node scripts/host-bridge/check-host-bridge-cli-prebuild-freshness.mjs",
    );
    const releaseManifest = JSON.parse(
      await fs.readFile("releases/host-bridge/cli-release.json", "utf8"),
    );
  ...
  it("checks cross-compiled CLI identity without executing the target binary", async function () {
    ...
    try {
      ...
      const identity =
        await import("../../scripts/host-bridge/check-zotero-bridge-cli-binary-identity.mjs");
  ...
  it("declares the standalone Zotero Library Agent bundle surface", async function () {
    ...
    const surfaces = JSON.parse(
      await fs.readFile("contracts/host-bridge/surfaces.json", "utf8"),
    );
  ...
  it("keeps current Host Bridge documentation on the executable v2 identities", async function () {
    const currentDocumentation = await Promise.all(
      [
        ...
        "docs/components/host-bridge-agent-surfaces.md",
        "docs/components/host-bridge-capability-registry.md",
        "docs/components/host-bridge-lifecycle.md",
