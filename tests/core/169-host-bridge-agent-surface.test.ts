...
} from "../../scripts/host-bridge/host-bridge-agent-surface";
import { buildHostBridgeSurfaceCatalog } from "../../scripts/host-bridge/host-bridge-surface-catalog";
import {
...
describe("Host Bridge agent surface contract", function () {
  ...
  it("keeps low-level remote transport private to the contract executor", function () {
    ...
    const executor = fs.readFileSync(
      path.join(root, "rust/zotero-bridge/src/client.rs"),
      "utf8",
    );
    const transport = fs.readFileSync(
      path.join(root, "rust/zotero-bridge/src/transport.rs"),
      "utf8",
    );
    const commands = fs.readFileSync(
      path.join(root, "rust/zotero-bridge/src/commands.rs"),
      "utf8",
    );
  ...
  it("governs every command result and every structured input from one registry", function () {
    ...
    assert.property(registry.commands, "library saved-searches list");
    assert.notInclude(
  ...
  it("governs fixed capability payload composition with real Clap argument ids", function () {
    const registry = JSON.parse(
      fs.readFileSync(
        path.join(process.cwd(), "contracts/host-bridge/cli-commands.v2.json"),
        "utf8",
      ),
    );
  ...
  it("describes canonical commands with control and recovery metadata", function () {
    ...
    assert.strictEqual(commands.size, descriptor.commands.length);
    ...
    assert.deepEqual(
      commands
        ...
        .map((entry) => entry.handle),
      ["workflowRunId", "submissionId", "fileId"],
    );
    assert.deepEqual(
      commands
