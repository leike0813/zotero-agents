...
} from "../../scripts/host-bridge/host-bridge-review-mirror";
...
function createFixture() {
  ...
  const surfaces = {
    ...
    cliRelease: "releases/host-bridge/cli-release.json",
    surfaces: [
      {
  ...
  write(
    join(root, "contracts/host-bridge/surfaces.json"),
    `${JSON.stringify(surfaces)}\n`,
  );
  write(
    join(root, "releases/host-bridge/cli-release.json"),
    '{"version":"7.4.0"}\n',
  );
  write(
...
describe("Host Bridge review mirror", function () {
  ...
  it("defines a minimum-complete Skill with one comprehensive reference", function () {
    ...
    assert.include(skill, "contracts/host-bridge/surfaces.json");
