...
} from "../../scripts/host-bridge/host-bridge-release-set";
import {
  collectHostBridgeReleaseChangedFiles,
  createHostBridgeReleasePlan,
  resolveHostBridgeReleaseBase,
  writeHostBridgeReleasePlan,
} from "../../scripts/host-bridge/host-bridge-release-plan";
import {
  materializeHostBridgeSurfaces,
  stagedHostBridgePayloadDigests,
} from "../../scripts/host-bridge/materialize-host-bridge-surfaces";
import { resolveExactCliReleaseIntent } from "../../scripts/host-bridge/host-bridge-version-intent";
import { renderHostBridgeReleaseSet } from "../../scripts/host-bridge/render-host-bridge-release-set";
import {
  resolveHostBridgePublicationRef,
  readImmutablePublicationSource,
  selectDispatchedHostBridgeRun,
} from "../../scripts/host-bridge/dispatch-host-bridge-release";
import {
...
} from "../../scripts/host-bridge/host-bridge-release-controller";
...
describe("Host Bridge release coordinator", function () {
  ...
  it("uses the committed completed release identity as the accumulated baseline", function () {
    ...
    mkdirSync(join(root, "releases/host-bridge"), { recursive: true });
    writeFileSync(
      join(root, "releases/host-bridge/latest-complete-release-receipt.json"),
    ...
    assert.sameMembers(collectHostBridgeReleaseChangedFiles(root).files, [
      "releases/host-bridge/latest-complete-release-receipt.json",
      "one.txt",
      "two.txt",
    ]);
  });
  ...
  it("does not treat a planned release set as a completed baseline", function () {
    ...
    writeFileSync(
      join(root, "releases/host-bridge/release-set.json"),
  ...
  it("keeps prepared release sets independent of ambient workflow run identity", function () {
    const releaseSet = JSON.parse(
      readFileSync("releases/host-bridge/release-set.json", "utf8"),
    );
