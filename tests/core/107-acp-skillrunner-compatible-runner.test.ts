...
} from "../../src/modules/acp/skillRun/acpAgentFamilyResolver";
import {
...
import { readAcpSkillRunOutputRevisions } from "../../src/modules/acp/skillRun/acpSkillRunPayloadStore";
import {
...
} from "../../src/modules/acp/skillRun/acpSkillRunInteractionFiles";
import { insertAcpSkillProxyPatchBlock } from "../../src/modules/acp/skillRun/acpSkillReferenceRewriter";
import {
...
} from "../../src/modules/acp/skillRun/acpSkillRunnerOrchestrator";
import {
  applyHostBridgeCliEnvToBackend,
  materializeHostBridgeCliRunInjection,
} from "../../src/modules/hostBridge/cli/hostBridgeCliInjection";
import {
...
} from "../../src/modules/acp/skillRun/acpSkillPatchTemplates";
import {
  ACP_RUNTIME_PROMPT_TEMPLATES,
  ACP_RUNTIME_PROMPT_TEMPLATES_BY_ID,
  loadAcpRuntimePromptTemplate,
  renderAcpRuntimePromptTemplate,
} from "../../src/modules/acp/skillRun/acpRuntimePromptTemplates";
import { buildAcpStartupPromptPreamble } from "../../src/modules/acp/skillRun/acpStartupPromptPreambles";
import { rescanWorkflowRegistry } from "../../src/modules/workflow/catalog/workflowRuntime";
import {
  buildAcpSkillOutputRepairPrompt,
  validateAcpSkillFinalPayload,
} from "../../src/modules/acp/skillRun/acpSkillOutputValidator";
import { validateAcpSkillRunRequestAgainstSchemas } from "../../src/modules/acp/skillRun/acpSkillSchemaAssets";
import { resolveAcpSkillResultFileFallback } from "../../src/modules/acp/skillRun/acpSkillResultFileFallback";
import {
  createAcpSkillRunnerWorkspace,
  resetAcpWorkflowWorkspaceRegistryForTests,
} from "../../src/modules/acp/skillRun/acpSkillRunnerWorkspace";
import {
  appendAcpSkillRunAuditUpdate,
  flushAcpSkillRunAuditTrailWrites,
  flushAcpSkillRunAuditTrailWritesForTests,
} from "../../src/modules/acp/skillRun/acpSkillRunAuditTrail";
...
import type { AcpConnectionAdapter } from "../../src/modules/acp/transport/acpConnectionAdapter";
import {
  RequestError,
  type RequestPermissionOutcome,
} from "../../src/modules/acpProtocol";
import { setAssistantExecutionDisplayMode } from "../../src/modules/assistant/publication/assistantExecutionDisplayPolicy";
import {
  ACP_SKILLS_WORKSPACE_ADAPTER,
  readAcpSkillRunWorkspaceRegions,
} from "../../src/modules/acp/skillRun/acpSkillsWorkspaceSurface";
import {
...
import {
  ...
  handleChildAction,
} from "../../src/modules/assistant/workspace/assistantWorkspaceActionRouter";
import type { AssistantWorkspaceTab } from "../../src/shared/assistantWireContract";
import { type AssistantWorkspaceHostRuntime } from "../../src/modules/assistant/workspace/assistantWorkspaceSidebar";
...
} from "../../src/modules/acp/skillRun/acpSkillRunTranscriptStore";
...
} from "../../src/modules/workflow/catalog/workflowProductStore";
import {
...
} from "../../src/modules/acp/skillRun/acpSkillRunRuntimeCatalog";
import {
  getSelectedAcpSkillRunRequestId,
  selectAcpSkillRun,
} from "../../src/modules/acp/skillRun/acpSkillRunWorkspaceSelection";
import {
  hasAcpSkillRunController,
  registerAcpSkillRunController,
} from "../../src/modules/acp/skillRun/acpSkillRunControllerRegistry";
import { isEligibleForPostTerminalAcpSkillRunConversation } from "../../src/modules/acp/skillRun/acpSkillRunStatus";
import {
  resolveAcpSkillRunPermissionRequest,
  setAcpSkillRunPermissionRequest,
} from "../../src/modules/acp/skillRun/acpSkillRunPermissionQueue";

async function mkTempRoot() {
  return fs.mkdtemp(path.join(os.tmpdir(), "zs-acp-skillrunner-"));
}

async function readRunTranscriptItems(requestId: string) {
  return readCanonicalRunTranscriptItems(requestId);
}
...
async function readRunOutputRevisions(requestId: string) {
  await flushAcpSkillRunRuntimeFileWritesForTests();
  const record = getAcpSkillRunRecord(requestId);
  return readAcpSkillRunOutputRevisions(record?.runtimeDir);
}
...
describe("ACP SkillRunner-compatible runner", function () {
      ...
      updates: [
        {
          ...
          status: "pending",
          rawInput: {
            authorization: "Bearer audit-tool-token",
            query: "find papers",
          },
        },
        {
    ...
    try {
      const result = await executeAcpSkillRunnerJob({
        ...
        request: {
          ...
          fetch_type: "result",
          input: {
            authorization: "Bearer audit-prompt-token",
            normal: "visible",
          },
        },
        dependencies: {
      ...
      assert.equal(result.status, "succeeded");
      assert.equal(record?.auditTrail?.initialized, true);
      ...
      assert.notInclude(prompt, "audit-prompt-token");
      assert.include(prompt, "<redacted>");

      const updatesText = await waitForTextFile(files.acpUpdates, /tool_call/);
      assert.notInclude(updatesText, "audit-tool-token");
      const updates = parseNdjson(updatesText);
  ...
  it("does not fail ACP skill runs when .acp audit files cannot be written", async function () {
    const root = await mkTempRoot();
    const { entry } = await createSkill(root);
    const runtimeFile = path.join(root, "runtime-file");
    await fs.writeFile(runtimeFile, "not a directory", "utf8");
    ...
    try {
      ...
      const record = getAcpSkillRunRecord(result.requestId);
      assert.equal(result.status, "succeeded");
      assert.equal(record?.auditTrail?.initialized, false);
      assert.match(
        record?.auditTrail?.lastError || "",
        /directory|file|exist/i,
      );
    } finally {
      await flushAcpSkillRunAuditTrailWritesForTests();
      await fs.rm(root, { recursive: true, force: true });
    ...
    try {
      const result = await executeAcpSkillRunnerJob({
        ...
        dependencies: {
          ...
          createWorkspace: (args) =>
            import("../../src/modules/acp/skillRun/acpSkillRunnerWorkspace").then(
              (mod) =>
  ...
  it("settles received ACP transcript before hard timeout disconnect cleanup", async function () {
    ...
    try {
      ...
      const record = getAcpSkillRunRecord(result.requestId);
      const transcriptItems = await readRunTranscriptItems(result.requestId);
  ...
  it("fails recoverably without output repair when end_turn returns no assistant output", async function () {
    ...
    try {
      ...
      assert.deepEqual(
        await readRunOutputRevisions(record?.requestId || ""),
        [],
      );
      assert.include(stages, "acp-prompt-no-output");
      assert.notInclude(stages, "output-validation-failed");
      assert.notInclude(stages, "repair-started");
      const transcriptItems = await readRunTranscriptItems(
        record?.requestId || "",
      );
      assert.isTrue(
  ...
  it("exposes and resolves ACP permission requests through the run store", async function () {
    const root = await mkTempRoot();
    let resolved: unknown = null;
    try {
      upsertAcpSkillRun({
        requestId: "run-permission",
        status: "waiting_user",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
      ...
      const pendingPermissionItems = (
        await readRunTranscriptItems("run-permission")
      ...
      const resolvedTranscriptItems =
        await readRunTranscriptItems("run-permission");
      const resolvedPermissionItems = resolvedTranscriptItems.filter(
  ...
  it("cancels pending ACP permissions on controller removal and ignores stale actions", async function () {
    ...
    try {
      ...
      assert.equal(record?.replyState, "idle");
      const permissionItems = (
        await readRunTranscriptItems("run-stale-permission")
      ).filter((item: any) => item.kind === "permission");
  ...
  it("routes independent Skills reasoning through thought_level without folding plain models", async function () {
    const requestId = "run-runtime-independent-reasoning";
    const sessionId = "session-runtime-independent-reasoning";
    const modelSelections: string[] = [];
    const configSelections: string[] = [];
    upsertAcpSkillRun({
      requestId,
  ...
  it("marks tool updates with output payload as completed when ACP omits status", async function () {
    ...
    try {
      ...
      const tool = (await readRunTranscriptItems("run-tool-output")).find(
  ...
  it("keeps panel snapshot selection reads from mutating the globally selected ACP skill run", async function () {
    ...
    await selectAcpSkillRun("run-selected-original");
  ...
  it("surfaces workspace activity as visible status while ACP prompt is active", async function () {
    ...
    try {
      ...
      const statusItem = (
        await readRunTranscriptItems("run-workspace-activity")
      ).find(
  ...
  it("keeps assistant streaming message contiguous around workspace activity", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    try {
      upsertAcpSkillRun({
  ...
  it("keeps run updatedAt stable while appending streaming text chunks", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    try {
      upsertAcpSkillRun({
      ...
      const assistantMessages = (
        await readRunTranscriptItems("run-stream-updated-at-stable")
  ...
  it("keeps low-signal ACP skill success events out of transcript", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    try {
      for (const stage of [
      ...
      const transcript = await readRunTranscriptItems("run-noise");
      assert.isFalse(
        transcript.some(
          (item: any) =>
            item.kind === "status" &&
  ...
  it("keeps ACP Skills run lists metadata-only while selected snapshots carry transcript items", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    try {
      upsertAcpSkillRun({
  ...
  it("serves prompt-active selected snapshots from memory while JSONL persistence is blocked", async function () {
    const root = await mkTempRoot();
    const runtimeDir = path.join(root, ".acp");
    const blockedIndexPath = path.normalize(
      path.join(runtimeDir, "transcript.index.json"),
    );
    const blockedWriteStarted = createDeferred<void>();
    const releaseBlockedWrite = createDeferred<void>();
    const previousIOUtils = redefineGlobalProperty("IOUtils", {
      writeUTF8: async (targetPath: string, content: string) => {
        if (path.normalize(targetPath) === blockedIndexPath) {
          blockedWriteStarted.resolve();
          await releaseBlockedWrite.promise;
        }
    ...
    try {
      ...
      recordAcpSkillRunSessionUpdate("run-live-mirror", {
      ...
      const snapshot = await Promise.race([
        prepareAcpSkillRunPanelSnapshot({
          selectedRequestId: "run-live-mirror",
        }),
        delay(250).then(() => "timeout" as const),
      ]);
      if (snapshot === "timeout") {
        throw new Error(
          "Prompt-active transcript snapshot waited for JSONL persistence.",
        );
      }
      ...
      const diagnostics =
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-live-mirror");
  ...
  it("keeps lifecycle-open non-foreground transcript mirrors after prompt stop", async function () {
    const root = await mkTempRoot();
    ...
    try {
      ...
      recordAcpSkillRunSessionUpdate("run-release-a", {
        sessionId: "session-a",
        update: {
          sessionUpdate: "agent_message_chunk",
      ...
      await flushAcpSkillRunRuntimeFileWritesForTests();
      assert.isTrue(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-release-a")
          .mirrorLoaded,
      );

      upsertAcpSkillRun({
        requestId: "run-release-b",
        status: "running",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
      ...
      await selectAcpSkillRun("run-release-b");
      upsertAcpSkillRun({
        requestId: "run-release-a",
        activePrompt: false,
        applyResultState: "pending",
      ...
      const retained =
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-release-a");
      assert.isTrue(retained.mirrorLoaded);
      assert.isAtLeast(retained.itemCount, 1);

      await selectAcpSkillRun("run-release-a");
      const snapshot = await prepareAcpSkillRunPanelSnapshot({
        selectedRequestId: "run-release-a",
      });
      ...
      assert.isAtLeast(hydrated.itemCount, 1);
    } finally {
      resetAcpSkillRunsForTests();
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("releases lifecycle-settled non-foreground transcript mirrors and hydrates in the background when selected again", async function () {
    const root = await mkTempRoot();
    const runtimeA = path.join(root, ".acp-a");
    const runtimeB = path.join(root, ".acp-b");
    resetAcpSkillRunsForTests();
    try {
      upsertAcpSkillRun({
        requestId: "run-release-settled-a",
        status: "running",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
      ...
      recordAcpSkillRunSessionUpdate("run-release-settled-a", {
        sessionId: "session-a",
        update: {
          sessionUpdate: "agent_message_chunk",
          content: { type: "text", text: "persisted transcript" },
      ...
      await flushAcpSkillRunRuntimeFileWritesForTests();
      upsertAcpSkillRun({
        requestId: "run-release-settled-b",
        status: "running",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
      ...
      await selectAcpSkillRun("run-release-settled-b");
      upsertAcpSkillRun({
        requestId: "run-release-settled-a",
        status: "succeeded",
        activePrompt: false,
      ...
      const released = getAcpSkillRunTranscriptMirrorDiagnosticsForTests(
        "run-release-settled-a",
      );
      assert.isFalse(released.mirrorLoaded);
      assert.equal(released.itemCount, 0);
      assert.isTrue(released.needsHydrate);

      upsertAcpSkillRun({
        requestId: "run-release-settled-a",
        event: {
          stage: "apply-pending",
          message: "Apply is settling.",
          level: "info",
        },
      });
      const afterEvent = getAcpSkillRunTranscriptMirrorDiagnosticsForTests(
        "run-release-settled-a",
      );
      assert.isFalse(afterEvent.mirrorLoaded);
      assert.isTrue(afterEvent.needsHydrate);

      await selectAcpSkillRun("run-release-settled-a");
      const readySnapshot = await prepareAcpSkillRunPanelSnapshot({
        selectedRequestId: "run-release-settled-a",
      });
      assert.equal(
        readySnapshot.selectedRun?.requestId,
        "run-release-settled-a",
      );
      assert.equal(readySnapshot.transcriptRegion.status, "ready");
      assert.isTrue(
        (readySnapshot.transcriptRegion.page?.items || []).some(
          (item) =>
            item.itemKind === "message" &&
            String(item.text || "").includes("persisted transcript"),
        ),
  ...
  it("keeps lifecycle-open transcript mirror before appending recovered reply transcript", async function () {
    const root = await mkTempRoot();
    const runtimeA = path.join(root, ".acp-a");
    const runtimeB = path.join(root, ".acp-b");
    resetAcpSkillRunsForTests();
    try {
      ...
      recordAcpSkillRunSessionUpdate("run-recover-a", {
      ...
      await selectAcpSkillRun("run-recover-b");
      upsertAcpSkillRun({
        requestId: "run-recover-a",
        activePrompt: false,
        applyResultState: "pending",
      ...
      assert.isTrue(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-recover-a")
          .mirrorLoaded,
      );
  ...
  it("returns a selected completed run transcript page before full cold mirror hydrate completes", async function () {
    const root = await mkTempRoot();
    const runtimeA = path.join(root, ".acp-a");
    const runtimeB = path.join(root, ".acp-b");
    resetAcpSkillRunsForTests();
    try {
      upsertAcpSkillRun({
        requestId: "run-partial-complete-a",
        status: "running",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
      ...
      recordAcpSkillRunSessionUpdate("run-partial-complete-a", {
        sessionId: "session-partial",
        update: {
          sessionUpdate: "agent_message_chunk",
          content: { type: "text", text: "old persisted " },
        },
      } as any);
      await flushAcpSkillRunRuntimeFileWritesForTests();

      upsertAcpSkillRun({
        requestId: "run-partial-complete-b",
        status: "running",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
      ...
      await selectAcpSkillRun("run-partial-complete-b");
      upsertAcpSkillRun({
        requestId: "run-partial-complete-a",
        status: "succeeded",
        activePrompt: false,
      ...
      await selectAcpSkillRun("run-partial-complete-a");
      assert.isUndefined(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests(
          "run-partial-complete-a",
        ).hydrateState,
      ...
      const loadingSnapshot = await prepareAcpSkillRunPanelSnapshot({
        selectedRequestId: "run-partial-complete-a",
      ...
      assert.equal(loadingSnapshot.transcriptRegion.status, "loading");
      assert.isNull(loadingSnapshot.transcriptRegion.page);
      assert.isUndefined(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests(
          "run-partial-complete-a",
        ).hydrateState,
      ...
      assert.equal(snapshot.transcriptRegion.status, "ready");
      assert.isTrue(
        (snapshot.transcriptRegion.page?.items || []).some(
          (item) =>
            item.itemKind === "message" &&
      ...
      const warmed = getAcpSkillRunTranscriptMirrorDiagnosticsForTests(
        "run-partial-complete-a",
      );
      assert.isAtMost(warmed.coldMirrorCacheSize, 10);
    } finally {
      resetAcpSkillRunsForTests();
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("reads ACP Skills transcript page items with many indexed event ranges", async function () {
    const root = await mkTempRoot();
    const runtimeDir = path.join(root, ".acp");
    resetAcpSkillRunsForTests();
    try {
      upsertAcpSkillRun({
        requestId: "run-indexed-ranges",
        status: "running",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
      ...
      recordAcpSkillRunSessionUpdate("run-indexed-ranges", {
        sessionId: "session-indexed-ranges",
        update: {
          sessionUpdate: "agent_message_chunk",
      ...
      assert.include(String((item as any).text || ""), "start chunk-0");
      assert.include(String((item as any).text || ""), "chunk-39");
    } finally {
      resetAcpSkillRunsForTests();
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("keeps ACP Skills live transcript mirrors pinned while cold mirrors use a 10 slot LRU", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    try {
      upsertAcpSkillRun({
        requestId: "run-lru-live",
        status: "running",
        backendId: "backend-acp",
        backendType: "acp",
        workspaceDir: root,
        runtimeDir: path.join(root, ".acp-live"),
        activePrompt: true,
      });
      recordAcpSkillRunSessionUpdate("run-lru-live", {
        sessionId: "session-lru-live",
        update: {
          sessionUpdate: "agent_message_chunk",
          content: { type: "text", text: "live mirror" },
        },
      } as any);
      await flushAcpSkillRunRuntimeFileWritesForTests();

      for (let index = 0; index < 11; index += 1) {
        const requestId = `run-lru-cold-${index}`;
        upsertAcpSkillRun({
          requestId,
          status: "running",
          backendId: "backend-acp",
          backendType: "acp",
          workspaceDir: root,
          runtimeDir: path.join(root, `.acp-cold-${index}`),
          activePrompt: true,
        });
        recordAcpSkillRunSessionUpdate(requestId, {
          sessionId: `session-lru-cold-${index}`,
          update: {
            sessionUpdate: "agent_message_chunk",
            content: { type: "text", text: `cold mirror ${index}` },
          },
        } as any);
        await flushAcpSkillRunRuntimeFileWritesForTests();
        upsertAcpSkillRun({
      ...
      assert.isTrue(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-lru-live")
          .mirrorLoaded,
      );
      assert.isFalse(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-lru-cold-0")
          .mirrorLoaded,
      );
      assert.isTrue(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-lru-cold-10")
          .mirrorLoaded,
      );
      assert.equal(
        getAcpSkillRunTranscriptMirrorDiagnosticsForTests("run-lru-cold-10")
  ...
  it("materializes the implicit ACP Skills selection before transcript hydrate settles", async function () {
    ...
    try {
      ...
      assert.equal(readySnapshot.transcriptRegion.status, "ready");
      assert.equal(
        readySnapshot.transcriptRegion.page?.pageKey,
        "run-implicit-selection\ntail:80",
      );
      assert.isTrue(
        (readySnapshot.transcriptRegion.page?.items || []).some(
  ...
  it("does not use a stale transcript page request for another selected run", async function () {
    ...
    try {
      ...
      assert.notEqual(
        snapshot.transcriptRegion.page?.pageKey,
        "run-stale-page-a",
      );
      assert.isFalse(
        (snapshot.transcriptRegion.page?.items || []).some((item) =>
  ...
  it("streams ACP skill text updates naturally without dropping transcript data", async function () {
    ...
    try {
      ...
      const transcript = await readRunTranscriptItems("run-coalesced-updates");
      const thought = transcript.find((item) => item.kind === "thought");
      ...
      assert.isArray(visibleSnapshot.transcriptRegion.page?.items);
    } finally {
      unsubscribe();
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("suppresses ACP Skills text chunk notifications until a boundary when streaming render is disabled", async function () {
    ...
    try {
      ...
      assert.isTrue(transcript.some((item) => item.kind === "tool_call"));
    } finally {
      unsubscribe();
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("keeps ACP Skills usage updates on the direct selected-run transcript snapshot", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    setAssistantStreamingRenderEnabled(false);
    upsertAcpSkillRun({
    ...
    try {
      ...
      assert.isUndefined(
        (thoughtOnlySnapshot.transcriptRegion.page?.items || []).find(
          (item) =>
            item.itemKind === "thought" && item.text === "visible thinking",
        ),
      );
      ...
      assert.isArray(beforeBoundary.transcriptRegion.page?.items);
      assert.isUndefined(
        (beforeBoundary.transcriptRegion.page?.items || []).find(
          (item) =>
            item.itemKind === "message" &&
            item.role === "assistant" &&
            item.text === "partial",
      ...
      assert.isArray(afterBoundary.transcriptRegion.page?.items);
      const visibleMessage = (
        afterBoundary.transcriptRegion.page?.items || []
      ).find(
        (item) => item.itemKind === "message" && item.role === "assistant",
      );
      assert.equal(visibleMessage?.itemKind, "message");
      if (visibleMessage?.itemKind === "message") {
        assert.equal(visibleMessage.text, "partial");
        assert.equal(visibleMessage.status, "complete");
      }
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("shows ACP Skills workspace activity immediately on the direct selected-run transcript snapshot", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    setAssistantStreamingRenderEnabled(false);
    upsertAcpSkillRun({
    ...
    try {
      ...
      assert.notProperty(visibleSnapshot.selectedRun as any, "transcriptItems");
      assert.isArray(visibleSnapshot.transcriptRegion.page?.items);
      const visible = visibleSnapshot.transcriptRegion.page?.items || [];
      assert.isUndefined(
        visible.find(
          (item) =>
            item.itemKind === "message" &&
      ...
      const activity = visible.find(
        (item) =>
          item.itemKind === "status" && item.label === "workspace-activity",
      );
      assert.equal(activity?.itemKind, "status");
      if (activity?.itemKind === "status") {
  ...
  it("shows ACP Skills tool completion immediately when streaming render is disabled", async function () {
    const root = await mkTempRoot();
    resetAcpSkillRunsForTests();
    setAssistantStreamingRenderEnabled(false);
    upsertAcpSkillRun({
    ...
    try {
      ...
      let visibleTool = (
        await readRunTranscriptItems("run-streaming-tool-update-disabled")
      ).find(
        (item) => item.kind === "tool_call" && item.toolCallId === "tool-1",
      );
  ...
  it("auto-continues a detached recoverable running run after explicit connect", async function () {
    ...
    try {
      ...
      const recoveredTranscript = await readRunTranscriptItems(
        workspace.requestId,
      );
      assert.isFalse(
  ...
  it("runs output repair after a recovered workflow reply returns invalid output", async function () {
    ...
    try {
      ...
      assert.deepEqual(
        (await readRunOutputRevisions(workspace.requestId)).map(
          (entry) => entry.status,
        ),
        ["invalid", "pending"],
      );
  ...
  it("fails recovered workflow replies recoverably without repair on empty end_turn", async function () {
    ...
    try {
      ...
      assert.deepEqual(await readRunOutputRevisions(workspace.requestId), []);
      assert.include(stages, "acp-prompt-no-output");
  ...
  it("materializes skill, repairs invalid turn output, and returns provider result", async function () {
    ...
    assert.isArray(panelSnapshot.transcriptRegion.page?.items);
    ...
    assert.include(
      outputRevisions[0]?.candidateText || "",
      "This is not JSON.",
    );
    assert.include(outputRevisions[1]?.candidateText || "", "ok");
    ...
    const transcript = await readRunTranscriptItems(result.requestId);
  ...
  it("keeps interactive pending turn out of apply until a final reply arrives", async function () {
    ...
    const transcriptAfterInteractiveReply = await readRunTranscriptItems(
      result.requestId,
    );
    const pendingAssistantMessages = transcriptAfterInteractiveReply.filter(
      (item) => item.kind === "message" && item.role === "assistant",
    );
    assert.isTrue(
    ...
    assert.equal(capturedWaiting?.outputRevisionCount, 1);
    assert.isTrue(
    ...
    assert.isArray(finishedSnapshot.transcriptRegion.page?.items);
  ...
  it("bounds ACP skill run shutdown when controller disconnect never settles", async function () {
    this.timeout(5000);
    resetAcpSkillRunsForTests();
    upsertAcpSkillRun({
      requestId: "acp-shutdown-stalled-controller",
      backendId: "backend-acp",
      backendType: "acp",
      status: "waiting_user",
      conversationState: "active",
    ...
    registerAcpSkillRunController("acp-shutdown-stalled-controller", {
      cancel: async () => undefined,
      disconnect: async () => {
        disconnectCalled += 1;
    ...
    const record = getAcpSkillRunRecord("acp-shutdown-stalled-controller");
    assert.equal(record?.status, "waiting_user");
    assert.equal(record?.activePrompt, false);
    assert.equal(record?.conversationState, "closed");
    assert.equal(record?.conversationRecoveryState, "available");
  ...
  it("converges a live deferred reply final output and continues sequence apply", async function () {
    ...
    try {
      ...
      assert.deepEqual(
        (await readRunOutputRevisions(requestId)).map((entry) => entry.status),
        ["final"],
      );
      assert.notProperty(finished as any, "outputRevisions");
      assert.notProperty(finished as any, "transcriptItems");
