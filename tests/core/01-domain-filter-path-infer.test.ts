...
describe("domain filter path inference", function () {
  it("infers core domain from relative path", function () {
    const domain = inferDomainFromFilePath("tests/core/00-startup.test.ts");
  ...
  it("infers domain from absolute path", function () {
    const domain = inferDomainFromFilePath(
      "/workspace/zotero-agents/tests/core/00-startup.test.ts",
    );
    assert.equal(domain, "core");
  });
  ...
  it("infers domain from windows backslash path", function () {
    const domain = inferDomainFromFilePath(
      "C:\\workspace\\zotero-agents\\tests\\ui\\40-gui-preferences-menu-scan.test.ts",
    );
    assert.equal(domain, "ui");
  });

  it("returns all for unknown path", function () {
    const domain = inferDomainFromFilePath("tests/misc/unknown.test.ts");
  ...
  it("prunes non-retained Zotero core files from routine suite", function () {
    assert.equal(
      isZoteroRoutineAllowedFile(
        "tests/core/73a-skillrunner-deploy-lifecycle.test.ts",
        "lite",
      ),
      false,
    );
  ...
  it("allows stable Zotero full coverage-gate core files only in full", function () {
    assert.equal(
      isZoteroRoutineAllowedFile("tests/core/42-task-runtime.test.ts", "lite"),
      false,
    );
    assert.equal(
      isZoteroRoutineAllowedFile("tests/core/42-task-runtime.test.ts", "full"),
      true,
    );
  });
  ...
  it("prunes non-retained workflow files from Zotero routine suite", function () {
    assert.equal(
      isZoteroRoutineAllowedFile(
        "tests/workflow-custom-legacy/63-custom-import-protocol.test.ts",
        "lite",
      ),
      false,
    );
