...
import { clearSkillRunnerSkillDisplayRegistryMemoryForTests } from "../../src/modules/skillRunner/surface/skillRunnerSkillDisplayRegistry";
import {
...
describe("separated ACP and SkillRunner run stores", function () {
  ...
  it("projects SkillRunner pre-ready jobs without backend interaction", function () {
    ...
    const defensiveRequestReady = applySkillRunnerRunEvent({
      type: "backend.snapshot",
      backendId: "skillrunner-local",
      requestId: "sr-created-only",
    ...
    const terminal = applySkillRunnerRunEvent({
      type: "backend.snapshot",
      backendId: "skillrunner-local",
      requestId: "sr-created-only",
      state: "succeeded",
