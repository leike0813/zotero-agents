...
} from "../../scripts/host-bridge/host-bridge-surface-model";
import { renderHostBridgeSurfaces } from "../../scripts/host-bridge/render-host-bridge-surfaces";
...
describe("host bridge surface definitions", function () {
  ...
  it("declares the three-layer composition and generic skill mounts", function () {
    const manifest = JSON.parse(
      readFileSync(
        join(process.cwd(), "contracts/host-bridge/surfaces.json"),
        "utf8",
      ),
    );
  ...
  it("binds the exact seven-Skill addon inventory to the CLI identity", function () {
    ...
    const release = JSON.parse(
      readFileSync(
        join(process.cwd(), "releases/host-bridge/cli-release.json"),
        "utf8",
      ),
    );
