...
async function resolveCurrentHostBridgeCli(root = process.cwd()) {
  ...
  cliBuildPromise = (async () => {
    const governance =
      (await import("../../scripts/host-bridge/host-bridge-cli-release-governance.mjs")) as {
      ...
      try {
        await execFileAsync(
          ...
          [
            ...
            "--locked",
            "--manifest-path",
            path.join(root, "rust/zotero-bridge/Cargo.toml"),
            "--bin",
            "zotero-bridge",
            "--target-dir",
            targetDir,
        ...
        throw new Error(
          `Unable to build the current Host Bridge CLI. Install the Rust toolchain declared by rust/zotero-bridge/cli-build-recipe.json and retry.\n${detail.stderr || detail.message}`,
        );
      }
