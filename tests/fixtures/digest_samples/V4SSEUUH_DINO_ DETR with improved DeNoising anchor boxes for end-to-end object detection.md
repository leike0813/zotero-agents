...
- 计算成本 ：5-scale 模型 GFLOPS 高达 860，推理速度降至 10 FPS

- TTA 增益有限 ：与经典检测器相比，DINO 的测试时增强增益不显著（+0.1AP）

- 超参数敏感 ：去噪查询数量需仔细调优（100 对 CDN 查询最佳，过多反而下降）

- 复现细节 ：

- 代码将开源：https://github.com/IDEACVR/DINO

- 关键超参数：lr=1e-4，weight_decay=1e-4，batch_size=16（8×A100）

- 学习率调度：11/20/30 epoch 处×0.1（12/24/36 epoch 设置）

- 损失权重：分类 1.0，L1 5.0，GIOU 2.0

- CDN 参数：λ₁=1.0，λ₂=2.0，100 对 CDN 查询

- 动态去噪组：固定 DN 查询数，根据图像目标数动态调整组数

- 大模型训练：Objects365 预训练 26 epochs（64×A100），COCO 微调 18 epochs（16×A100）

## 分章节总结

### 1 Introduction

介绍目标检测任务背景，对比经典卷积检测器与 DETR 类检测器的发展现状。指出经典检测器（如 DyHead、HTC++）仍是 COCO 排行榜主导，而 DETR 类模型虽消除手工设计组件但性能落后。分析两大原因：（1）DETR 类模型性能确实落后（最佳